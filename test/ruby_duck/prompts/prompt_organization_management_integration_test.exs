defmodule RubberDuck.Prompts.PromptOrganizationManagementIntegrationTest do
  ...
  describe "Phase 2B.2: Prompt Organization & Management - Integration Testing" do
    test "complete prompt organization workflow with flexible categorization schemes" do
      ...
      {:ok, code_review_prompt} = Prompt.create_user_prompt(%{
        content: "Review the code for quality issues and suggest improvements. Focus on {{review_focus}} and check for {{quality_criteria}}.",
      ...
      {:ok, documentation_prompt} = Prompt.create_user_prompt(%{
        content: "Generate documentation for {{component_name}} including {{doc_sections}} and examples.",
      ...
      {:ok, testing_prompt} = Prompt.create_user_prompt(%{
      ...
      assert {:ok, hierarchical_result} = PromptOrganizer.organize_prompts_for_user(
        user_id,
        prompt_collection,
        hierarchical_options
      )
      ...
      assert {:ok, tag_result} = PromptOrganizer.organize_prompts_for_user(
        user_id,
        prompt_collection,
        tag_based_options
      )
      ...
      assert {:ok, custom_result} = PromptOrganizer.organize_prompts_for_user(
        user_id,
        prompt_collection,
        custom_options
      )
      ...
      assert {:ok, suggestions} = PromptOrganizer.get_organization_suggestions(
        user_id,
        prompt_collection,
        suggestion_options
      )
      ...
      assert {:ok, categorization_result} = PromptOrganizer.auto_categorize_prompt(
        code_review_prompt,
        user_context
      )
    ...
    test "advanced tagging system with hierarchical relationships and auto-suggestions" do
      ...
      {:ok, complex_prompt} = Prompt.create_user_prompt(%{
        content: "Analyze the code for security vulnerabilities and performance issues. Generate a comprehensive report with {{detail_level}} coverage.",
      ...
      assert {:ok, content_suggestions} = PromptTagManager.suggest_tags_for_prompt(
        complex_prompt,
        %{workflow_type: :code_review},
      ...
      assert {:ok, usage_suggestions} = PromptTagManager.suggest_tags_for_prompt(
        complex_prompt,
        user_context,
      ...
      assert {:ok, comprehensive_suggestions} = PromptTagManager.suggest_tags_for_prompt(
        complex_prompt,
        user_context,
      ...
      assert length(comprehensive_suggestions.suggested_tags) >= length(content_suggestions.suggested_tags)
      ...
      assert {:ok, application_result} = PromptTagManager.apply_tags_to_prompt(
        complex_prompt.id,
        tags_to_apply,
        user_id
      )
      ...
      assert {:ok, tag_analytics} = PromptTagManager.get_tag_usage_analytics(user_id, "security-review")
    ...
    test "template variable system with parsing, validation, and inheritance" do
      ...
      {:ok, base_template_prompt} = Prompt.create_user_prompt(%{
        content: "Analyze {{code_component}} for {{analysis_type}} issues. Use {{methodology}} approach and provide {{output_format}} results.",
      ...
      {:ok, derived_template_prompt} = Prompt.create_user_prompt(%{
        content: "Analyze {{code_component}} for security vulnerabilities. Use comprehensive approach and provide {{output_format|detailed}} results with {{additional_context}}.",
      ...
      assert {:ok, base_variables} = PromptTemplateManager.parse_template_variables(base_template_prompt.content)

      # Should extract all template variables
      assert length(base_variables) == 4  # code_component, analysis_type, methodology, output_format
      ...
      assert {:ok, derived_variables} = PromptTemplateManager.parse_template_variables(derived_template_prompt.content)
      ...
      assert output_format_var.required == false  # Has default value

      # Step 3: Test template structure validation
      assert {:ok, base_validation} = PromptTemplateManager.validate_template_structure(base_template_prompt.content)
      ...
      assert {:ok, template_definition} = PromptTemplateManager.create_template_definition(
      ...
      assert {:ok, inheritance_result} = PromptTemplateManager.apply_template_inheritance(
        base_template_prompt,
      ...
      assert {:ok, substitution_validation} = PromptTemplateManager.validate_variable_substitution(
      ...
      assert {:ok, invalid_validation} = PromptTemplateManager.validate_variable_substitution(
        base_template_prompt.content,
        invalid_variable_values
      )
    ...
    test "organization suggestions and intelligent categorization" do
      ...
      created_prompts = for prompt_data <- prompts_with_patterns do
        {:ok, prompt} = Prompt.create_user_prompt(Map.merge(prompt_data, %{
          tenant_id: tenant_id,
          user_id: user_id
        }))
        prompt
      ...
      assert {:ok, usage_suggestions} = PromptOrganizer.get_organization_suggestions(
        user_id,
        created_prompts,
        suggestion_options
      )
      ...
      assert {:ok, content_suggestions} = PromptOrganizer.get_organization_suggestions(
        user_id,
        created_prompts,
      ...
      assert {:ok, comprehensive_suggestions} = PromptOrganizer.get_organization_suggestions(
        user_id,
        created_prompts,
        %{analysis_type: :comprehensive}
      )
      ...
      assert {:ok, security_categorization} = PromptOrganizer.auto_categorize_prompt(
        List.first(created_prompts),  # Security prompt
        security_context
      )
    ...
    test "template inheritance and extension patterns" do
      ...
      {:ok, base_prompt} = Prompt.create_user_prompt(%{
        content: "Perform {{analysis_type}} analysis on {{target_component}}. Follow {{standards}} and generate {{deliverable_type}} report.",
      ...
      {:ok, security_derived_prompt} = Prompt.create_user_prompt(%{
        content: "Perform security analysis on {{target_component}}. Follow OWASP standards and generate comprehensive report with {{threat_model}} assessment.",
        name: "security_analysis_template",
        tenant_id: tenant_id,
        user_id: user_id,
      ...
      assert {:ok, extension_result} = PromptTemplateManager.apply_template_inheritance(
        base_prompt,
      ...
      assert {:ok, override_result} = PromptTemplateManager.apply_template_inheritance(
        base_prompt,
        security_derived_prompt,
      ...
      assert {:ok, merge_result} = PromptTemplateManager.apply_template_inheritance(
        base_prompt,
        security_derived_prompt,
      ...
      assert {:ok, inherited_definition} = PromptTemplateManager.create_template_definition(
        security_derived_prompt,
        variable_specs,
    ...
    test "performance validation for large prompt collections and organization operations" do
      ...
      large_prompt_collection = for i <- 1..100 do
        content_type = Enum.random(["analysis", "generation", "documentation", "testing", "debugging"])
        
        {:ok, prompt} = Prompt.create_user_prompt(%{
          content: "#{String.capitalize(content_type)} prompt #{i} with {{variable_#{i}}} and complex content for testing organization performance.",
      ...
      assert {:ok, organization_result} = PromptOrganizer.organize_prompts_for_user(
        user_id,
      ...
      assert organization_time_ms < 1000, "Organization time #{organization_time_ms}ms exceeds 1000ms target for 100 prompts"
      ...
      assert {:ok, suggestions} = PromptTagManager.suggest_tags_for_prompt(
        sample_prompt,
      ...
      assert suggestion_time_ms < 100, "Tag suggestion time #{suggestion_time_ms}ms exceeds 100ms target"
      ...
      assert {:ok, parsed_variables} = PromptTemplateManager.parse_template_variables(sample_prompt.content)
      ...
      assert variable_parsing_time_ms < 50, "Variable parsing time #{variable_parsing_time_ms}ms exceeds 50ms target"
    ...
    test "integration with Section 1 prompt storage foundation" do
      ...
      assert {:ok, three_tier_organization} = PromptOrganizer.organize_prompts_for_user(
        user_id,
      ...
      assert {:ok, system_tag_suggestions} = PromptTagManager.suggest_tags_for_prompt(system_prompt)
      assert {:ok, project_tag_suggestions} = PromptTagManager.suggest_tags_for_prompt(project_prompt)  
    ...
    test "backward compatibility with existing prompt management workflows" do
      ...
      {:ok, simple_prompt} = Prompt.create_user_prompt(%{
      ...
      assert {:ok, no_variables} = PromptTemplateManager.parse_template_variables(simple_prompt.content)
      assert Enum.empty?(no_variables)

      # Should organize simple prompts without issues
      assert {:ok, simple_organization} = PromptOrganizer.organize_prompts_for_user(
        user_id,
        [simple_prompt],
        %{scheme: :flat}
      )
      ...
      assert {:ok, simple_validation} = PromptTemplateManager.validate_template_structure(simple_prompt.content)
  ...
  describe "Integration Quality Requirements" do
    ...
    test "Section 1 foundation integration validation" do
      ...
      {:ok, test_prompt} = Prompt.create_user_prompt(%{
      ...
      assert {:ok, _organization} = PromptOrganizer.organize_prompts_for_user(user_id, [test_prompt])
      ...
      assert {:ok, _variables} = PromptTemplateManager.parse_template_variables(test_prompt.content)

      # Should integrate with existing PromptCategory resource
      {:ok, test_category} = PromptCategory.create(%{
