defmodule RubberDuck.Prompts.Services.LlmPromptSelectorTest do
  ...
  describe "LlmPromptSelector" do
    ...
    test "get_available_prompts returns three-tier organized prompts", %{user_id: user_id, project_id: project_id} do
      assert {:ok, organized_prompts} = LlmPromptSelector.get_available_prompts(user_id, project_id)
    ...
    test "get_available_prompts without project_id returns user and system prompts only", %{user_id: user_id} do
    ...
    test "search_prompts finds prompts across tiers with relevance ranking", %{user_id: user_id, project_id: project_id} do
      search_query = "testing"
      
      assert {:ok, search_results} = LlmPromptSelector.search_prompts(user_id, search_query, project_id)
    ...
    test "search_prompts with content exclusion searches only names and descriptions", %{user_id: user_id, project_id: project_id} do
      ...
      assert {:ok, search_results} = LlmPromptSelector.search_prompts(user_id, content_only_query, project_id, search_options)

      # Should find fewer results when excluding content from search
      assert {:ok, all_content_results} = LlmPromptSelector.search_prompts(user_id, content_only_query, project_id, %{include_content: true})
      
      assert length(search_results) <= length(all_content_results)
    end

    test "get_recent_prompts returns recently used prompts", %{user_id: user_id, project_id: project_id, user_prompt: user_prompt} do
      # Create usage record for user prompt
      {:ok, _usage} = PromptUsage.create(%{
        user_id: user_id,
    ...
    test "get_favorite_prompts returns frequently used prompts", %{user_id: user_id, project_id: project_id, user_prompt: user_prompt} do
    ...
    test "get_prompts_by_category filters prompts by category", %{user_id: user_id, project_id: project_id} do
      ...
      {:ok, categorized_prompt} = Prompt.create_user_prompt(%{
      ...
      assert {:ok, category_prompts} = LlmPromptSelector.get_prompts_by_category(user_id, category.id, project_id)
    ...
    test "caching improves performance for repeated queries", %{user_id: user_id, project_id: project_id} do
    ...
    test "search performance meets requirements for large collections", %{user_id: user_id, project_id: project_id, tenant_id: tenant_id} do
      ...
      assert {:ok, _search_results} = LlmPromptSelector.search_prompts(user_id, "test", project_id)
    ...
    test "three-tier access control works correctly" do
      ...
      {:ok, user1_prompt} = Prompt.create_user_prompt(%{
      ...
      {:ok, project1_prompt} = Prompt.create_project_prompt(%{
      ...
      assert user1_prompt.id not in user2_project1_ids  # Should NOT see User1's personal prompts
      assert project1_prompt.id in user2_project1_ids     # Should see Project1 prompts
      ...
      assert project1_prompt.id not in user1_no_project_ids  # Should NOT see project prompts without project context
    end
