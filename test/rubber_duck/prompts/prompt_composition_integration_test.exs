defmodule RubberDuck.Prompts.PromptCompositionIntegrationTest do
  ...
  describe "composition engine logic (2B.2.3)" do
    test "CompositionEngine performs hierarchical composition with proper ordering" do
      ...
      assert metadata.strategy_used in [:hierarchical_merge, :priority_override, :template_inheritance, :adaptive_composition]
    ...
    test "CompositionEngine handles batch composition efficiently" do
      ...
      assert {:ok, batch_result} = CompositionEngine.compose_prompts_batch(prompt_requests, context)
      
      # Validate batch result structure
      assert %{successes: successes, failures: failures, batch_metadata: batch_meta} = batch_result
    ...
    test "composition engine handles missing prompts gracefully with fallback" do
      ...
      assert {:ok, result} = CompositionEngine.compose_prompt("nonexistent_prompt", context, options)
  ...
  describe "variable interpolation (2B.2.4)" do
    ...
    test "VariableInterpolator prevents dangerous variable injection" do
      ...
      for dangerous_vars <- dangerous_variables do
        assert {:error, _reason} = VariableInterpolator.interpolate(content, dangerous_vars, context)
      end
    end
    ...
    test "VariableInterpolator validates variable names and prevents reserved words" do
      ...
      for reserved_vars <- reserved_variables do
        assert {:error, _reason} = VariableInterpolator.interpolate(content, reserved_vars, context)
      end
      ...
      invalid_names = [
        ...
        %{"invalid@name" => "value"}    # Special chars not allowed
      ]

      for invalid_vars <- invalid_names do
        assert {:error, _reason} = VariableInterpolator.interpolate(content, invalid_vars, context)
      end
    end
  ...
  describe "token optimization (2B.2.3)" do
    test "TokenOptimizer estimates tokens accurately for different models" do
      ...
      assert claude_tokens <= gpt4_tokens + 2  # Allow for small differences
    end
    ...
    test "TokenOptimizer provides compression recommendations" do
      ...
      recommendations = TokenOptimizer.get_compression_recommendations(verbose_content, "gpt-4", 0.3)
    ...
    test "token optimization preserves semantic integrity" do
      ...
      case TokenOptimizer.optimize(content, options) do
        {:ok, result} ->
          ...
          assert String.contains?(optimized_content, "algorithm") or String.contains?(optimized_content, "implementation")
    ...
    test "orchestrator handles validation failures gracefully" do
      params = %{
        ...
        composition_context: %{
          tenant_id: Ash.UUID.generate(),
          variables: %{
            "dangerous_var" => "<script>alert('test')</script>"  # Should trigger validation failure
          }
        },
      ...
      case PromptOrchestratorAgent.start_agent(params) do
        ...
        {:error, {error_type, _reason, fallback_result}} when error_type == :orchestration_failed_with_fallback ->
  ...
  describe "integration validation (2B.2.5-2B.2.6)" do
    ...
    test "token optimization works across different model types" do
      content = "This is a comprehensive test prompt that includes multiple sentences and various elements to test token optimization across different language models."
    ...
    test "caching system provides performance benefits" do
      ...
      case {result1, result2} do
        {{:ok, _}, {:ok, _}} ->
          ...
          assert time2 >= 0  # Just verify we got a time measurement
          assert time1 >= 0
  ...
  describe "performance and optimization validation (2B.2.3-2B.2.6)" do
    test "composition engine meets performance targets consistently" do
      ...
      composition_times = Enum.map(1..5, fn _i ->
      ...
      assert average_time_ms < 1000  # Under 1 second is reasonable for basic implementation
    end
    ...
    test "token optimization maintains quality across compression levels" do
      base_content = "This is a comprehensive prompt that contains multiple important elements and should be optimized carefully."
      ...
      for compression_ratio <- compression_levels do
        ...
        case TokenOptimizer.optimize(base_content, options) do
          {:ok, result} ->
            ...
            assert result.optimization_metadata.compression_ratio <= compression_ratio + 0.1  # Allow small variance
