defmodule RubberDuck.Prompts.WorkflowSystemIntegrationEndToEndTest do
  ...
  describe "Phase 6.2: Workflow System Integration - End-to-End Testing" do
    test "complete saved prompt selection and usage workflow in Reactor workflow contexts" do
      ...
      {:ok, code_review_system_prompt} = Prompt.create_system_prompt(%{
        content: "Review the {{code_component}} for {{review_focus}} issues. Consider {{team_standards}} when evaluating.",
        name: "code_review_system_template",
        tenant_id: tenant_id,
        description: "System-wide code review template for workflow steps"
      })

      {:ok, documentation_project_prompt} = Prompt.create_project_prompt(%{
        content: "Generate {{doc_type}} documentation for {{component_name}}. Target audience: {{target_audience}}.",
      ...
      {:ok, testing_user_prompt} = Prompt.create_user_prompt(%{
        content: "Create {{test_type}} tests for {{function_name}} covering {{test_scenarios}}. Use {{test_framework}}.",
      ...
      assert {:ok, code_review_suitable} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, code_review_context)
      ...
      code_review_prompt = Enum.find(system_prompts, fn p -> p.id == code_review_system_prompt.id end)
      ...
      assert {:ok, review_search_results} = WorkflowPromptSelector.search_workflow_prompts(
        user_id, 
        "review", 
        code_review_context
      )
      ...
      for prompt <- review_search_results do
        ...
        assert String.contains?(prompt.name, "review") or String.contains?(prompt.content, "review")
      end

      # Step 4: Test step-specific prompt recommendations
      assert {:ok, analysis_recommendations} = WorkflowPromptSelector.get_recommended_prompts_for_step(
        user_id,
        :analysis,
        code_review_context
      )
      ...
      assert {:ok, available_variables} = WorkflowPromptSelector.get_available_workflow_variables(documentation_context)
      ...
      assert {:ok, preparation_result} = WorkflowPromptSelector.prepare_prompt_for_workflow(
      ...
      assert {:ok, testing_preparation} = WorkflowPromptSelector.prepare_prompt_for_workflow(
      ...
      assert String.contains?(testing_preparation.prepared_content, "unit")  # From workflow context
      assert String.contains?(testing_preparation.prepared_content, "ExUnit")  # From workflow context
      assert String.contains?(testing_preparation.prepared_content, "calculate_user_score")  # From user variables
      assert String.contains?(testing_preparation.prepared_content, "valid input, edge cases")  # From user variables
      ...
      assert {:ok, workflow_usage_stats} = PromptUsageTracker.get_prompt_usage_stats(code_review_system_prompt.id, user_id)
      ...
      assert {:ok, _workflow_prompts} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, code_review_context)
      ...
      assert workflow_selection_time_ms < 250, "Workflow prompt selection #{workflow_selection_time_ms}ms exceeds 250ms target"

      # Test workflow search performance
      workflow_search_start = System.monotonic_time(:microsecond)

      assert {:ok, _search_results} = WorkflowPromptSelector.search_workflow_prompts(user_id, "review", code_review_context)
      ...
      assert workflow_search_time_ms < 200, "Workflow search #{workflow_search_time_ms}ms exceeds 200ms requirement"
    ...
    test "workflow context variable extraction and substitution" do
      ...
      {:ok, complex_prompt} = Prompt.create_user_prompt(%{
        content: """
      ...
      assert {:ok, available_variables} = WorkflowPromptSelector.get_available_workflow_variables(complex_workflow_context)
      ...
      user_variable_values = %{
        "quality_threshold" => "0.9"  # Override default
      }

      assert {:ok, complex_preparation} = WorkflowPromptSelector.prepare_prompt_for_workflow(
      ...
      assert String.contains?(prepared_content, "code_review")  # workflow_type
      assert String.contains?(prepared_content, "comprehensive_quality_analysis")  # step_name
      assert String.contains?(prepared_content, project_id)  # project_id
      assert String.contains?(prepared_content, "security and performance")  # analysis_type
      assert String.contains?(prepared_content, "authentication, data validation")  # focus_areas
      assert String.contains?(prepared_content, "0.9")  # quality_threshold (user override)
    ...
    test "workflow step recommendations and context awareness" do
      ...
      {:ok, analysis_prompt} = Prompt.create_user_prompt(%{
        content: "Analyze the code structure and identify potential issues in {{component_name}}",
      ...
      {:ok, generation_prompt} = Prompt.create_user_prompt(%{
        content: "Generate comprehensive {{output_type}} for {{input_component}} following {{style_guide}}",
      ...
      {:ok, validation_prompt} = Prompt.create_user_prompt(%{
        content: "Validate {{validation_target}} against {{validation_criteria}} and verify {{compliance_requirements}}",
      ...
      analysis_prompt_ids = Enum.map(analysis_recommendations.recommended_prompts, fn p -> p.id end)
      assert analysis_prompt.id in analysis_prompt_ids

      # Analysis prompt should have high step relevance
      recommended_analysis = Enum.find(analysis_recommendations.recommended_prompts, fn p -> p.id == analysis_prompt.id end)
      ...
      assert {:ok, generation_recommendations} = WorkflowPromptSelector.get_recommended_prompts_for_step(
        user_id,
        :generation,
        documentation_context
      )
      ...
      generation_prompt_ids = Enum.map(generation_recommendations.recommended_prompts, fn p -> p.id end)
      ...
      assert {:ok, validation_recommendations} = WorkflowPromptSelector.get_recommended_prompts_for_step(
        user_id,
        :validation,
        testing_context
      )
      ...
      validation_prompt_ids = Enum.map(validation_recommendations.recommended_prompts, fn p -> p.id end)
    ...
    test "workflow-specific search with relevance ranking" do
      ...
      {:ok, high_relevance_prompt} = Prompt.create_user_prompt(%{
        content: "Comprehensive code review focusing on security vulnerabilities and performance optimization",
      ...
      {:ok, medium_relevance_prompt} = Prompt.create_user_prompt(%{
        content: "Review the implementation for basic code quality and readability improvements",
      ...
      {:ok, low_relevance_prompt} = Prompt.create_user_prompt(%{
        content: "Generate documentation for the user interface components and styling guidelines",
      ...
      assert {:ok, code_review_search} = WorkflowPromptSelector.search_workflow_prompts(
        user_id,
        "review",
        code_review_context
      )
      ...
      relevance_scores = Enum.map(code_review_search, fn prompt -> 
      ...
      if Map.has_key?(relevance_scores, high_relevance_prompt.id) and 
        ...
        assert relevance_scores[high_relevance_prompt.id] > relevance_scores[medium_relevance_prompt.id]
      end
      ...
      assert {:ok, doc_search} = WorkflowPromptSelector.search_workflow_prompts(
        user_id,
        "documentation",
        documentation_context
      )
      ...
      assert low_relevance_prompt.id in doc_prompt_ids  # UI documentation prompt should be found
    end
    ...
    test "performance validation with workflow integration overhead" do
      ...
      workflow_prompts = for i <- 1..200 do
        workflow_type = Enum.random([:code_review, :documentation, :testing, :refactoring, :debugging])
        
        {:ok, prompt} = Prompt.create_user_prompt(%{
          content: "Workflow prompt #{i} for #{workflow_type} with {{parameter_#{i}}} and {{context_#{i}}}",
      ...
      assert {:ok, suitable_prompts} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, performance_context)
      ...
      assert workflow_suitable_time_ms < 300, "Workflow suitable prompts #{workflow_suitable_time_ms}ms exceeds 300ms target"
      assert suitable_prompts.total_count > 0

      # Test workflow search performance with large collection
      workflow_search_start = System.monotonic_time(:microsecond)

      assert {:ok, search_results} = WorkflowPromptSelector.search_workflow_prompts(
        user_id,
        "code_review",
        performance_context
      )
      ...
      assert workflow_search_time_ms < 250, "Workflow search #{workflow_search_time_ms}ms exceeds 250ms target"
      ...
      assert {:ok, step_recs} = WorkflowPromptSelector.get_recommended_prompts_for_step(
        user_id,
        :analysis,
        performance_context
      )
      ...
      assert step_recommendations_time_ms < 150, "Step recommendations #{step_recommendations_time_ms}ms exceeds 150ms target"
      ...
      assert {:ok, _preparation_result} = WorkflowPromptSelector.prepare_prompt_for_workflow(
        sample_prompt.content,
        performance_context,
        %{"parameter_1" => "test_value", "context_1" => "test_context"}
      )

      variable_substitution_time = System.monotonic_time(:microsecond) - variable_substitution_start
      variable_substitution_time_ms = variable_substitution_time / 1000

      # Variable substitution should be fast
      assert variable_substitution_time_ms < 50, "Variable substitution #{variable_substitution_time_ms}ms exceeds 50ms target"
    end
    ...
    test "workflow integration with Section 6.1 infrastructure reuse" do
      ...
      {:ok, shared_prompt} = Prompt.create_user_prompt(%{
        content: "Shared prompt for both LLM operations and workflow steps with {{shared_variable}}",
      ...
      assert {:ok, workflow_prompts} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, workflow_context)

      workflow_prompt_ids = (workflow_prompts.user_prompts || [])
      ...
      PromptUsageTracker.track_llm_usage(user_id, shared_prompt.id, workflow_usage_context, %{workflow_integration: true})

      # Allow tracking to process
      Process.sleep(100)

      # Should track usage in both contexts
      assert {:ok, shared_usage_stats} = PromptUsageTracker.get_prompt_usage_stats(shared_prompt.id, user_id)

      assert shared_usage_stats.total_uses >= 2  # Both LLM and workflow usage
    end
    ...
    test "backward compatibility with existing workflow patterns" do
      ...
      assert {:ok, empty_prompts} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, basic_workflow_context)
      ...
      assert {:ok, empty_recommendations} = WorkflowPromptSelector.get_recommended_prompts_for_step(
        user_id,
        :analysis,
        basic_workflow_context
      )
      ...
      assert {:ok, context_variables} = WorkflowPromptSelector.get_available_workflow_variables(basic_workflow_context)
  ...
  describe "Integration Quality Requirements" do
    test "comprehensive workflow integration component validation" do
      ...
      assert Code.ensure_loaded?(WorkflowPromptSelector), "WorkflowPromptSelector should be loaded"
    ...
    test "workflow integration maintains Section 6.1 performance standards" do
      ...
      assert {:ok, _workflow_prompts} = WorkflowPromptSelector.get_workflow_suitable_prompts(user_id, workflow_context)
      ...
      assert workflow_perf_time_ms < 300, "Workflow prompt selection #{workflow_perf_time_ms}ms degrades Section 6.1 performance"
      ...
      assert {:ok, _search_results} = WorkflowPromptSelector.search_workflow_prompts(
        user_id,
        "performance",
        workflow_context
      )
      ...
      assert workflow_search_perf_time_ms < 250, "Workflow search #{workflow_search_perf_time_ms}ms degrades Section 6.1 performance"
    end
  end
end