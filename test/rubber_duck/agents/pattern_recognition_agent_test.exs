defmodule RubberDuck.Agents.PatternRecognitionAgentTest do
  ...
  describe "recognize_patterns/4" do
    ...
    test "recognizes success patterns successfully", %{agent: agent} do
      ...
      {:ok, patterns, updated_agent} =
        PatternRecognitionAgent.recognize_patterns(
          agent,
          :success_patterns,
          evaluation_data
        )
  ...
  describe "analyze_focused_patterns/4" do
    ...
    test "analyzes multiple focus areas successfully", %{agent: agent} do
      ...
      {:ok, focused_patterns, updated_agent} =
        PatternRecognitionAgent.analyze_focused_patterns(
          agent,
          focus_areas,
          data_context
        )
  ...
  describe "get_recognition_stats/1" do
    ...
    test "tracks statistics after pattern recognition" do
      ...
      assert length(updated_stats.recent_analysis_history) >
               length(initial_stats.recent_analysis_history)
    end
  end
  ...
  describe "reset_learning_models/2" do
    ...
    test "can reset with history clearing" do
      ...
      agent_with_history = %{agent | state: %{agent.state | analysis_history: [%{test: "data"}]}}

      {:ok, reset_agent} =
        PatternRecognitionAgent.reset_learning_models(
          agent_with_history,
          keep_history: false
        )
  ...
  describe "error handling and edge cases" do
    test "handles malformed evaluation data" do
      ...
      malformed_data = [
        ...
        "not a map",
        nil,
        # Invalid evaluation_id
        %{evaluation_id: nil}
      ]

      # Should handle gracefully - may filter out invalid data or fail appropriately
      result =
        PatternRecognitionAgent.recognize_patterns(agent, :success_patterns, malformed_data)
  ...
  describe "learning model evolution" do
    test "tracks model evolution over time" do
      ...
      final_agent =
        Enum.reduce(datasets, agent, fn dataset, acc_agent ->
          {:ok, _patterns, updated_agent} =
            PatternRecognitionAgent.recognize_patterns(
              acc_agent,
              :user_preferences,
