defmodule RubberDuck.Agents.LearningCoordinatorAgentTest do
  ...
  describe "orchestrate_learning/3" do
    ...
    test "orchestrates learning workflow successfully", %{agent: agent} do
      ...
      {:ok, result, updated_agent} = LearningCoordinatorAgent.orchestrate_learning(
        agent,
        learning_request
      )
    ...
    test "validates learning request requirements", %{agent: agent} do
      invalid_request = %{
        ...
        target_engines: [:unknown_engine],  # Invalid engine
        learning_priority: :invalid_priority  # Invalid priority
      }
      
      {:error, reason, returned_agent} = LearningCoordinatorAgent.orchestrate_learning(
        agent,
        invalid_request
      )
    ...
    test "handles missing required fields in learning request", %{agent: agent} do
      ...
      {:error, reason, _agent} = LearningCoordinatorAgent.orchestrate_learning(
        agent,
        incomplete_request
      )
  ...
  describe "validate_learning_effectiveness/4" do
    setup do
      ...
      {:ok, _result, agent_with_session} = LearningCoordinatorAgent.orchestrate_learning(
        agent,
        learning_request
      )
    ...
    test "validates effective learning and approves", %{agent: agent} do
      ...
      result = LearningCoordinatorAgent.validate_learning_effectiveness(
        agent,
        session_id,
        validation_data
      )
    ...
    test "detects ineffective learning and triggers rollback", %{agent: agent} do
      ...
      poor_validation_data = [
        %{metric: :accuracy, before: 0.8, after: 0.7},  # Decreased accuracy
        %{metric: :user_satisfaction, before: 0.75, after: 0.6}  # Decreased satisfaction
      ]
  ...
  describe "learning request validation" do
    ...
    test "rejects invalid target engines" do
      ...
      {:error, reason, _agent} = LearningCoordinatorAgent.orchestrate_learning(agent, invalid_request)
  ...
  describe "learning effectiveness tracking" do
    test "tracks learning session effectiveness over time" do
      ...
      final_agent = Enum.reduce(requests, agent, fn request, acc_agent ->
        {:ok, _result, updated_agent} = LearningCoordinatorAgent.orchestrate_learning(acc_agent, request)
        updated_agent
      end)
