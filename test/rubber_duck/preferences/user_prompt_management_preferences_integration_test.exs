defmodule RubberDuck.Preferences.UserPromptManagementPreferencesIntegrationTest do
  ...
  describe "Phase 6.3: User Prompt Management Preferences - Integration Testing" do
    test "complete user prompt management preference integration with Phase 1A" do
      ...
      # Display preferences
      {:ok, _display_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _sort_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _items_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _org_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _auto_cat_pref} = UserPreference.set_preference(
        user_id,
      ...
      # Search preferences
      {:ok, _search_scope_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _fuzzy_pref} = UserPreference.set_preference(
        user_id,
      ...
      {:ok, _recent_limit_pref} = UserPreference.set_preference(
        user_id,
      ...
      assert {:ok, display_preferences} = PromptPreferenceResolver.resolve_display_preferences(user_id, project_id)
      ...
      assert {:ok, organization_preferences} = PromptPreferenceResolver.resolve_organization_preferences(user_id, project_id)

      assert organization_preferences.categorization_scheme == "tag_based"
      assert organization_preferences.auto_categorization == "true"

      assert {:ok, search_preferences} = PromptPreferenceResolver.resolve_search_preferences(user_id, project_id)

      assert search_preferences.default_scope == "favorites"
      assert search_preferences.fuzzy_search == "true"

      assert {:ok, workflow_preferences} = PromptPreferenceResolver.resolve_workflow_preferences(user_id, project_id)

      assert workflow_preferences.recent_limit == "15"

      # Step 4: Test comprehensive preference resolution
      assert {:ok, all_preferences} = PromptPreferenceResolver.resolve_all_prompt_management_preferences(user_id, project_id)
      ...
      assert {:ok, customized_interface} = PromptInterfaceCustomizer.customize_prompt_browser(
        user_id,
        base_interface_config,
        project_id
      )
      ...
      assert {:ok, _cached_display} = PromptPreferenceResolver.resolve_display_preferences(user_id, project_id)
      ...
      assert {:ok, _cached_display_2} = PromptPreferenceResolver.resolve_display_preferences(user_id, project_id)
      ...
      assert cache_hit_time_ms < cache_miss_time_ms / 2, "Cache hit (#{cache_hit_time_ms}ms) should be much faster than cache miss (#{cache_miss_time_ms}ms)"

      # Step 7: Test cache invalidation on preference changes
      # Update a preference
      {:ok, _updated_pref} = UserPreference.set_preference(
        user_id,
        "prompt_management.display.view_mode",
        "cards",
      ...
      assert {:ok, updated_display} = PromptPreferenceResolver.resolve_display_preferences(user_id, project_id)

      assert updated_display.view_mode == "cards"  # Should reflect the update

      # Step 8: Test interface customization metadata
      assert {:ok, customization_metadata} = PromptInterfaceCustomizer.get_customization_metadata(user_id, project_id)
    ...
    test "intelligent defaults for users without configured preferences" do
      ...
      assert {:ok, default_display} = PromptPreferenceResolver.resolve_display_preferences(new_user_id)
      ...
      assert is_integer(default_display.items_per_page) or is_binary(default_display.items_per_page)

      # Test organization defaults
      assert {:ok, default_organization} = PromptPreferenceResolver.resolve_organization_preferences(new_user_id)

      assert default_organization.categorization_scheme in ["hierarchical", "flat", "tag_based"]
      assert is_boolean(default_organization.auto_categorization) or default_organization.auto_categorization in ["true", "false"]

      # Test search defaults
      assert {:ok, default_search} = PromptPreferenceResolver.resolve_search_preferences(new_user_id)

      assert default_search.default_scope in ["all", "user_only", "favorites"]
      assert is_boolean(default_search.fuzzy_search) or default_search.fuzzy_search in ["true", "false"]

      # Test workflow defaults
      assert {:ok, default_workflow} = PromptPreferenceResolver.resolve_workflow_preferences(new_user_id)
      ...
      assert {:ok, default_interface} = PromptInterfaceCustomizer.customize_prompt_browser(new_user_id, base_config)
    ...
    test "preference performance requirements validation" do
      ...
      assert {:ok, _all_prefs} = PromptPreferenceResolver.resolve_all_prompt_management_preferences(perf_user_id)
      ...
      assert resolution_time_ms < 100, "Preference resolution #{resolution_time_ms}ms exceeds 100ms requirement"
      ...
      assert {:ok, _customized_interface} = PromptInterfaceCustomizer.customize_prompt_browser(perf_user_id, base_config)

      interface_customization_time = System.monotonic_time(:microsecond) - interface_customization_start
      interface_customization_time_ms = interface_customization_time / 1000

      # Should meet <100ms requirement for interface adaptation
      assert interface_customization_time_ms < 100, "Interface customization #{interface_customization_time_ms}ms exceeds 100ms requirement"
    end
    ...
    test "preference hierarchy integration with Phase 1A system" do
      ...
      {:ok, _user_display_pref} = UserPreference.set_preference(
        hierarchy_user_id,
        "prompt_management.display.view_mode",
        "grid",
      ...
      assert {:ok, user_level_prefs} = PromptPreferenceResolver.resolve_display_preferences(hierarchy_user_id)
      ...
      assert {:ok, other_user_prefs} = PromptPreferenceResolver.resolve_display_preferences(other_user_id)

      # Should not inherit first user's grid preference (should get defaults)
      assert other_user_prefs.view_mode != "grid" or Map.get(other_user_prefs, :source) == :default

      # Step 3: Test preference resolution with project context
      # Should still get user preference even in project context
      assert {:ok, user_project_prefs} = PromptPreferenceResolver.resolve_display_preferences(hierarchy_user_id, hierarchy_project_id)
      ...
      assert {:ok, updated_prefs} = PromptPreferenceResolver.resolve_display_preferences(hierarchy_user_id)
    ...
    test "interface customization with different preference combinations" do
      ...
      power_user_preferences = [
        ...
        {"prompt_management.search.quick_filters", "[\"recent\", \"favorites\", \"most_used\", \"by_category\"]"},
        {"prompt_management.organization.categorization_scheme", "tag_based"}
      ]

      for {key, value} <- power_user_preferences do
        {:ok, _pref} = UserPreference.set_preference(power_user_id, key, value, "Power user config")
      end

      assert {:ok, power_user_interface} = PromptInterfaceCustomizer.customize_prompt_browser(power_user_id, %{})
      ...
      for {key, value} <- casual_user_preferences do
        {:ok, _pref} = UserPreference.set_preference(casual_user_id, key, value, "Casual user config")
      end

      assert {:ok, casual_user_interface} = PromptInterfaceCustomizer.customize_prompt_browser(casual_user_id, %{})
      ...
      {:ok, _mixed_pref} = UserPreference.set_preference(
        mixed_user_id,
        "prompt_management.display.view_mode",
        "list",
      ...
      assert {:ok, mixed_interface} = PromptInterfaceCustomizer.customize_prompt_browser(mixed_user_id, %{})
    ...
    test "real-time preference updates and interface adaptation" do
      ...
      {:ok, _initial_pref} = UserPreference.set_preference(
        realtime_user_id,
        "prompt_management.display.view_mode",
        "list",
        "Initial preference"
      ...
      assert {:ok, initial_interface} = PromptInterfaceCustomizer.customize_prompt_browser(realtime_user_id, %{})
      ...
      assert {:ok, updated_interface} = PromptInterfaceCustomizer.customize_prompt_browser(realtime_user_id, %{})
    ...
    test "preference integration with Sections 6.1 and 6.2 infrastructure" do
      ...
      {:ok, _search_pref} = UserPreference.set_preference(
        integration_user_id,
      ...
      assert {:ok, preferences} = PromptPreferenceResolver.resolve_all_prompt_management_preferences(integration_user_id, integration_project_id)

      # Test interface customization
      base_config = %{components: [:search, :display]}

      assert {:ok, customized_config} = PromptInterfaceCustomizer.customize_prompt_browser(
        integration_user_id,
        base_config,
    ...
    test "backward compatibility with existing prompt management workflows" do
      ...
      assert {:ok, default_interface} = PromptInterfaceCustomizer.customize_prompt_browser(compatibility_user_id, base_config)
      ...
      assert {:ok, default_all_prefs} = PromptPreferenceResolver.resolve_all_prompt_management_preferences(compatibility_user_id)
  ...
  describe "Integration Quality Requirements" do
    ...
    test "Phase 1A integration validation" do
      ...
      {:ok, _test_pref} = UserPreference.set_preference(
        phase1a_user_id,
        "prompt_management.display.view_mode",
        "grid",
      ...
      assert {:ok, resolved_prefs} = PromptPreferenceResolver.resolve_display_preferences(phase1a_user_id)
      ...
      prompt_management_prefs = Enum.filter(user_preferences, fn pref ->
    ...
    test "performance scalability with multiple concurrent users" do
      ...
      concurrent_users = for i <- 1..20 do
        user_id = Ash.UUID.generate()
        
        # Set preferences for each user
        {:ok, _pref} = UserPreference.set_preference(
          user_id,
          "prompt_management.display.view_mode",
      ...
      concurrent_tasks = Enum.map(concurrent_users, fn user_id ->
        Task.async(fn ->
      ...
      assert concurrent_time_ms < 500, "Concurrent preference resolution #{concurrent_time_ms}ms too slow"

      # Average per-user time should meet requirements
      avg_time_per_user = concurrent_time_ms / 20
      assert avg_time_per_user < 25, "Average per-user resolution #{avg_time_per_user}ms exceeds 25ms target"
    end
  end
end