  ...
  describe "evaluation lifecycle" do
    test "can start, complete, and track evaluation" do
      ...
      with_mock EvaluationRun, [:passthrough],
        ...
        assert {:ok, completed_run} = EvaluationRun.complete_evaluation(started_run, completion_params)
    ...
    test "can fail evaluation with error message" do
      ...
      with_mock EvaluationRun, [:passthrough],
        fail_evaluation: fn _run, params -> {:ok, %{mock_run | status: :failed, error_message: params.error_message}} end do
        
        assert {:ok, failed_run} = EvaluationRun.fail_evaluation(mock_run, %{error_message: "API timeout"})
