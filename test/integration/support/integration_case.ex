defmodule RubberDuck.IntegrationCase do
  ...
  def create_evaluation_requests(count, options \\ %{}) do
    ...
    |> Enum.map(fn index ->
      %{
        code: load_test_code_sample(Enum.random(["simple_elixir_function.ex", "complex_elixir_module.ex", "security_test_code.ex"])),
  ...
  def assert_cost_tracked_correctly(evaluation_result, user, project) do
    ...
    max_expected_cost = 5.0  # Reasonable upper bound for single evaluation
  ...
  def assert_no_resource_leaks(baseline_metrics, final_metrics) do
    ...
    max_ets_growth = baseline_ets + 10  # Allow some ETS table growth
  ...
  def assert_providers_balanced_across_evaluations(evaluation_results) do
    providers_used = evaluation_results
    ...
    max_acceptable_percentage = 0.8  # Allow some imbalance due to preferences
  ...
  def assert_configuration_resolution_performant(evaluation_results) do
    ...
    resolution_times = evaluation_results
  ...
  def assert_evaluation_workflow_successful(evaluation_result) do
    ...
    assert is_number(evaluation_result.score) and evaluation_result.score >= 0 and evaluation_result.score <= 1,
           "Invalid evaluation score: #{evaluation_result.score}"
    
    assert is_number(evaluation_result.confidence) and evaluation_result.confidence >= 0 and evaluation_result.confidence <= 1,
  ...
  def assert_performance_within_bounds(evaluation_results, max_time_ms \\ 15_000) do
    execution_times = evaluation_results
