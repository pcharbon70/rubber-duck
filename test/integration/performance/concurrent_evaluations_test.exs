defmodule RubberDuck.Integration.Performance.ConcurrentEvaluationsTest do
  ...
  describe "concurrent evaluation performance" do
    test "handles 50+ concurrent evaluations without degradation", %{integration_context: context} do
      ...
      users =
        ...
        |> Enum.map(fn index ->
          create_integration_test_user(%{
            preferred_providers:
              Enum.random([["openai"], ["anthropic"], ["openai", "anthropic"]]),
            # Varied preferences
            cost_quality_balance: :rand.uniform() * 0.4 + 0.3,
            # Every 3rd user
            constitutional_ai_enabled: rem(index, 3) == 0
          })
        end)
      ...
      projects =
        ...
        |> Enum.map(fn user ->
          create_integration_test_project(user, %{
            # 0.7-1.0
            quality_threshold: :rand.uniform() * 0.3 + 0.7,
      ...
      Logger.info(
        "Starting concurrent evaluation performance test with #{length(evaluation_requests)} requests"
      )
      ...
      p95_eval_time =
        evaluation_times |> Enum.sort() |> Enum.at(trunc(length(evaluation_times) * 0.95))
    ...
    test "validates provider routing performance under load", %{integration_context: context} do
      ...
      successful_routings =
        Enum.filter(routing_results, fn
          {:ok, _} -> true
          _ -> false
        end)
    ...
    test "validates skills registry and orchestration performance under load", %{
      ...
      successful_recommendations =
        Enum.filter(recommendation_results, fn
          {:ok, _} -> true
          _ -> false
        end)

      recommendation_success_rate =
        length(successful_recommendations) / length(recommendation_results)
        ...
        |> Enum.map(fn index ->
          %{
            skills_actions: [
              %{
                type: :skill,
                module: RubberDuck.Skills.CodeAnalysisSkill,
                params: %{code: "test code #{index}"}
              },
              %{
                type: :skill,
                module: RubberDuck.Skills.LearningSkill,
                params: %{experience: "test experience #{index}"}
              },
              %{
                type: :llm_assisted,
                description: "Analyze and learn from code #{index}",
                params: %{}
              }
            ],
            execution_context: %{
      ...
      successful_workflows =
        Enum.filter(workflow_results, fn
    ...
    test "validates system scalability with mixed workload", %{integration_context: context} do
            ...
            Task.async(fn ->
              code =
                load_test_code_sample(
                  Enum.random(["simple_elixir_function.ex", "complex_elixir_module.ex"])
                )
      ...
      execution_time =
        System.monotonic_time(:millisecond) - baseline_metrics.timestamp.microsecond / 1000

      final_metrics = capture_system_metrics()

      # Validate mixed workload performance
      successful_tasks =
        Enum.count(mixed_results, fn
          {:ok, _} -> true
          _ -> false
        end)
  ...
  describe "configuration system performance under load" do
    test "validates configuration resolution performance with concurrent access", %{
      integration_context: context
    } do
      ...
      Logger.info(
        "Testing concurrent configuration resolution with #{length(config_resolution_tasks)} requests"
      )
      ...
      p95_resolution_time =
        resolution_times |> Enum.sort() |> Enum.at(trunc(length(resolution_times) * 0.95))
