  ...
  describe "Complete authentication workflow" do
    test "full user registration and sign-in flow" do
      ...
      assert error.class == :forbidden || 
             (error.errors && Enum.any?(error.errors, fn e -> 
    ...
    test "username case-insensitivity in authentication" do
      ...
      assert {:ok, signed_in} = Accounts.sign_in_user(%{
        username: "CASETEST",
        password: "Password123!"
      }, authorize?: false)
      ...
      assert {:ok, signed_in2} = Accounts.sign_in_user(%{
        username: "CaseTest",
        password: "Password123!"
      }, authorize?: false)
    ...
    test "password validation requirements" do
      ...
      assert error.errors |> Enum.any?(fn e -> 
        (Map.get(e, :field) == :password_confirmation || Map.get(e, :fields) == [:password_confirmation]) &&
    ...
    test "token lifecycle and policies" do
      ...
      {:ok, signed_in} = Accounts.sign_in_user(%{
        username: "token_test",
        password: "Password123!"
      }, authorize?: false)
    ...
    test "authentication with invalid inputs" do
      # Empty username
      assert {:error, error} = Accounts.sign_in_user(%{
        username: "",
        password: "Password123!"
      }, authorize?: false)
      assert error.errors |> Enum.any?(fn e -> 
    ...
    test "password change requires current password" do
      ...
      assert {:error, error} = Accounts.change_user_password(user, %{
      ...
      assert {:ok, _} = Accounts.change_user_password(user, %{
