defmodule RubberDuck.Integration.Workflows.CompleteEvaluationWorkflowTest do
  ...
  describe "complete evaluation workflow integration" do
    test "evaluates code through full pipeline with all Phase 1B components", %{integration_context: context} do
      ...
      user = create_integration_test_user(%{
        ...
        cost_quality_balance: 0.6  # Slight quality preference
      })
      ...
      {:ok, evaluation_result} = RubberDuck.Verdict.Engine.evaluate_code(
        code_sample,
        :security,  # Security evaluation should trigger Constitutional AI
        user_id: user.id,
      ...
      event_domains = telemetry_events
    ...
    test "handles progressive evaluation workflow with provider routing", %{integration_context: context} do
      # Test progressive evaluation: screening → detailed analysis
      user = create_integration_test_user(%{
        cost_quality_balance: 0.8,  # Quality focused
      ...
      if Map.has_key?(metadata, :evaluation_stages) do
        ...
        assert String.contains?(first_stage.model_used, "mini") or String.contains?(first_stage.model_used, "haiku"),
               "First stage should use screening model"
      end
    end
    
    test "evaluates with skills integration and action orchestration", %{integration_context: context} do
  ...
  describe "configuration-driven evaluation workflows" do
    test "adapts workflow based on three-tier configuration preferences", %{integration_context: context} do
      ...
      config_resolution_times = results
      |> Map.values()
      |> Enum.map(fn result -> 
    ...
    test "handles real-time configuration changes during evaluation", %{integration_context: context} do
      # Test configuration changes affecting active evaluations
      user = create_integration_test_user()
      project = create_integration_test_project(user)
      
      # Start long-running evaluation
      long_evaluation_task = Task.async(fn ->
      ...
      Process.sleep(100)  # Let evaluation start
      
      configuration_change = %{
        preferred_providers: ["anthropic"],  # Change provider preference
        quality_threshold: 0.95  # Increase quality requirement
      }
  ...
  describe "provider integration workflow validation" do
    test "validates Universal Provider System integration across evaluation workflow", %{integration_context: context} do
    ...
    test "handles provider failover during evaluation workflow", %{integration_context: context} do
      ...
      user = create_integration_test_user(%{
        preferred_providers: ["openai", "anthropic"],  # Fallback available
      ...
      simulate_provider_failure(:openai, 2000)  # Fail for 2 seconds
  ...
  describe "learning and feedback integration" do
    test "integrates with continuous learning system throughout workflow", %{integration_context: context} do
      ...
      evaluation_requests = create_evaluation_requests(5, %{
        user_id: user.id,
        project_id: project.id
      })
      
      results = Enum.map(evaluation_requests, fn request ->
        {:ok, result} = RubberDuck.Verdict.Engine.evaluate_code(
          request.code,
          ...
          metadata: Map.merge(request.metadata, %{
            integration_test: true,
            test_id: context.test_id,
            learning_test: true
          })
      ...
      learning_events = get_integration_telemetry()
      ...
      if length(results) >= 3 do
        ...
        if last_time < first_time * 0.8 do
          Logger.info("Learning system appears to have improved performance: #{first_time}ms → #{last_time}ms")
        end
      end
    end
  end
  ...
  describe "skills and actions integration workflow" do
    test "integrates skills registry and action orchestration with evaluation", %{integration_context: context} do
