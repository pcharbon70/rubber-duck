defmodule RubberDuck.Verdict.Feedback.FeedbackCollector do
  ...
  defp validate_feedback_sources(feedback_sources) do
    validation_results = Enum.map(feedback_sources, fn {source_type, data} ->
      case validate_source_type(source_type) do
        :ok ->
          case FeedbackValidator.validate_feedback_data(data, source_type) do
            {:ok, validated_data} -> {:ok, {source_type, validated_data}}
            {:error, reason} -> {:error, "#{source_type}: #{reason}"}
          end
        {:error, reason} ->
          {:error, "Invalid source type #{source_type}: #{reason}"}
      end
    end)
  ...
  defp process_feedback_sources(evaluation_id, validated_sources, options) do
    processing_results = Enum.map(validated_sources, fn {source_type, data} ->
      case source_type do
        ...
        type when type in [:implicit_acceptance, :implicit_rejection, :implicit_retry, :implicit_edit] ->
  ...
  defp aggregate_feedback(processed_feedback, options) do
    ...
    high_confidence_feedback = Enum.filter(processed_feedback, fn feedback ->
  ...
  defp calculate_data_quality_adjustment(data) when is_map(data) do
    completeness = map_size(data) / 10.0  # Rough completeness metric
  ...
  defp extract_actionable_insights(feedback_list) do
    ...
    high_learning_value = Enum.filter(feedback_list, fn feedback ->
      feedback.learning_value > 0.8
    end)
    
    insights = Enum.map(high_learning_value, fn feedback ->
      %{
  ...
  defp generate_processing_summary(processed_feedback) do
    ...
    %{
      ...
      processing_distribution: Enum.map(by_type, fn {type, list} -> {type, length(list)} end) |> Map.new(),
  ...
  defp assess_feedback_quality(feedback_data) do
    ...
    data_completeness = if Map.has_key?(feedback_data, :data) and not is_nil(feedback_data.data) do
      0.1
    else
      -0.1
    end
    
    user_context = if Map.has_key?(feedback_data, :user_id) do
      0.05
    else
      0.0
    end
  ...
  defp assess_learning_value(feedback_data) do
    base_value = case determine_feedback_type(feedback_data) do
      :explicit_correction -> 0.95
      :explicit_rating -> 0.8
  ...
  defp calculate_feedback_richness(feedback_data) do
    ...
    present_details * 0.05  # Up to 0.2 bonus for complete detailed feedback
  end
  ...
  defp extract_trend_insights(trends) do
    ...
    trend_insights = if trends.confidence_trend == :improving do
      [%{type: :trend_insight, insight: :confidence_improving, action: :maintain_current_approach} | trend_insights]
    else
      trend_insights
    end
    
    trend_insights = if trends.volume_trend == :increasing do
      [%{type: :trend_insight, insight: :volume_increasing, action: :scale_processing_capacity} | trend_insights]
    else
      trend_insights
    end
  ...
  defp get_feedback_distribution_by_type(_since_time), do: %{explicit_rating: 20, implicit_acceptance: 15, judge_agreement: 7}
  defp get_feedback_distribution_by_source(_since_time), do: %{user_interface: 25, api: 12, system_automated: 5}
  ...
  defp get_processing_performance_stats(_since_time), do: %{avg_processing_time_ms: 45, success_rate: 0.97}
