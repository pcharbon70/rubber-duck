defmodule RubberDuck.Verdict.Feedback.ExplicitFeedbackProcessor do
    ...
    case validate_correction_data(correction_details, user_explanation) do
      :ok ->
        processed_feedback = %{
          ...
          confidence: 0.9,
          learning_value:
            calculate_correction_learning_value(correction_details, user_explanation),
          insights:
            extract_correction_insights(correction_details, user_explanation, suggested_score),
  ...
  defp validate_correction_data(correction_details, _explanation) when is_map(correction_details),
    do: :ok

  defp validate_correction_data(nil, explanation)
       when is_binary(explanation) and byte_size(explanation) > 10,
       do: :ok

  defp validate_correction_data(_correction, _explanation),
    do: {:error, "Invalid correction data"}
  ...
  defp calculate_correction_learning_value(correction_details, user_explanation) do
    # Corrections are always high value
    base_value = 0.9

    # Detailed explanations increase value
    explanation_bonus =
      if is_binary(user_explanation) and String.length(user_explanation) > 50 do
        0.05
      else
        0.0
      end
    ...
    structure_bonus =
      if is_map(correction_details) and map_size(correction_details) > 2 do
        0.05
      else
        0.0
      end
  ...
  defp generate_rating_actions(rating, context) do
    case rating do
      r when r <= 2 ->
        ["investigate_low_satisfaction", "improve_evaluation_quality", "check_judge_selection"]

      r when r >= 4 ->
        ["reinforce_current_approach", "identify_success_patterns"]

      _ ->
        ["general_quality_monitoring"]
    end
  end
  ...
  defp generate_correction_actions(correction_details, user_explanation) do
    ...
    explanation_actions =
      if is_binary(user_explanation) and String.length(user_explanation) > 20 do
  ...
  defp calculate_score_adjustment(suggested_score) when is_number(suggested_score),
    do: suggested_score - 0.5
  ...
  defp analyze_comment_text(comment_text) do
    %{
      ...
      contains_suggestions:
        String.contains?(comment_text || "", ["suggest", "recommend", "should"]),
      contains_criticism:
        String.contains?(comment_text || "", ["wrong", "bad", "poor", "terrible"])
    }
  end
