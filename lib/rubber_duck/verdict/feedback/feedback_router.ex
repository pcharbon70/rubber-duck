defmodule RubberDuck.Verdict.Feedback.FeedbackRouter do
  ...
  def route_feedback_batch(target_engine, feedback_batch, priority, options \\ []) do
    Logger.info("Routing #{length(feedback_batch)} items to #{target_engine} with priority #{priority}")
    ...
    case validate_routing_request(target_engine, feedback_batch, priority) do
      :ok ->
        case execute_routing_strategy(target_engine, feedback_batch, priority, routing_strategy, options) do
          {:ok, routing_result} ->
  ...
  defp determine_optimal_targets(feedback_item, options) do
    ...
    potential_targets = Enum.filter(@learning_engine_targets, fn {_engine, categories} ->
  ...
  defp route_to_multiple_targets(feedback_item, target_engines, options) do
    routing_tasks = Enum.map(target_engines, fn engine ->
      Task.async(fn ->
  ...
  defp route_round_robin(target_engine, feedback_batch, priority, options) do
    ...
    routing_results = Enum.map(batches, fn batch ->
  ...
  defp route_category_specialized(target_engine, feedback_batch, priority, options) do
    ...
    routing_results = Enum.map(feedback_by_category, fn {category, category_feedback} ->
  ...
  defp route_to_single_target(feedback_item, engine, options) do
    ...
    routing_latency = :rand.uniform(100) + 50  # 50-150ms
  ...
  defp route_batch_to_engine(engine, feedback_batch, priority, options) do
    ...
    estimated_time = batch_size * 75  # Rough estimate
  ...
  defp select_optimal_targets(potential_targets, priority, options) do
    ...
    scored_targets = Enum.map(potential_targets, fn {engine, _categories} ->
  ...
  defp calculate_target_score(engine, priority) do
    ...
    priority_multiplier = case priority do
      :critical -> 1.5
      :high -> 1.2
      :medium -> 1.0
      :low -> 0.8
  ...
  defp group_feedback_by_learning_category(feedback_batch) do
    Enum.group_by(feedback_batch, fn feedback_item ->
      ...
      List.first(categories)  # Use primary category for grouping
    end)
  end

  defp find_specialized_engine_for_category(category, fallback_engine) do
    specialized_engine = Enum.find(@learning_engine_targets, fn {_engine, categories} ->
  ...
  defp make_adaptive_routing_decision(target_engine, feedback_batch, priority) do
    ...
    cond do
      ...
      batch_size > 20 ->
        ...
        %{strategy: :split_route, split_plan: %{primary: split_point, secondary: batch_size - split_point}}
  ...
  defp consolidate_batch_routing_results(routing_results, strategy) do
    ...
    total_routed = Enum.reduce(successful_results, 0, fn {:ok, result}, acc ->
  ...
  defp get_engine_current_load(_engine), do: :rand.uniform(20)  # Simulate current load
  ...
  defp get_routing_distribution_by_engine(_time_window), do: %{judge_selection_learner: 45, criteria_adaptation_engine: 32}
  ...
  defp get_engine_load_status(), do: %{judge_selection_learner: 0.6, criteria_adaptation_engine: 0.4}
  defp calculate_routing_success_rate(_time_window), do: 0.94
  defp get_failed_routing_reasons(_time_window), do: %{capacity_exceeded: 3, engine_unavailable: 1}
  ...
  defp calculate_queue_delay(_engine), do: 30_000  # 30 seconds default delay
