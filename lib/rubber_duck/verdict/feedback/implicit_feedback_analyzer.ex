defmodule RubberDuck.Verdict.Feedback.ImplicitFeedbackAnalyzer do
  ...
  defp analyze_acceptance_behavior(evaluation_id, behavior_data, options) do
    ...
    learning_value = calculate_acceptance_learning_value(acceptance_speed, interaction_count, context)
    ...
    analyzed_behavior = %{
      ...
      insights: %{
        ...
        user_satisfaction_indicator: assess_satisfaction_from_acceptance(acceptance_speed, interaction_count),
  ...
  defp analyze_rejection_behavior(evaluation_id, behavior_data, options) do
    ...
    learning_value = calculate_rejection_learning_value(rejection_reason, alternative_action, rejection_speed)
    ...
    analyzed_behavior = %{
      ...
      insights: %{
        ...
        judge_effectiveness_issue: assess_judge_effectiveness_issue(rejection_reason, alternative_action)
      },
  ...
  defp analyze_edit_behavior(evaluation_id, behavior_data, options) do
    ...
    analyzed_behavior = %{
      ...
      insights: %{
        evaluation_completeness: assess_evaluation_completeness_from_edits(edit_count, edit_types),
        user_expertise_level: infer_user_expertise_from_edits(edit_types),
        system_guidance_effectiveness: assess_guidance_effectiveness(time_to_first_edit, edit_count)
      },
  ...
  defp calculate_rejection_learning_value(rejection_reason, alternative_action, rejection_speed) do
    base_value = 0.8  # Rejections are high learning value
    
    # Specific rejection reasons are more valuable
    reason_value = if is_atom(rejection_reason) and rejection_reason != :unknown, do: 0.1, else: 0.0
  ...
  defp infer_user_confidence(acceptance_time, interaction_count) do
    case {classify_acceptance_speed(acceptance_time), classify_interaction_pattern(interaction_count)} do
  ...
  defp assess_dissatisfaction_level(rejection_reason, rejection_speed) do
    reason_severity = case rejection_reason do
      :quality_issues -> :high
    ...
    speed_factor = case classify_rejection_speed(rejection_speed) do
      :immediate -> :high
      :quick -> :medium
      _ -> :low
    end
  ...
  defp identify_improvement_areas_from_rejection(:quality_issues), do: [:evaluation_accuracy, :judge_selection]
  defp identify_improvement_areas_from_rejection(:speed_issues), do: [:performance_optimization, :caching]
  ...
  defp assess_judge_effectiveness_issue(rejection_reason, alternative_action) do
    if rejection_reason in [:quality_issues, :accuracy_concerns] and not is_nil(alternative_action) do
      :significant
    else
      :minor
    end
  ...
  defp generate_rejection_learning_actions(rejection_reason, alternative_action) do
    ...
    reason_actions = case rejection_reason do
  ...
  defp generate_retry_learning_actions(retry_count, modifications) do
    base_actions = case classify_retry_frequency(retry_count) do
      freq when freq in [:frequent, :excessive] -> ["investigate_system_reliability", "improve_first_time_success"]
      _ -> ["monitor_retry_patterns"]
    end
    
    modification_actions = case assess_modification_intensity(modifications) do
      intensity when intensity in [:moderate, :extensive] -> ["analyze_modification_patterns", "improve_initial_evaluation"]
      _ -> []
    end
  ...
  defp classify_edit_complexity(edit_types) do
    complexity_score = Enum.reduce(edit_types, 0, fn edit_type, acc ->
      case edit_type do
  ...
  defp generate_edit_learning_actions(edit_count, edit_types) do
    frequency_actions = case classify_edit_frequency(edit_count) do
      freq when freq in [:frequent, :extensive] -> ["improve_initial_evaluation", "enhance_guidance"]
      _ -> []
    end
    
    complexity_actions = case classify_edit_complexity(edit_types) do
      complexity when complexity in [:complex, :very_complex] -> ["analyze_edit_patterns", "improve_evaluation_completeness"]
      _ -> []
    end
  ...
  defp identify_peak_usage_times(_session_data), do: [9, 14, 16]  # Hours
  defp analyze_feature_usage_patterns(_session_data), do: %{most_used: :evaluation, least_used: :advanced_settings}
  ...
  defp identify_optimization_opportunities(_patterns), do: ["workflow_streamlining", "feature_enhancement"]
  defp assess_personalization_potential(_profile), do: :high
  defp generate_adaptation_recommendations(_patterns, _profile), do: ["personalize_evaluation_flow", "optimize_common_workflows"]
  ...
  defp infer_evaluation_difficulty(retry_count, modifications) do
    ...
    case {retry_factor, modification_factor} do
      {freq, intensity} when freq in [:frequent, :excessive] and intensity in [:moderate, :extensive] -> :very_high
      {freq, _} when freq in [:frequent, :excessive] -> :high
      {_, intensity} when intensity in [:moderate, :extensive] -> :moderate
      _ -> :low
    end
  end
  ...
  defp infer_user_expertise_from_edits(edit_types) do
    complex_edits = Enum.count(edit_types, fn type -> type in [:criteria_adjustment, :complete_rewrite] end)
