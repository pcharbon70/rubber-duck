defmodule RubberDuck.Verdict.Learning.JudgeSelectionLearner do
  ...
  @selection_strategies [:random, :round_robin, :specialization_based, :performance_weighted, :user_preference_based]
  @optimization_objectives [:accuracy_optimization, :speed_optimization, :cost_optimization, :user_satisfaction_optimization]
  ...
  def update_specialization_weights(judge_performance_feedback, options \\ []) do
    Logger.info("Updating judge specialization weights from #{length(judge_performance_feedback)} feedback items")
  ...
  defp analyze_judge_selection_patterns(patterns) when is_list(patterns) do
    ...
    if Enum.empty?(selection_patterns) do
      ...
      analysis_result = %{
        ...
        judge_performance_correlations: analyze_judge_performance_correlations(selection_patterns),
  ...
  defp calculate_learning_confidence(optimizations) when is_list(optimizations) do
    if Enum.empty?(optimizations) do
      ...
      confidences = Enum.map(optimizations, fn optimization ->
  ...
  defp identify_optimization_objectives(patterns) when is_list(patterns) do
    ...
    objective_indicators = Enum.flat_map(patterns, fn pattern ->
  ...
  defp analyze_current_routing_performance(routing_data, optimization_target) do
    ...
    improvement_opportunities = identify_routing_improvement_opportunities(performance_metrics, bottlenecks)
  ...
  defp generate_routing_improvements(performance_analysis, optimization_target, options) do
    ...
    improvements = case optimization_target do
  ...
  defp analyze_judge_specialization_effectiveness(feedback) when is_list(feedback) do
    ...
    judge_feedback_groups = Enum.group_by(feedback, fn feedback_item ->
      Map.get(feedback_item, :judge_type, :unknown)
    end)
    
    specialization_analysis = Enum.map(judge_feedback_groups, fn {judge_type, judge_feedback} ->
  ...
  defp calculate_specialization_weight_updates(specialization_analysis) do
    ...
    Enum.map(specialization_analysis, fn judge_analysis ->
      ...
      effectiveness_score = Map.get(judge_analysis.effectiveness_metrics, :overall_effectiveness, 0.7)
  ...
  defp validate_weight_updates(weight_updates) when is_list(weight_updates) do
    ...
    max_individual_change = Enum.max_by(weight_updates, fn update -> abs(update.weight_change) end).weight_change
  ...
  defp extract_successful_selection_patterns(selection_patterns) do
    success_patterns = Enum.filter(selection_patterns, fn pattern ->
  ...
  defp extract_failed_selection_patterns(selection_patterns) do
    failure_patterns = Enum.filter(selection_patterns, fn pattern ->
  ...
  defp analyze_judge_performance_correlations(selection_patterns) do
    ...
    performance_data = Enum.map(selection_patterns, fn pattern ->
      pattern_data = Map.get(pattern, :pattern_data, %{})
  ...
  defp add_specialization_optimizations(analysis, optimizations) do
    ...
    specialization_opts = Enum.map(successful_selections, fn selection ->
      %{
  ...
  defp add_performance_routing_optimizations(analysis, optimizations) do
    ...
    performance_opts = Enum.map(correlations.optimal_judge_combinations, fn combination ->
      %{
  ...
  defp add_user_preference_optimizations(analysis, optimizations) do
    ...
    preference_opts = Enum.map(preference_patterns, fn pattern ->
      %{
  ...
  defp add_temporal_optimizations(analysis, optimizations) do
    ...
    temporal_opts = Enum.map(temporal_patterns, fn pattern ->
      %{
  ...
  defp extract_routing_performance_metrics(routing_data, target) when is_list(routing_data) do
    ...
    target_specific_metrics = case target do
      :accuracy_optimization ->
  ...
  defp identify_routing_improvement_opportunities(metrics, bottlenecks) do
    ...
    opportunities = if Map.get(metrics, :success_rate, 1.0) < 0.9 do
    ...
    bottleneck_opportunities = Enum.map(bottlenecks, fn bottleneck ->
      %{
  ...
  defp generate_accuracy_routing_improvements(performance, opportunities) do
    accuracy_opportunities = Enum.filter(opportunities, fn opp ->
    ...
    Enum.map(accuracy_opportunities, fn opportunity ->
      %{
        ...
        target_accuracy: Map.get(performance, :average_accuracy, 0.75) + opportunity.potential_gain,
  ...
  defp generate_speed_routing_improvements(performance, opportunities) do
    speed_opportunities = Enum.filter(opportunities, fn opp ->
    ...
    Enum.map(speed_opportunities, fn opportunity ->
      %{
        ...
        target_processing_time: Map.get(performance, :average_processing_time, 5_000) * (1 - opportunity.potential_gain),
  ...
  defp generate_cost_routing_improvements(performance, opportunities) do
    cost_opportunities = Enum.filter(opportunities, fn opp ->
  ...
  defp generate_satisfaction_routing_improvements(performance, opportunities) do
    satisfaction_opportunities = Enum.filter(opportunities, fn opp ->
    ...
    Enum.map(satisfaction_opportunities, fn opportunity ->
      %{
        ...
        target_satisfaction: Map.get(performance, :average_user_satisfaction, 0.7) + opportunity.potential_gain,
  ...
  defp calculate_judge_effectiveness_metrics(judge_feedback) when is_list(judge_feedback) do
    if Enum.empty?(judge_feedback) do
      ...
      overall_effectiveness = if Enum.empty?(valid_scores) do
        0.5
      else
  ...
  defp assess_specialization_strength(judge_feedback, judge_type) when is_list(judge_feedback) do
    ...
    specialization_feedback = Enum.filter(judge_feedback, fn feedback ->
    ...
    if Enum.empty?(specialization_feedback) do
      0.5  # Neutral strength if no specialization data
    else
  ...
  defp analyze_judge_performance_trend(judge_feedback) when is_list(judge_feedback) do
    ...
    if length(judge_feedback) < 3 do
      ...
      sorted_feedback = Enum.sort_by(judge_feedback, fn feedback ->
        Map.get(feedback, :timestamp, DateTime.utc_now())
      end)
      
      effectiveness_scores = Enum.map(sorted_feedback, &extract_effectiveness_score/1)
  ...
  defp generate_weight_change_reasoning(judge_analysis, weight_adjustment) do
    ...
    cond do
      weight_adjustment > 0.05 -> "Increasing weight due to high effectiveness (#{Float.round(effectiveness, 2)}) and #{trend} trend"
      weight_adjustment < -0.05 -> "Decreasing weight due to low effectiveness (#{Float.round(effectiveness, 2)}) and #{trend} trend"  
      true -> "Maintaining weight - performance is stable at #{Float.round(effectiveness, 2)}"
    end
  end
  ...
  defp calculate_weight_update_confidence(specialization_analysis) when is_list(specialization_analysis) do
  ...
  defp analyze_temporal_selection_effectiveness(_patterns), do: %{peak_performance_times: [10, 14, 16]}
  ...
  defp extract_accuracy_correlations(correlations), do: Map.get(correlations, :judge_accuracy_correlation, 0.0)
  defp extract_efficiency_correlations(correlations), do: Map.get(correlations, :judge_efficiency_correlation, 0.0)
  defp extract_satisfaction_correlations(correlations), do: 0.7
  defp identify_optimal_combinations(_correlations), do: [%{judges: [:code_quality, :architecture], performance_gain: 0.15}]
  ...
  defp analyze_processing_time_distribution(_routing_data), do: %{p50: 4_000, p95: 8_500, std_dev: 1_200}
  ...
  defp identify_accuracy_bottlenecks(_routing_data), do: [%{type: :judge_specialization_mismatch, impact: 0.15}]
  defp identify_cost_bottlenecks(_routing_data), do: [%{type: :overuse_expensive_judges, impact: 0.25}]
