defmodule RubberDuck.Verdict.Configuration.VerdictConfigurationResolver do
  ...
  def invalidate_cache(user_id, project_id \\ nil) do
    ...
    Logger.info(
      "Invalidated Verdict configuration cache for user #{user_id}, project #{project_id}"
    )
  ...
  defp resolve_fresh_configuration(user_id, project_id, options) do
    Logger.debug(
      "Resolving fresh Verdict configuration for user #{user_id}, project #{project_id}"
    )
  ...
  defp resolve_user_configuration(user_id, system_config) do
    case get_user_preferences(user_id) do
      {:ok, user_preferences} ->
        merged_config =
          UserVerdictPreferences.apply_user_preferences(system_config, user_preferences)

        {:ok, merged_config}

      {:error, :not_found} ->
        Logger.debug(
          "No user Verdict preferences found for user #{user_id}, using system defaults"
        )
  ...
  defp resolve_project_configuration(project_id, user_config) do
    case get_project_settings(project_id) do
      {:ok, project_settings} ->
        merged_config =
          ProjectVerdictSettings.apply_project_settings(user_config, project_settings)

        {:ok, merged_config}

      {:error, :not_found} ->
        Logger.debug(
          "No project Verdict settings found for project #{project_id}, using user/system config"
        )
  ...
  def get_configuration_source_chain(user_id, project_id \\ nil) do
    ...
    chain =
      case get_system_configuration() do
    ...
    chain =
      case get_user_preferences(user_id) do
        {:ok, _preferences} ->
          [%{level: :user, source: "user_preferences", user_id: user_id} | chain]
    ...
    chain =
      if project_id do
        case get_project_settings(project_id) do
          {:ok, _settings} ->
            [%{level: :project, source: "project_settings", project_id: project_id} | chain]
  ...
  def resolve_preference_key(key, user_id, project_id \\ nil) do
    case resolve_configuration(user_id, project_id) do
      {:ok, config} ->
