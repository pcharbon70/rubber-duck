defmodule RubberDuck.Verdict.Configuration.ConfigurationValidator do
  ...
  def validate_hierarchy(user_id, project_id \\ nil, _options \\ []) do
    ...
    with {:ok, system_config} <- validate_system_configuration(),
         ...
         :ok <- validate_cross_tier_consistency(system_config, user_config, project_config, resolved_config) do
  ...
  def validate_user_configuration(user_id, preferences \\ nil) do
    ...
    case preferences do
      ...
      {:error, :not_found} -> {:ok, :not_configured}
      error -> error
    end
  end
  ...
  defp validate_required_system_fields(config) do
    required_fields = [:enabled, :default_screening_model, :default_detailed_model, :global_daily_budget]
  ...
  defp validate_system_budget_constraints(config) do
    ...
    budget_value = case global_budget do
    ...
    cond do
      ...
      max_tokens < 100 or max_tokens > 10000 ->
  ...
  defp validate_user_provider_preferences(user_prefs) do
    ...
    cond do
      not is_list(preferred) -> {:error, "Preferred providers must be a list"}
      not is_list(avoided) -> {:error, "Avoided providers must be a list"}
  ...
  defp validate_user_criteria_weights(user_prefs) do
    case Map.get(user_prefs, :custom_criteria_weights) do
      ...
      _ -> :ok  # No custom weights is valid
    end
  end
  ...
  defp validate_project_team_settings(project_settings) do
    ...
    cond do
      team_size < 1 -> {:error, "Team size must be at least 1"}
  ...
  defp validate_project_provider_restrictions(project_settings) do
    ...
    cond do
      not is_list(allowed) -> {:error, "Allowed providers must be a list"}
      not is_list(restricted) -> {:error, "Restricted providers must be a list"}
      ...
      length(allowed) > 0 and length(Enum.filter(allowed, &(&1 not in restricted))) == 0 ->
  ...
  defp validate_project_security_requirements(project_settings) do
    ...
    cond do
      ...
      security_level in [:high, :critical] and length(compliance_standards) == 0 ->
  ...
  defp validate_cross_tier_consistency(_system_config, user_config, project_config, resolved_config) do
    validation_results = [
  ...
  defp validate_provider_conflicts(user_prefs, project_settings) do
    ...
    cond do
      length(user_preferred) > 0 and length(project_allowed) > 0 ->
        validate_preferred_allowed_overlap(user_preferred, project_allowed)
      length(user_avoided) > 0 and length(project_restricted) > 0 ->
  ...
  defp validate_combined_restrictions(user_avoided, project_restricted) do
    ...
    combined_available = MapSet.intersection(MapSet.new(user_available), MapSet.new(project_available))
  ...
  defp validate_model_available_in_providers(model, providers) do
    case get_model_provider(model) do
      ...
      {:error, _} -> {:error, "Unknown model: #{model}"}
    end
  end
  
  defp get_model_provider(model) do
    provider = Enum.find(@valid_models, fn {_provider, models} ->
      model in models
    end)
  ...
  defp validate_project_criteria_weights(_), do: {:error, "Project criteria weights must be a map"}
  ...
  defp load_system_configuration do
    RubberDuck.Verdict.VerdictSystemConfiguration.get_active_configuration()
  end
  
  defp load_user_preferences(user_id) do
    RubberDuck.Verdict.UserVerdictPreferences.by_user(user_id)
  end
  
  defp load_project_settings(project_id) do
    RubberDuck.Verdict.ProjectVerdictSettings.by_project(project_id)
  end
  ...
  defp summarize_configuration(config) do
    %{
      ...
      providers_configured: length(Map.get(config, :preferred_providers, [])) > 0,
  ...
  defp validate_system_configuration_data(config_data) do
    RubberDuck.Verdict.VerdictSystemConfiguration.validate_configuration(config_data)
  end
