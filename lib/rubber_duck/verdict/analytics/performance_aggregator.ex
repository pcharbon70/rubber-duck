defmodule RubberDuck.Verdict.Analytics.PerformanceAggregator do
  ...
  def compute_cost_efficiency(filters \\ []) do
    with {:ok, progressive_runs} <- get_progressive_evaluation_runs(filters),
      ...
      efficiency_metrics = %{
        ...
        savings_percentage:
          calculate_savings_percentage(baseline_costs.total_cost, actual_costs.total_cost),
  ...
  defp get_evaluation_runs_in_period(_start_time, _end_time, _filters) do
  ...
  defp compute_aggregated_metrics(runs, time_period, filters) do
    ...
    enhanced_metrics =
      case Keyword.get(filters, :judge_unit_type) do
        nil -> base_metrics
  ...
  defp get_progressive_evaluation_runs(_filters) do
  ...
  defp estimate_baseline_costs(runs) do
    ...
    # Assume detailed evaluation cost
    estimated_cost = length(runs) * 0.015
  ...
  defp get_evaluation_results_for_bias_analysis(_judge_unit_type, _model_name) do
  ...
  defp get_metrics_update_candidates(_time_period) do
  ...
  defp determine_cost_trend(_metrics) do
  ...
  defp detect_performance_alerts(metrics) do
    ...
    alerts =
      if calculate_success_rate(metrics) < 0.90 do
    ...
    alerts =
      if metrics.average_cost > 0.10 do
  ...
  defp calculate_calibration_score(_results) do
  ...
  defp calculate_overconfidence_rate(_results) do
    # Calculate rate of overconfident predictions
    # For now, return a placeholder
    0.15
  end

  defp detect_length_bias(_results) do
