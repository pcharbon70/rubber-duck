defmodule RubberDuck.Verdict.Providers.EvaluationContext do
  ...
  defp apply_configuration_to_context(context, config) do
    %{
      ...
      | quality_threshold: Map.get(config, :default_quality_threshold, context.quality_threshold),
        ...
        max_cost_per_evaluation:
          Map.get(config, :max_cost_per_evaluation, context.max_cost_per_evaluation),
        ...
        restricted_providers:
          Map.get(config, :restricted_providers, context.restricted_providers),
        ...
        min_confidence_threshold:
          Map.get(config, :escalation_threshold, context.min_confidence_threshold)
    }
  end
  ...
  def allows_provider?(%__MODULE__{} = context, provider_type) when is_atom(provider_type) do
    ...
    meets_preferences =
      Enum.empty?(context.preferred_providers) or
  ...
  def calculate_complexity_score(%__MODULE__{} = context) do
    ...
    type_complexity =
      case context.evaluation_type do
        # Security evaluations are complex
        :security -> 0.8
  ...
  def validate_context_for_provider(%__MODULE__{} = context, provider_type, provider_capabilities) do
    ...
    cond do
      ...
      context.streaming_required and
          not Map.get(provider_capabilities, :supports_streaming, false) ->
