defmodule RubberDuck.Verdict.Providers.ProviderRegistry do
  ...
  def handle_call(:get_providers, _from, state) do
    ...
    |> Enum.map(fn {type, provider_info} ->
      {type, Map.put(provider_info, :health_status, Map.get(state.health_status, type, :unknown))}
    end)
    |> Enum.into(%{})
      ...
      provider_info ->
        ...
        case initialize_provider(provider_info.module, new_config) do
          {:ok, new_provider_state} ->
            updated_provider_info = %{provider_info | 
  ...
  defp update_provider_health(state, provider_type, health_result) do
    {new_health_status, failure_increment} = case health_result do
      {:ok, health_data} ->
    ...
    updated_providers = case Map.get(state.providers, provider_type) do
      nil -> state.providers
      provider_info ->
    ...
    updated_failures = Map.update(state.failure_counts, provider_type, failure_increment, fn
      count when new_health_status == :healthy -> 0  # Reset on recovery
  ...
  defp meets_requirements?(provider_info, requirements) do
    ...
    evaluation_type_supported = case Map.get(requirements, :evaluation_type) do
      nil -> true
      type -> type in Map.get(capabilities, :evaluation_types, [:quality, :security, :performance, :maintainability, :style])
    end
  ...
  def load_provider_configurations do
    ...
    %{
      ...
      ollama: %{
        ...
        rate_limits: %{
          requests_per_minute: 1000,  # Local models have higher limits
    ...
    Enum.each(configs, fn {provider_type, config} ->
      if Map.get(config, :enabled, false) do
        ...
        case register_provider(provider_type, provider_module, config) do
          :ok -> Logger.info("Auto-registered provider: #{provider_type}")
  ...
  def handle_call(:shutdown_all, _from, state) do
    ...
    Enum.each(state.providers, fn {provider_type, provider_info} ->
      try do
        ...
        error ->
        Logger.error("Error shutting down provider #{provider_type}: #{Exception.message(error)}")
      end
    end)
