defmodule RubberDuck.Verdict.Providers.ProviderHealthMonitor do
  ...
  def handle_call({:get_provider_health, provider_type}, _from, state) do
    case Map.get(state.health_data, provider_type) do
      ...
      health_info ->
        ...
        detailed_health = Map.merge(health_info, %{
          performance_metrics: calculate_performance_metrics(performance_data),
  ...
  def handle_cast({:record_result, provider_type, result}, state) do
    ...
    |> Map.update(provider_type, [performance_entry], fn entries ->
      ...
      recent_entries = Enum.filter(entries, &(DateTime.compare(&1.timestamp, cutoff_time) != :lt))
      [performance_entry | recent_entries] |> Enum.take(1000)  # Limit memory usage
    end)
    
    # Update health data with latest performance
    updated_health_data = update_health_from_performance(state.health_data, provider_type, performance_entry)
    ...
    if state.monitoring_enabled do
      ...
      case ProviderRegistry.get_providers() do
        {:ok, providers} ->
          ...
          updated_stats = Map.update!(updated_state.stats, :health_checks_performed, &(&1 + map_size(providers)))
  ...
  defp update_provider_health_data(state, provider_type, health_check_result) do
    ...
    health_info = case health_check_result do
      {:ok, health_data} ->
        %{
  ...
  defp calculate_performance_metrics(performance_data) when is_list(performance_data) do
    if Enum.empty?(performance_data) do
      ...
      %{
        ...
        avg_response_time_ms: Enum.reduce(performance_data, 0, & &1.response_time_ms + &2) / total_evaluations,
        total_evaluations: total_evaluations,
        avg_cost_per_evaluation: Enum.reduce(performance_data, 0, & &1.cost_usd + &2) / total_evaluations,
        avg_confidence: Enum.reduce(performance_data, 0, & &1.confidence + &2) / total_evaluations,
  ...
  defp get_recent_performance_history(performance_data) when is_list(performance_data) do
    performance_data
    |> Enum.take(10)  # Last 10 evaluations
    |> Enum.map(fn entry ->
      %{
  ...
  defp calculate_health_trend(performance_data) when is_list(performance_data) do
    if length(performance_data) < 5 do
      ...
      older_success_rate = if Enum.empty?(older_data) do
        recent_success_rate
      else
  ...
  defp generate_health_recommendations(health_info, performance_data) do
    ...
    # Check success rate
    recommendations = if health_info.status != :healthy do
      ["Provider showing health issues - consider using alternative provider"] ++ recommendations
    else
      recommendations
    end
    ...
    recommendations = if avg_response_time > @alert_thresholds.response_time_warning do
    ...
    recommendations = case calculate_cost_efficiency(performance_data) do
      :high_cost -> ["Provider costs above average - consider cost optimization"] ++ recommendations
      _ -> recommendations
    end
  ...
  defp calculate_analytics_for_window(performance_data, time_window) when is_list(performance_data) do
    cutoff_time = case time_window do
    ...
    window_data = Enum.filter(performance_data, &(DateTime.compare(&1.timestamp, cutoff_time) != :lt))
  ...
  defp analyze_reliability_trends(performance_data) do
    ...
    %{
      ...
      avg_confidence: Enum.reduce(performance_data, 0, & &1.confidence + &2) / length(performance_data),
  ...
  defp generate_analytics_recommendations(performance_data) do
    ...
    recommendations = if performance_metrics.success_rate < 0.95 do
      ["Consider provider configuration review due to low success rate"] ++ recommendations
    else
      recommendations
    end
    
    recommendations = if performance_metrics.avg_response_time_ms > 5000 do
      ["Response times elevated - check provider load and network conditions"] ++ recommendations
    else
      recommendations
    end
    
    recommendations = if performance_metrics.avg_cost_per_evaluation > 0.3 do
  ...
  defp update_health_from_performance(health_data, provider_type, performance_entry) do
    current_health = Map.get(health_data, provider_type, %{
      status: :unknown,
      consecutive_successes: 0,
      consecutive_failures: 0
    })
    
    updated_health = if performance_entry.success do
      %{current_health |
        status: :healthy,
  ...
  defp check_and_send_health_alerts(provider_type, performance_entry, thresholds) do
    ...
    alerts = if not performance_entry.success do
      ["Provider #{provider_type} evaluation failed"] ++ alerts
    else
      alerts
    end
    
    alerts = if performance_entry.response_time_ms > thresholds.response_time_critical do
      ["Provider #{provider_type} response time critical: #{performance_entry.response_time_ms}ms"] ++ alerts
    else
      alerts
    end
  ...
  defp send_health_alerts(provider_type, alerts) do
    ...
    Phoenix.PubSub.broadcast(RubberDuck.PubSub, "provider_health_alerts", 
