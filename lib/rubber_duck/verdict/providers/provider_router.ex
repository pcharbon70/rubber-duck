defmodule RubberDuck.Verdict.Providers.ProviderRouter do
  ...
  def select_provider(evaluation_request, user_id, project_id \\ nil, options \\ []) do
    ...
    with {:ok, config} <- get_routing_configuration(user_id, project_id),
         ...
         {:ok, routing_decision} <- apply_routing_strategy(available_providers, evaluation_request, config, options) do
  ...
  defp score_provider_for_cost({provider_type, provider_info}, request, _config) do
    ...
    %{
      ...
      cost_score: 1.0 / max(cost_estimate, 0.001)  # Inverse of cost
    }
  end
  ...
  defp score_provider_balanced({provider_type, provider_info}, request, config) do
    ...
    max_cost = 2.0  # Assume max reasonable cost
  ...
  defp get_provider_quality_score(provider_info, evaluation_type) do
    ...
    type_modifier = case {get_provider_type(provider_info), evaluation_type} do
      {:anthropic, :security} -> 0.1    # Claude excels at security analysis
      {:openai, :performance} -> 0.05   # GPT-4 good at performance analysis
      {:ollama, :style} -> -0.1         # Local models less sophisticated for style
      _ -> 0.0
    end
  ...
  defp get_provider_speed_score(provider_info) do
    ...
    case provider_type do
      ...
      :anthropic -> 0.7  # Claude can be slower but thoughtful
      _ -> 0.6
    end
  end
  ...
  defp get_provider_reliability_score(provider_info) do
    ...
    case provider_type do
      :openai -> 0.95      # OpenAI generally very reliable
      :anthropic -> 0.92   # Anthropic very reliable
      :ollama -> 0.85      # Local models depend on hardware
      _ -> 0.8
    end
  end
  ...
  defp estimate_token_count(code, evaluation_type) do
    ...
    context_tokens = case evaluation_type do
      :security -> 800   # Security evaluations need more context
      :performance -> 600
      :quality -> 500
      :maintainability -> 400
      :style -> 300
  ...
  defp get_model_cost_rate(provider_info, model) do
    ...
    case {provider_type, model} do
      ...
      {:ollama, _} -> 0.0  # Local models have no API cost
      _ -> 0.015  # Default fallback rate
    end
  end
  ...
  defp get_historical_accuracy(provider_type, evaluation_type) do
    ...
    base_accuracy = case provider_type do
      :anthropic -> 0.92
      :openai -> 0.90
      :ollama -> 0.82
      _ -> 0.85
    ...
    type_modifier = case {provider_type, evaluation_type} do
  ...
  defp get_average_response_time(provider_type) do
    ...
    case provider_type do
      :ollama -> 1500    # Fast local inference
      :openai -> 2500    # Generally fast API
      :anthropic -> 3500 # More thoughtful, slower
      _ -> 3000
    end
  end
  ...
  defp get_provider_uptime(provider_type) do
    ...
    case provider_type do
      :openai -> 99.5
      :anthropic -> 99.2
      :ollama -> 98.0  # Depends on local hardware
      _ -> 98.5
    end
  end
  ...
  defp build_provider_recommendation({provider_type, provider_info}, request, config) do
    ...
    %{
      ...
      speed_score: speed_score,
      reliability_score: reliability_score,
      recommendation_reason: generate_recommendation_reason(provider_type, quality_score, cost_estimate, speed_score)
    }
  end
  ...
  defp generate_recommendation_reason(provider_type, quality_score, cost_estimate, speed_score) do
    ...
    case {provider_type, strengths} do
      {:anthropic, strengths} -> "Claude provider - #{Enum.join(strengths, ", ")} with safety focus"
      {:openai, strengths} -> "OpenAI provider - #{Enum.join(strengths, ", ")} with broad capabilities" 
      {:ollama, strengths} -> "Local model - #{Enum.join(strengths, ", ")} with privacy benefits"
      {type, []} -> "#{type} provider - standard capabilities"
      {type, strengths} -> "#{type} provider - #{Enum.join(strengths, ", ")}"
    end
  end
