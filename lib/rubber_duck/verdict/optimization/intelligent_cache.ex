defmodule RubberDuck.Verdict.Optimization.IntelligentCache do
  ...
  """
  @spec generate_cache_key(code :: String.t(), evaluation_type :: atom(), config :: map()) :: String.t()
    ...
    case Map.get(state.cache_store, cache_key) do
      nil ->
        ...
        case find_similar_cached_result(cache_key, state) do
          {:ok, similar_result} ->
            new_stats = %{state.stats | hits: state.stats.hits + 1, similarity_matches: state.stats.similarity_matches + 1}
      ...
      cached_entry ->
        case is_cache_entry_valid?(cached_entry) do
          true ->
          ...
          false ->
            ...
            new_stats = %{state.stats | misses: state.stats.misses + 1, evictions: state.stats.evictions + 1}
  ...
  defp find_similar_cache_entries(target_code_hash, evaluation_type, cache_store) do
    ...
    |> Enum.filter(fn {_key, entry} ->
      ...
      is_cache_entry_valid?(entry) and
    ...
    |> Enum.take(3)  # Top 3 similar entries
  ...
  defp is_cache_entry_valid?(entry) do
  ...
  defp remove_expired_entries(cache_store) do
    ...
    {valid_entries, expired_entries} = Enum.split_with(cache_store, fn {_key, entry} ->
  ...
  defp schedule_cleanup do
    Process.send_after(self(), :cleanup_expired, 300_000)  # 5 minutes
  end
end