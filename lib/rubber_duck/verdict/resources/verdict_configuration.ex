defmodule RubberDuck.Verdict.Resources.VerdictConfiguration do
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :configuration_name, :configuration_type, :enabled, :default_model,
        :detailed_model, :quality_threshold, :confidence_threshold,
        :max_tokens_per_evaluation, :budget_per_day, :progressive_evaluation,
        :cache_enabled, :cache_ttl_seconds, :bias_mitigation_enabled,
        :feedback_learning_enabled, :evaluation_timeout_ms, :retry_attempts,
        :supported_evaluation_types, :model_preferences, :cost_optimization_settings,
        :performance_thresholds, :version
      ]
    ...
    update :update_settings do
      ...
      accept [
        :enabled, :default_model, :detailed_model, :quality_threshold,
        :confidence_threshold, :max_tokens_per_evaluation, :budget_per_day,
        :progressive_evaluation, :cache_enabled, :cache_ttl_seconds,
        :bias_mitigation_enabled, :feedback_learning_enabled, :evaluation_timeout_ms,
        :retry_attempts, :supported_evaluation_types, :model_preferences,
        :cost_optimization_settings, :performance_thresholds, :version
      ]
    end
  ...
  calculations do
    ...
    calculate :has_progressive_optimization, :boolean, expr(progressive_evaluation and cache_enabled)
    calculate :estimated_daily_evaluations, :integer, expr(round(budget_per_day / 0.02))
    calculate :configuration_completeness, :decimal, expr(
      cond do
        not is_nil(default_model) and not is_nil(detailed_model) and budget_per_day > 0 -> 1.0
        not is_nil(default_model) and not is_nil(detailed_model) -> 0.8
        not is_nil(default_model) and budget_per_day > 0 -> 0.7
        true -> 0.5
      end
    )
  end
