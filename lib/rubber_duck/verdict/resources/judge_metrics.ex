defmodule RubberDuck.Verdict.Resources.JudgeMetrics do
  ...
  attributes do
    ...
    attribute :evaluation_type, :atom do
      ...
      constraints one_of: [:quality, :security, :performance, :maintainability, :best_practices, :comprehensive, :all]
      allow_nil? false
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :judge_unit_type, :model_name, :evaluation_type, :time_period,
        :period_start, :period_end, :total_evaluations, :successful_evaluations,
        :failed_evaluations, :average_score, :average_confidence, :total_cost_usd,
        :total_tokens_used, :average_latency_ms, :cache_hit_rate,
        :progressive_efficiency, :bias_indicators, :quality_trends, :performance_alerts
      ]
      
      validate present([:judge_unit_type, :model_name, :evaluation_type, :period_start, :period_end])
    end
    ...
    update :update_metrics do
      ...
      accept [
        ...
        :average_score, :average_confidence, :total_cost_usd, :total_tokens_used,
        :average_latency_ms, :cache_hit_rate, :progressive_efficiency,
        :bias_indicators, :quality_trends, :performance_alerts
      ]
    end
  ...
  policies do
    ...
    policy action_type(:read) do
      ...
      authorize_if actor_attribute_equals(:role, :user)  # Users can see basic metrics
    end
    
    policy action([:create, :update_metrics, :add_alert]) do
      description "Only system can create/update metrics"
      authorize_if always()  # System-level operations
    end
