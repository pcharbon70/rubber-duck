defmodule RubberDuck.Verdict.Resources.EvaluationFeedback do
  ...
  attributes do
    ...
    attribute :feedback_type, :atom do
      ...
      constraints one_of: [:accuracy, :usefulness, :bias_report, :improvement_suggestion, :general]
      allow_nil? false
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :evaluation_run_id, :evaluation_result_id, :user_id, :feedback_type,
        :rating, :agreed_with_result, :comments, :suggested_score,
        :issues_missed, :false_positives, :bias_indicators,
        :helpful_recommendations, :unhelpful_recommendations, :context_information
      ]
    ...
    update :update_details do
      ...
      accept [
        :rating, :comments, :suggested_score, :issues_missed, :false_positives,
        :bias_indicators, :helpful_recommendations, :unhelpful_recommendations,
        :context_information
      ]
    end
  ...
  calculations do
    ...
    calculate :has_bias_report, :boolean, expr(fragment("array_length(?, 1) > 0", bias_indicators))
    calculate :has_improvement_suggestions, :boolean, expr(not is_nil(comments) and comments != "")
    calculate :feedback_value_score, :decimal, expr(
      cond do
        rating >= 4 and agreed_with_result and fragment("array_length(?, 1)", issues_missed) == 0 -> 1.0
        rating <= 2 and not agreed_with_result -> 0.8
        true -> 0.5
      end
    )
  end
