defmodule RubberDuck.Verdict.Resources.EvaluationResult do
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :evaluation_run_id, :judge_unit_type, :model_used, :score, :confidence,
        :issues_found, :recommendations, :reasoning, :evaluation_stage,
        :tokens_used, :cost_usd, :latency_ms, :prompt_template,
        :llm_request_metadata, :validation_errors, :bias_indicators, :quality_metrics
      ]
      
      validate present([
        :evaluation_run_id, :judge_unit_type, :model_used, :score, :confidence,
        :evaluation_stage, :tokens_used, :cost_usd, :latency_ms
      ])
    end
  ...
  calculations do
    ...
    calculate :recommendation_count, :integer, expr(fragment("array_length(?, 1)", recommendations))
  end
  ...
  policies do
    ...
    policy action_type(:create) do
      ...
      authorize_if always()  # System-level creation
    end
