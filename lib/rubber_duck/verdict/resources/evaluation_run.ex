defmodule RubberDuck.Verdict.Resources.EvaluationRun do
  ...
  attributes do
    ...
    attribute :evaluation_type, :atom do
      ...
      constraints one_of: [:quality, :security, :performance, :maintainability, :best_practices, :comprehensive]
      allow_nil? false
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :user_id, :project_id, :evaluation_type, :code_hash, :code_size_bytes,
        :configuration_snapshot, :judge_units_used, :evaluation_strategy,
        :cache_hit, :similar_evaluation_id, :metadata
      ]
  ...
  calculations do
    calculate :duration_ms, :integer, expr(
      cond do
        not is_nil(completed_at) -> fragment("EXTRACT(epoch FROM (? - ?)) * 1000", completed_at, started_at)
        true -> nil
      end
    )
