defmodule RubberDuck.Verdict.Adaptation.AdaptationHistory do
  ...
  validations do
    ...
    validate numericality(:effectiveness_score,
  ...
  attributes do
    ...
    attribute :adaptation_type, :atom do
      ...
      constraints one_of: [
                    ...
                    :consensus_mechanism_update,
                    :cost_optimization,
                    :bias_mitigation,
                    :criteria_weight_adjustment,
  ...
  calculations do
    ...
    calculate :adaptation_age_hours,
              :integer,
              expr(fragment("EXTRACT(EPOCH FROM (? - ?)) / 3600", now(), applied_at))

    calculate :time_since_validation_hours,
              :integer,
              expr(fragment("EXTRACT(EPOCH FROM (? - ?)) / 3600", now(), validated_at))

    calculate :needs_validation,
              :boolean,
              expr(
                adaptation_status == :monitoring and
                  adaptation_age_hours >= monitoring_duration_hours
              )
  end
  ...
  def find_similar_adaptations(target_adaptation, similarity_threshold \\ 0.8) do
    ...
    similar_adaptations =
      Enum.filter(all_adaptations, fn adaptation ->
  ...
  defp calculate_rollback_rate(adaptations_by_status) do
    total_adaptations =
      Enum.sum(
        Enum.map(adaptations_by_status, fn {_status, adaptations} ->
          length(adaptations)
        end)
      )
  ...
  defp analyze_effectiveness_trends(adaptations) when is_list(adaptations) do
    if length(adaptations) < 3 do
      ...
      effectiveness_scores =
        Enum.map(sorted_adaptations, fn adaptation ->
  ...
  defp analyze_adaptation_impacts(adaptations) when is_list(adaptations) do
    ...
    if Enum.empty?(validated_adaptations) do
      ...
      %{
        user_satisfaction_impact:
          calculate_average_impact(validated_adaptations, :user_satisfaction_impact),
        system_performance_impact:
          calculate_average_impact(validated_adaptations, :system_performance_impact),
  ...
  defp calculate_adaptation_similarity(adaptation1, adaptation2) do
    ...
    type_similarity =
      if adaptation1.adaptation_type == adaptation2.adaptation_type, do: 1.0, else: 0.0

    source_similarity =
      if adaptation1.adaptation_source == adaptation2.adaptation_source, do: 0.5, else: 0.0
  ...
  defp compare_adaptation_effectiveness(adaptation1, adaptation2) do
    ...
    %{
      ...
      better_adaptation:
        if(effectiveness1 > effectiveness2, do: adaptation1.id, else: adaptation2.id),
      confidence_comparison: compare_adaptation_confidence(adaptation1, adaptation2)
    }
  end

  defp calculate_parameter_similarity(params1, params2)
       when is_map(params1) and is_map(params2) do
    ...
    if Enum.empty?(common_keys) do
      ...
      similarities = calculate_parameter_similarities(common_keys, params1, params2)
  ...
  defp calculate_parameter_similarities(common_keys, params1, params2) do
