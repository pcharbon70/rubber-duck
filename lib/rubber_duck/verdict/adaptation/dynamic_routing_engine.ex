defmodule RubberDuck.Verdict.Adaptation.DynamicRoutingEngine do
  ...
  """
  def select_optimal_judges(evaluation_context, available_judges, user_profile \\ nil, options \\ []) do
    ...
    case load_routing_models(routing_strategy) do
      {:ok, routing_models} ->
        case apply_dynamic_routing_strategy(routing_strategy, evaluation_context, available_judges, user_profile, routing_models, options) do
          {:ok, judge_selection} ->
            selection_reasoning = generate_selection_reasoning(judge_selection, routing_strategy, evaluation_context)
  ...
  def adapt_coordination_strategy(coordination_context, learned_patterns, options \\ []) do
    Logger.info("Adapting coordination strategy from #{length(learned_patterns)} learned patterns")
    
    case analyze_coordination_effectiveness(learned_patterns, coordination_context) do
      {:ok, effectiveness_analysis} ->
        case generate_adaptive_coordination_strategy(effectiveness_analysis, coordination_context, options) do
          {:ok, strategy} ->
            ...
            result = %{
              ...
              adaptation_confidence: calculate_coordination_adaptation_confidence(effectiveness_analysis),
  ...
  defp load_routing_models(routing_strategy) do
    ...
    models = Enum.reduce(model_types, %{}, fn model_type, acc ->
  ...
  defp apply_dynamic_routing_strategy(strategy, context, available_judges, user_profile, models, options) do
    case strategy do
      :learned_optimal ->
      ...
      :performance_weighted ->
        apply_performance_weighted_routing(context, available_judges, user_profile, models, options)
  ...
  defp apply_learned_optimal_routing(context, available_judges, user_profile, models, options) do
    ...
    case generate_learned_judge_selection(context, available_judges, judge_selection_model, pattern_model) do
      {:ok, selection} ->
        # Enhance with user preferences if available
        enhanced_selection = if user_profile do
  ...
  defp apply_temporal_adaptive_routing(context, available_judges, user_profile, models, options) do
    ...
    judge_selection = generate_temporal_adapted_selection(available_judges, temporal_factors, temporal_model)
    
    {:ok, judge_selection}
  end

  defp apply_performance_weighted_routing(context, available_judges, user_profile, models, options) do
    ...
    judge_selection = generate_performance_weighted_selection(context, available_judges, performance_weights)
  ...
  defp apply_cost_optimized_routing(context, available_judges, user_profile, models, options) do
    ...
    judge_selection = generate_cost_optimized_selection(available_judges, cost_constraints, cost_model)
  ...
  defp generate_learned_judge_selection(context, available_judges, judge_model, pattern_model) do
    ...
    optimal_judges = case evaluation_complexity do
    ...
    selection = %{
      ...
      selection_reasoning: "Learned optimal based on evaluation complexity: #{evaluation_complexity}"
    }
  ...
  defp generate_preference_based_selection(context, available_judges, user_profile, models) do
    ...
    final_judges = if length(preferred_judges) >= 2 do
    ...
    selection = %{
      ...
      expected_performance: %{
        ...
        user_satisfaction: 0.9  # High satisfaction due to preference alignment
      },
  ...
  defp generate_temporal_adapted_selection(available_judges, temporal_factors, temporal_model) do
    ...
    judge_effectiveness_predictions = Enum.map(available_judges, fn judge ->
      predicted_effectiveness = predict_temporal_judge_effectiveness(judge, temporal_factors, temporal_model)
      {judge, predicted_effectiveness}
    end)
    
    # Select top performing judges for current time
    optimal_temporal_judges = judge_effectiveness_predictions
  ...
  defp generate_cost_optimized_selection(available_judges, cost_constraints, cost_model) do
    ...
    judge_cost_efficiency = Enum.map(available_judges, fn judge ->
    ...
    selected_judges = judge_cost_efficiency
  ...
  defp generate_quality_focused_selection(available_judges, quality_requirements) do
    ...
    high_quality_judges = Enum.filter(available_judges, fn judge ->
    ...
    selected_judges = if length(high_quality_judges) >= 3 do
      Enum.take(high_quality_judges, 4)  # More judges for higher quality
    else
    ...
    %{
      ...
      expected_performance: %{
        accuracy: 0.92,
        processing_time: 6_500,  # Longer due to thoroughness
  ...
  defp analyze_coordination_effectiveness(patterns, coordination_context) do
    ...
    if Enum.empty?(coordination_patterns) do
      ...
      effectiveness_analysis = %{
        consensus_mechanism_effectiveness: analyze_consensus_mechanism_effectiveness(coordination_patterns),
        negotiation_strategy_effectiveness: analyze_negotiation_strategy_effectiveness(coordination_patterns),
        timeout_configuration_effectiveness: analyze_timeout_configuration_effectiveness(coordination_patterns),
  ...
  defp assess_evaluation_complexity(evaluation_context) when is_map(evaluation_context) do
    ...
    complexity_score = Enum.reduce(complexity_factors, 0, fn {factor, weights}, acc ->
  ...
  defp select_comprehensive_judge_team(available_judges, judge_model) do
    ...
    if length(available_judges) >= 4 do
      Enum.take(available_judges, 4)  # All judge types for comprehensive analysis
    else
      available_judges
    end
  end
  ...
  defp select_balanced_judge_team(available_judges, judge_model) do
    ...
    selected = Enum.filter(available_judges, fn judge ->
  ...
  defp select_efficient_judge_team(available_judges, judge_model) do
    ...
    efficient_judges = [:code_quality, :architecture]  # Core judges for efficiency
  ...
  defp enhance_selection_with_user_preferences(selection, user_profile) when is_map(user_profile) do
  ...
  defp filter_judges_by_user_preferences(available_judges, user_preferences) when is_list(user_preferences) do
    Enum.filter(available_judges, fn judge ->
      judge in user_preferences
    end)
  end

  defp integrate_user_preferences(current_judges, user_preferred_judges) when is_list(current_judges) and is_list(user_preferred_judges) do
  ...
  defp calculate_preference_alignment(selected_judges, user_preferred_judges) when is_list(selected_judges) and is_list(user_preferred_judges) do
  ...
  defp generate_performance_weighted_selection(context, available_judges, performance_weights) do
    ...
    judge_scores = Enum.map(available_judges, fn judge ->
  ...
  defp generate_cost_optimized_selection(available_judges, cost_constraints, cost_model) do
    ...
    judge_cost_data = Enum.map(available_judges, fn judge ->
  ...
  defp extract_quality_requirements(context, options) do
    %{
      min_accuracy: Keyword.get(options, :min_accuracy, Map.get(context, :accuracy_requirement, 0.8)),
  ...
  defp generate_quality_focused_selection(available_judges, quality_requirements) do
    ...
    quality_capable_judges = Enum.filter(available_judges, fn judge ->
      get_judge_quality_rating(judge) >= min_accuracy
    end)
    
    # Select all quality-capable judges for thoroughness
    selected_judges = if length(quality_capable_judges) >= 3 do
    ...
    %{
      judges: Enum.take(selected_judges, 4),  # Comprehensive team
      confidence: 0.88,
      expected_performance: %{
        accuracy: 0.91,
  ...
  defp fallback_to_default_routing(available_judges, evaluation_context) do
    ...
    default_judges = case length(available_judges) do
      n when n >= 3 -> Enum.take(available_judges, 3)
      n when n >= 2 -> available_judges ++ [:code_quality]  # Ensure minimum coverage
      _ -> [:code_quality, :architecture]  # Default minimum set
    end
  ...
  defp calculate_judge_team_cost(judges) when is_list(judges) do
    ...
    judge_costs = Enum.map(judges, fn judge ->
      case judge do
        :security -> base_cost_per_judge * 1.2  # Security judges cost more
  ...
  defp select_judges_within_budget(judge_cost_data, budget_limit) do
    ...
    sorted_by_efficiency = Enum.sort_by(judge_cost_data, fn {_judge, efficiency} -> -efficiency end)
    
    {selected_judges, _remaining_budget} = Enum.reduce_while(sorted_by_efficiency, {[], budget_limit}, fn {judge, _efficiency}, {acc_judges, remaining_budget} ->
  ...
  defp select_judges_within_budget_optimized(judge_cost_data, budget_limit) do
    ...
    efficient_judges = judge_cost_data
      |> Enum.sort_by(fn {_judge, data} -> -data.efficiency end)
      |> Enum.take(4)  # Top 4 most efficient
    
    # Simple selection within budget
    budget_compliant_judges = Enum.filter(efficient_judges, fn {_judge, data} ->
      data.cost <= available_budget / 3  # Rough budget allocation per judge
    end)
  ...
  defp get_hourly_effectiveness_adjustment(judge, hour) do
    ...
    case {judge, hour} do
      ...
      {_, h} when h in [0, 1, 2, 3, 4, 5] -> -0.1  # Late night penalty
      _ -> 0.0
    end
  end
  ...
  defp get_daily_effectiveness_adjustment(judge, day) do
    ...
    case day do
      day when day in [1, 2, 3] -> 0.02  # Mon-Wed bonus
      day when day in [6, 7] -> -0.05     # Weekend penalty
      _ -> 0.0
    end
  end
  ...
  defp assess_judge_requirement_fit(judge, requirements) when is_map(requirements) do
    ...
    accuracy_fit = if judge_capabilities.accuracy >= requirements.accuracy_requirement, do: 1.0, else: 0.5
  ...
  defp assess_speed_requirement_fit(capabilities, speed_requirement) do
    ...
    case speed_requirement do
      ...
      :standard -> 0.8  # Standard requirement, most judges fit
      _ -> 0.7
    end
  end
  ...
  defp get_routing_strategy_distribution(_time_window), do: %{learned_optimal: 120, user_preference_based: 80, temporal_adaptive: 45}
  defp get_average_selection_confidence(_time_window), do: 0.83
  defp get_model_usage_statistics(_time_window), do: %{judge_selection_model: 180, user_preference_model: 95}
  defp calculate_adaptation_success_rate(_time_window), do: 0.91
  defp calculate_fallback_usage_rate(_time_window), do: 0.08
  defp measure_performance_improvements(_time_window), do: %{accuracy_improvement: 0.12, satisfaction_improvement: 0.15}
  ...
  defp determine_base_coordination_strategy(_analysis, _context), do: %{strategy: :adaptive_consensus}
