defmodule RubberDuck.Analyzers.Code.Security do
  ...
  defp add_file_type_vulnerabilities(vulnerabilities, content, :elixir) do
    vulnerabilities =
      if String.contains?(content, ":os.cmd") do
        [
          %{type: :os_command_injection, severity: :high, message: "OS command execution detected"}
          | vulnerabilities
        ]
      else
        vulnerabilities
    ...
    vulnerabilities =
      if String.contains?(content, "String.to_atom") && 
        ...
        [
          %{type: :atom_injection, severity: :medium, message: "Unsafe atom creation from user input"}
          | vulnerabilities
        ]
      else
        vulnerabilities
  ...
  defp check_input_validation(_), do: %{validated_inputs: 0, unvalidated_risks: [], validation_score: 0.0}
