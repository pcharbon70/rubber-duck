defmodule RubberDuck.Analyzers.Code.Impact do
  ...
  def analyze(%ImpactAssess{} = msg, context) do
    ...
    if impact_result.risk_assessment.level == :high do
      ...
      require Logger
      Logger.warning("High risk detected for #{msg.file_path}: #{inspect(impact_result.risk_assessment)}")
    end
  ...
  def metadata do
    %{
      ...
      description: "Analyzes code change impact including dependencies, risks, and performance implications",
      version: "1.0.0",
  ...
  defp calculate_impact_severity(data) do
    ...
    severity_score = severity_score + if (data[:lines_changed] || data[:lines_affected] || 0) > 100, do: 2, else: 0
  ...
  defp calculate_impact_risk_score(changes) when is_map(changes) do
    ...
    size_risk = if (changes[:lines_changed] || changes[:lines_affected] || 0) > 100, do: 30, else: 0
    ...
    modules_risk = if (changes[:modules_affected] && length(changes[:modules_affected]) > 2), do: 20, else: 0
  ...
  defp identify_affected_files(file_path, _state) do
    ...
    |> Enum.take(5) # Limit to reasonable number
  end
end