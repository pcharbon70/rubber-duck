defmodule RubberDuck.Analyzers.Code.Performance do
  ...
  defp estimate_memory_usage(content) when is_binary(content) do
    ...
    total_score = large_structures + recursion
  ...
  defp detect_database_queries(content) when is_binary(content) do
    ...
    queries =
      if String.contains?(content, "Repo.") do
        ...
        [
          %{type: :ecto, count: ecto_count, operations: extract_repo_operations(content)}
          | queries
        ]
      else
        queries
  ...
  defp identify_bottlenecks(content) when is_binary(content) do
      ...
      if String.contains?(content, "Enum.") && String.contains?(content, "|> Enum.") do
        [
          %{
            ...
            severity: :medium,
            suggestion: "Consider using Stream or single pass"
          }
          | bottlenecks
        ]
      else
      ...
      if count_pattern(content, ~r/\bn\+1\b|N\+1/) > 0 do
        [
          %{
            type: :n_plus_one,
            severity: :high,
            suggestion: "Potential N+1 query pattern detected"
          }
          | bottlenecks
        ]
      else
    ...
    bottlenecks =
      if String.contains?(content, ["Repo.all", "Repo.get"]) &&
           String.contains?(content, ["Enum.map", "for"]) do
        [
          %{
            type: :potential_n_plus_one,
            severity: :high,
            suggestion: "Multiple database queries in loop"
          }
          | bottlenecks
        ]
      else
      ...
      if String.contains?(content, "Enum.") and not String.contains?(content, "Stream.") do
        [
          %{
            type: :no_streaming,
            severity: :low,
            suggestion: "Large data operations without Stream module"
          }
          | bottlenecks
        ]
      else
      ...
      if String.contains?(content, ["HTTPoison.get", "GenServer.call"]) &&
        ...
        [
          %{
            ...
            severity: :medium,
            suggestion: "Consider async operations for external calls"
          }
          | bottlenecks
        ]
      else
  ...
  defp identify_performance_hotspots(content) when is_binary(content) do
    ...
    nested_enum_pattern =
      String.contains?(content, ["for", "Enum.each"]) and
    ...
    hotspots =
      if nested_enum_pattern do
        [
          %{
            type: :nested_enumeration,
            severity: :high,
            description: "Nested enumerations detected",
            suggestion: "Consider flattening or using comprehensions"
          }
          | hotspots
        ]
      else
      ...
      if String.contains?(content, ["Enum.reduce", "for"]) && String.contains?(content, "<>") do
        [
          %{
            ...
            suggestion: "Consider using IO lists or Enum.join/2"
          }
          | hotspots
        ]
      else
  ...
  defp calculate_optimization_potential(content) when is_binary(content) do
    ...
    if String.trim(content) == "" do
      ...
      potential =
        potential +
          case complexity_analysis.complexity do
            :exponential -> 50
            :quadratic -> 30
            :logarithmic -> 10
            _ -> 5
