defmodule RubberDuck.Analyzers.Orchestrator do
  ...
  defp create_execution_plan(request) do
    ...
    plan = case request.strategy do
      ...
      :standard ->
        analyzed = if request.analyzers == :all do
        ...
        phases = []
        phases = if length(phase1) > 0, do: phases ++ [%{analyzers: phase1, parallel: true, depth: :moderate}], else: phases
        phases = if length(phase2) > 0, do: phases ++ [%{analyzers: phase2, parallel: true, depth: :moderate}], else: phases
  ...
  defp execute_analysis_plan(plan, request) do
    results = Enum.reduce(plan.phases, %{}, fn phase, acc ->
      phase_results = if Map.get(phase, :parallel, false) do
  ...
  defp execute_parallel_phase(phase, request, previous_results) do
    tasks = Enum.map(phase.analyzers, fn analyzer ->
      Task.async(fn ->
  ...
  defp execute_sequential_phase(phase, request, previous_results) do
    Enum.reduce(phase.analyzers, %{}, fn analyzer, acc ->
      ...
      result = case run_analyzer(analyzer, message, context) do
        {:ok, result} -> result
        {:error, reason} ->
          Logger.warning("Analyzer #{analyzer} failed: #{inspect(reason)}")
          %{error: reason}
  ...
  defp generate_cross_analyzer_insights(results) do
    ...
    insights = insights ++ correlate_security_performance(
      results[:security],
      results[:performance]
    )
    
    # Quality + Impact correlation
    insights = insights ++ correlate_quality_impact(
      results[:quality],
      results[:impact]
    )
    
    # Performance + Impact correlation
    insights = insights ++ correlate_performance_impact(
      results[:performance],
      results[:impact]
    )
  ...
  defp correlate_security_performance(security, performance) when is_map(security) and is_map(performance) do
    insights = []
    ...
    if length(security[:vulnerabilities] || []) > 0 and performance[:optimization_potential] > 0.3 do
      insights ++ [%{
        ...
        severity: :medium,
        message: "Security vulnerabilities detected alongside performance issues. Fix security first.",
  ...
  defp correlate_quality_impact(quality, impact) when is_map(quality) and is_map(impact) do
    ...
    if quality[:metrics][:complexity] > 10 and impact[:severity] in [:high, :critical] do
      insights ++ [%{
        ...
        severity: :high,
        message: "High complexity code with significant impact. Consider breaking into smaller changes.",
  ...
  defp correlate_performance_impact(performance, impact) when is_map(performance) and is_map(impact) do
    insights = []
    
    # Performance bottlenecks in high-impact areas
    if length(performance[:bottlenecks] || []) > 0 and impact[:scope] in [:system_wide, :module_wide] do
      insights ++ [%{
        type: :critical_performance_bottleneck,
        severity: :critical,
        message: "Performance bottlenecks in high-impact code paths require immediate attention.",
  ...
  defp generate_recommendations(results, insights) do
    ...
    recommendations = recommendations ++ generate_performance_recommendations(results[:performance])
  ...
  defp generate_performance_recommendations(performance) do
    ...
    if performance[:optimization_potential] > 0.5 do
      recommendations ++ [%{
        action: "Optimize performance bottlenecks for #{round(performance.optimization_potential * 100)}% improvement",
        priority: :high,
        effort: :moderate,
        impact: :high,
  ...
  defp generate_quality_recommendations(quality) do
    ...
    if quality[:quality_score] < 0.5 do
      recommendations ++ [%{
        action: "Improve code quality (current score: #{round(quality.quality_score * 100)}%)",
        priority: :medium,
        effort: :high,
        impact: :medium,
        analyzers: [:quality]
  ...
  defp prioritize_recommendations(recommendations) do
    recommendations
    |> Enum.sort_by(fn rec ->
      priority_score = case rec.priority do
        :critical -> 0
        :high -> 1
        :medium -> 2
        :low -> 3
  ...
  defp calculate_performance_score(performance) do
    1.0 - (performance[:optimization_potential] || 0)
  end
  ...
  defp merge_analysis_results(initial, deep) do
    %{initial |
      ...
      recommendations: deduplicate_recommendations(
        initial.recommendations ++ deep.recommendations
      ),
      overall_health: calculate_overall_health(
        Map.merge(initial.results, deep.results)
      )
    }
  end
