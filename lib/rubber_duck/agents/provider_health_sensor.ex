defmodule RubberDuck.Agents.ProviderHealthSensor do
  ...
  @health_check_interval 15_000  # 15 seconds - more frequent than base system
  @trend_analysis_interval 60_000  # 1 minute
  @anomaly_detection_interval 120_000  # 2 minutes
  @prediction_update_interval 300_000  # 5 minutes
  
  @health_thresholds %{
    critical_success_rate: 0.85,
    warning_success_rate: 0.90,
    critical_response_time: 15_000,  # 15 seconds
    warning_response_time: 8_000,    # 8 seconds
    critical_cost_variance: 0.5,     # 50% cost increase
    warning_cost_variance: 0.3       # 30% cost increase
  }
  ...
  def get_provider_health_details(agent, provider_type) do
    ...
    case Map.get(current_state.provider_health_data, provider_type) do
      ...
      provider_health ->
        ...
        provider_anomalies = get_provider_anomalies(current_state.anomaly_detection, provider_type)
        ...
        {:ok, %{
          ...
          recommendations: generate_provider_specific_recommendations(provider_health, provider_trends)
        }}
    end
  end
  ...
  def mount(agent) do
    ...
    initial_state = %{
      ...
      predictive_analytics: %{
        prediction_models: %{},
        forecast_horizon: 3600_000,  # 1 hour
        prediction_accuracy: %{}
      },
  ...
  def handle_signal(agent, %{pattern: "health_sensor.trend_analysis"} = signal) do
    ...
    trend_analysis = perform_trend_analysis(current_state.provider_health_data, current_state.health_trends)
  ...
  def handle_signal(agent, %{pattern: "health_sensor.anomaly_detection"} = signal) do
    ...
    anomalies = detect_provider_anomalies(current_state.provider_health_data, current_state.anomaly_detection)
    
    # Update anomaly detection state
    updated_anomaly_state = update_anomaly_detection_state(current_state.anomaly_detection, anomalies)
  ...
  def handle_signal(agent, %{pattern: "health_sensor.predictive_update"} = signal) do
    ...
    updated_predictions = update_predictive_models(
  ...
  defp enhance_health_data_with_metrics(provider_health) do
    ...
    enhanced_health = provider_health
  ...
  defp merge_health_data(current_data, new_data) do
    ...
    |> Enum.reduce(current_data, fn {provider, health_info}, acc ->
      Map.update(acc, provider, health_info, fn existing_data ->
        ...
        updated_history = [health_info | health_history] |> Enum.take(100)  # Keep last 100 entries
  ...
  defp calculate_provider_health_trend(existing_trend, new_health_info) do
    ...
    %{
      ...
      response_time_trend: calculate_metric_trend(existing_trend, new_health_info, :avg_response_time_ms),
  ...
  defp detect_statistical_anomalies(health_data, anomaly_state) do
    ...
    |> Enum.map(fn {provider, health_info} ->
      ...
      anomalies = if success_rate < 0.5 do
        [%{
      ...
      anomalies = if response_time > 20_000 do
        [%{
  ...
  defp detect_threshold_anomalies(health_data) do
    ...
    |> Enum.map(fn {provider, health_info} ->
      ...
      # Critical thresholds
      anomalies = if success_rate < @health_thresholds.critical_success_rate do
        [%{
      ...
      anomalies = if response_time > @health_thresholds.critical_response_time do
        [%{
  ...
  defp update_predictive_models(current_predictions, health_data, health_trends) do
    ...
    |> Enum.reduce(current_predictions, fn {provider, health_info}, acc ->
      Map.update(acc, provider, %{}, fn existing_predictions ->
        update_provider_predictions(existing_predictions, health_info, Map.get(health_trends, provider, %{}))
      end)
    end)
  end
  ...
  defp generate_health_recommendations(current_state) do
    ...
    recommendations = current_state.provider_health_data
  ...
  defp generate_provider_health_recommendations(provider, health_data) do
    ...
    recommendations = if success_rate < 0.9 do
      ["Consider investigating #{provider} reliability issues - success rate #{Float.round(success_rate * 100)}%" | recommendations]
    else
      recommendations
    end
    
    # Response time recommendations
    recommendations = if response_time > 8000 do
      ["Monitor #{provider} performance - response time #{response_time}ms above optimal" | recommendations]
    else
      recommendations
    end
  ...
  defp generate_system_health_recommendations(current_state) do
    ...
    system_recommendations = if total_providers < 2 do
  ...
  defp generate_provider_specific_recommendations(provider_health, provider_trends) do
    ...
    recommendations = case success_trend do
      :decreasing -> ["Monitor provider reliability - declining success rate trend detected" | recommendations]
      :increasing -> ["Provider showing improved reliability" | recommendations]
      _ -> recommendations
    end
  ...
  defp schedule_health_check do
    Process.send_after(self(), {:signal, %{pattern: "health_sensor.health_check"}}, @health_check_interval)
  end
  
  defp schedule_trend_analysis do
    Process.send_after(self(), {:signal, %{pattern: "health_sensor.trend_analysis"}}, @trend_analysis_interval)
  end
  
  defp schedule_anomaly_detection do
    Process.send_after(self(), {:signal, %{pattern: "health_sensor.anomaly_detection"}}, @anomaly_detection_interval)
  end
  
  defp schedule_prediction_update do
    Process.send_after(self(), {:signal, %{pattern: "health_sensor.predictive_update"}}, @prediction_update_interval)
  end
  ...
  defp broadcast_health_updates_if_needed(old_health, new_health) do
    ...
    if not Enum.empty?(significant_changes) do
      Phoenix.PubSub.broadcast(RubberDuck.PubSub, "health_sensor_alerts", 
