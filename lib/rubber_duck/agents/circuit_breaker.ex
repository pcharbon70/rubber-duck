defmodule RubberDuck.Agents.CircuitBreaker do
  ...
  def retry_dead_letters(agent_type, opts \\ []) do
    ...
    results =
      Enum.map(dead_letters, fn {instruction, original_opts} ->
  ...
  defp get_agent_config(agent_type) do
    ...
    agent_configs = %{
      # Critical agent
      llm_orchestrator: %{error_threshold: 3, timeout: 60_000},
      # Monitoring, lenient
      llm_monitoring: %{error_threshold: 10, timeout: 30_000},
      # Important
      project_agent: %{error_threshold: 5, timeout: 45_000},
      # Important
      code_file_agent: %{error_threshold: 5, timeout: 45_000},
      # Can be slow
      ai_analysis: %{error_threshold: 5, timeout: 60_000},
      # User-facing, strict
      user_agent: %{error_threshold: 3, timeout: 30_000}
    }
  ...
  defp handle_circuit_open(agent_type, instruction, opts) do
    ...
    if _fallback = Keyword.get(opts, :fallback) do
      attempt_fallback(agent_type, instruction, opts)
    else
      {:error, :circuit_open}
    end
