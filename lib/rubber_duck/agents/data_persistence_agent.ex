defmodule RubberDuck.Agents.DataPersistenceAgent do
  ...
  alias RubberDuck.Skills.QueryOptimizationSkill
  ...
  def analyze_query_patterns(agent, time_window_hours \\ 24) do
    ...
    recent_queries =
      Enum.filter(query_history, fn record ->
  ...
  def monitor_connection_pool(agent) do
    ...
    connection_stats =
      Map.merge(agent.connection_pool_stats, %{
  ...
  defp calculate_optimization_effectiveness([]),
    do: %{effectiveness: :no_data, optimization_rate: 0.0}

  defp calculate_optimization_effectiveness(queries) do
    optimized_queries = Enum.filter(queries, & &1.optimization_applied)
    optimization_rate = length(optimized_queries) / length(queries)
    avg_improvement = calculate_average_improvement(optimized_queries)
  ...
  defp calculate_average_improvement([]), do: 0.0

  defp calculate_average_improvement(optimized_queries) do
    improvements =
      Enum.map(optimized_queries, fn query ->
  ...
  defp generate_pattern_recommendations(queries) do
    ...
    recommendations =
      if unoptimized_count > length(queries) * 0.3 do
    ...
    recommendations =
      if length(slow_queries) > 5 do
        [
          "Focus on optimizing slow queries (#{length(slow_queries)} identified)"
          | recommendations
        ]
      else
        recommendations
  ...
  defp extract_access_patterns(query_history) do
    ...
    Enum.map(query_history, fn record ->
      %{
        ...
        # Each record represents one access
        access_count: 1,
  ...
  defp recommend_pool_adjustments(pool_stats) do
    ...
    adjustments =
      if pool_stats.average_wait_time_ms > 100 do
        ["Optimize query performance to reduce wait times" | adjustments]
      else
        adjustments
      end
  ...
  defp predict_pool_scaling_needs(pool_stats, agent) do
    ...
    query_history = agent.query_performance_history
    # Last hour
    recent_query_count = count_recent_queries(query_history, 3600)

    predicted_growth =
      case recent_query_count do
    ...
    scaling_recommendation =
      case {assess_pool_health(pool_stats), predicted_growth} do
        {:critical, _} ->
          :immediate_scaling_required

        {:warning, growth} when growth in [:high_growth_expected, :moderate_growth_expected] ->
          :proactive_scaling_recommended

        {:moderate, :high_growth_expected} ->
          :monitor_for_scaling
  ...
  defp summarize_query_optimization(agent) do
    ...
    %{
      ...
      average_performance_improvement: calculate_query_optimization_improvement(recent_history),
  ...
  defp summarize_cache_performance(agent) do
    ...
    %{
      ...
      predicted_hit_ratio:
        get_in(cache_perf, [:predicted_improvement, :predicted_hit_ratio]) || 0.0,
      cache_health: assess_cache_health(cache_perf),
      optimization_recommendations:
        get_in(cache_perf, [:last_optimization, :monitoring_recommendations]) || []
    }
  end
  ...
  defp calculate_overall_database_health(agent) do
    ...
    connection_health =
      Map.get(agent.connection_pool_stats, :last_analysis, %{}) |> Map.get(:pool_health, :unknown)
    ...
    total_score =
      health_scores[query_health] + health_scores[connection_health] + health_scores[cache_health]
  ...
  defp assess_temporal_coverage(query_history) do
    if Enum.empty?(query_history) do
      ...
      # Full confidence at 24+ hours coverage
      min(coverage_hours / 24.0, 1.0)
    end
  end
  ...
  defp calculate_query_optimization_improvement(queries) do
    optimized_queries = Enum.filter(queries, & &1.optimization_applied)
  ...
  defp assess_query_health(query_history) do
    ...
    if Enum.empty?(recent_queries) do
      ...
      slow_query_rate =
        Enum.count(recent_queries, fn query ->
          get_estimated_execution_time(query) > 1000
        end) / length(recent_queries)

      cond do
        # > 20% slow queries
        slow_query_rate > 0.2 -> :critical
        # > 10% slow queries
        slow_query_rate > 0.1 -> :warning
        # > 5% slow queries
        slow_query_rate > 0.05 -> :moderate
        true -> :healthy
      end
    end
  end
  ...
  defp assess_learning_effectiveness(_insights) do
    ...
    # Moderate effectiveness as default
    0.7
  end
end
