defmodule RubberDuck.Agents.UserAgent do
  ...
  defp calculate_frequency(pattern, _activity) do
    ...
    recent_activities_in_hour =
      Enum.count(pattern.recent_activities, fn activity ->
        DateTime.compare(activity.timestamp, hour_ago) == :gt
      end)

    # activities per hour
    recent_activities_in_hour / 1.0
  end

  defp generate_proactive_suggestions(patterns, current_activity) do
    patterns
    # Active patterns
    |> Enum.filter(fn {_type, pattern} -> pattern.frequency > 0.5 end)
  ...
  defp generate_suggestion_for_pattern(:code_analysis, pattern, _current_activity) do
    if pattern.frequency > 1.0 do
      %{
        type: :automation,
        message:
          "You frequently analyze code. Consider setting up automated analysis on file save.",
        priority: :medium,
  ...
  defp generate_suggestion_for_pattern(:project_navigation, pattern, _current_activity) do
    if pattern.count > 20 do
      %{
        type: :optimization,
        message:
          "Based on your navigation patterns, I can learn your preferred project structure.",
        priority: :low,
