defmodule RubberDuck.Agents.LLMOrchestratorAgent do
  ...
  def handle_instruction({:stream, request}, agent) do
    try do
      ...
      with {:ok, provider} <- select_optimal_provider(agent, request),
        ...
        # Wrap stream to track metrics with error handling
        tracked_stream = Stream.transform(stream, 0, fn chunk, token_count ->
          try do
  ...
  defp select_optimal_provider(agent, request) do
    ...
    if Enum.empty?(available_providers) do
      ...
      final_selection_reason = if insights.confidence > 0.7 && is_list(insights.recommendations) && length(insights.recommendations) > 0 do
        base_selection_reason <> " (Learning: " <> hd(insights.recommendations) <> ")"
      else
  ...
  defp calculate_provider_score(agent, provider, request) do
    ...
    # Success rate factor
    success_rate = calculate_success_rate(performance)
    success_factor = success_rate * 50
    
    # Speed factor (if we care about speed)
    avg_duration = calculate_average_duration(performance)
  ...
  defp calculate_success_rate(performance) do
  ...
  defp calculate_average_duration(performance) do
  ...
  defp apply_insight_adjustments(base_score, provider, insights) do
    ...
    if insights.applicable && insights.confidence > 0.5 do
      adjustments = insights.recommendations
      |> Enum.map(fn rec ->
        cond do
          String.contains?(rec, "#{provider.name}") && String.contains?(rec, "avoid") ->
            -20.0
          String.contains?(rec, "#{provider.name}") && String.contains?(rec, "prioritize") ->
            20.0
      ...
      adjustment = Enum.sum(adjustments) * insights.confidence
