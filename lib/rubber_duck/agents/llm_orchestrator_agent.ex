defmodule RubberDuck.Agents.LlmOrchestratorAgent do
  ...
  @performance_update_interval 300_000  # 5 minutes
      ...
      {:ok, provider_selection} ->
        ...
        case execute_optimized_request(optimized_request, provider_selection, domain, options) do
          {:ok, response} ->
            ...
            enhanced_response = enhance_response_with_orchestration_data(
              response, 
              provider_selection, 
              orchestration_time
            )
  ...
  def get_orchestration_analytics(agent) do
    ...
    %{
      ...
      provider_performance_summary: summarize_provider_performance(current_state.provider_performance_data),
  ...
  def handle_signal(agent, %{pattern: "orchestrator.health_check"} = signal) do
    ...
    updated_health = Map.merge(current_state.health_monitoring, %{
      last_health_check: DateTime.utc_now(),
    ...
    if health_status_changed_significantly?(current_state.health_monitoring, updated_health) do
      Phoenix.PubSub.broadcast(RubberDuck.PubSub, "orchestrator_events", 
  ...
  def handle_signal(agent, %{pattern: "orchestrator.performance_update"} = signal) do
    ...
    updated_learning = apply_learning_algorithms(current_state.learning_state, updated_performance)
  ...
  def handle_signal(agent, %{pattern: "orchestrator.learning_consolidation"} = signal) do
    ...
    learning_insights = consolidate_learning_insights(current_state.routing_decisions, current_state.learning_state)
      ...
      {:ok, available_providers} ->
        ...
        case apply_multi_criteria_selection(
          ...
          {:ok, selection} ->
            ...
            updated_decisions = [routing_decision | current_state.routing_decisions] |> Enum.take(1000)
  ...
  defp apply_multi_criteria_selection(available_providers, requirements, performance_data, learning_state) do
    # Multi-criteria optimization using performance data and learning
    scored_providers = available_providers
    |> Enum.map(fn {provider_type, provider_info} ->
      score_provider_for_requirements(provider_type, provider_info, requirements, performance_data, learning_state)
    end)
  ...
  defp score_provider_for_requirements(provider_type, provider_info, requirements, performance_data, learning_state) do
    ...
    historical_performance_score = calculate_historical_performance_score(provider_type, performance_data)
    learning_adjustment = calculate_learning_adjustment(provider_type, learning_state)
    cost_efficiency_score = calculate_cost_efficiency_score(provider_type, requirements, performance_data)
    ...
    %{
      ...
      performance_expectation: predict_provider_performance(provider_type, requirements, performance_data),
  ...
  defp optimize_request_for_provider(request, provider_selection, domain) do
    ...
    provider_optimizations = case provider_selection.provider do
      :anthropic ->
        %{
  ...
  defp execute_optimized_request(optimized_request, provider_selection, domain, options) do
    ...
    orchestration_options = Map.merge(options, %{
  ...
  defp learn_from_outcome(agent, provider_selection, response, orchestration_time) do
    ...
    updated_performance = update_provider_performance_data(current_state.provider_performance_data, learning_data)
    
    # Update optimization metrics
    updated_metrics = update_optimization_metrics(current_state.optimization_metrics, learning_data)
  ...
  defp handle_orchestration_failure(agent, provider_selection, error, request_requirements) do
    Logger.warning("Orchestration failed for provider #{provider_selection.provider}: #{inspect(error)}")
    ...
    updated_performance = record_provider_failure(current_state.provider_performance_data, failure_data)
  ...
  defp attempt_failure_recovery(agent, request_requirements, excluded_providers) do
    ...
    recovery_requirements = %{request_requirements |
      excluded_providers: Map.get(request_requirements, :excluded_providers, []) ++ excluded_providers
    }
  ...
  defp analyze_content_complexity(request) do
    content = case request.content do
  ...
  defp calculate_base_capabilities_score(provider_info, requirements) do
    ...
    feature_support = requirements.specialized_features
    |> Enum.map(fn feature ->
      if feature in Map.get(provider_info, :specializations, []), do: 1.0, else: 0.5
    end)
    |> case do
      [] -> 0.8  # No specific requirements
  ...
  defp calculate_historical_performance_score(provider_type, performance_data) do
    case Map.get(performance_data, provider_type) do
      nil -> 0.5  # No data, neutral score
      provider_data ->
        success_rate = Map.get(provider_data, :success_rate, 0.5)
        avg_quality = Map.get(provider_data, :avg_quality, 0.5)
        avg_performance = 1.0 - min(1.0, Map.get(provider_data, :avg_response_time, 5000) / 10_000)
    ...
    case Map.get(performance_data, provider_type) do
      ...
      provider_data ->
        ...
        if avg_cost <= max_acceptable_cost do
          1.0 - (avg_cost / max_acceptable_cost) * 0.5  # Better cost = higher score
        else
          0.2  # Cost too high
        end
    end
  ...
  defp determine_scoring_weights(requirements, learning_state) do
    ...
    adjusted_weights = case {requirements.cost_constraints.cost_priority, requirements.quality_requirements.quality_priority} do
  ...
  defp select_optimal_model_for_provider(provider_type, requirements) do
    case {provider_type, requirements.content_complexity, requirements.quality_requirements.quality_priority} do
  ...
  defp estimate_provider_cost(provider_type, requirements) do
    ...
    cost_per_1k_tokens = case provider_type do
      :anthropic -> 0.015
      :openai -> 0.02
      :ollama -> 0.0
      _ -> 0.015
  ...
  defp predict_provider_quality(provider_type, requirements, performance_data) do
    base_quality = case provider_type do
      :anthropic -> 0.9
      :openai -> 0.85
      :ollama -> 0.75
      _ -> 0.8
  ...
  defp predict_provider_performance(provider_type, requirements, performance_data) do
    base_performance = case provider_type do
  ...
  defp update_provider_performance_data(current_data, learning_data) do
    ...
    |> Map.update(provider, %{}, fn provider_data ->
      %{
        ...
        avg_response_time: update_average_response_time(provider_data, learning_data.actual_performance),
        cost_prediction_accuracy: update_prediction_accuracy(provider_data, :cost, learning_data),
        quality_prediction_accuracy: update_prediction_accuracy(provider_data, :quality, learning_data),
  ...
  defp update_prediction_accuracy(provider_data, metric_type, learning_data) do
    {predicted, actual} = case metric_type do
  ...
  defp estimate_response_quality(response) do
    ...
    length_quality = min(1.0, content_length / 500)  # Longer responses often better
  ...
  defp load_orchestrator_configuration do
    %{
      ...
      learning_consolidation_interval: 300_000  # 5 minutes
    }
  end
  ...
  defp schedule_health_monitoring do
    Process.send_after(self(), {:signal, %{pattern: "orchestrator.health_check"}}, @health_check_interval)
  end
  
  defp schedule_performance_updates do
    Process.send_after(self(), {:signal, %{pattern: "orchestrator.performance_update"}}, @performance_update_interval)
  end
  
  defp schedule_learning_consolidation do
    interval = 300_000  # 5 minutes
    Process.send_after(self(), {:signal, %{pattern: "orchestrator.learning_consolidation"}}, interval)
  end
  ...
  defp check_orchestrator_internal_health(current_state) do
    %{
      ...
      performance_data_health: assess_performance_data_health(current_state.provider_performance_data),
  ...
  defp assess_recent_decision_quality(routing_decisions) do
    ...
    if length(recent_decisions) > 5 do
      avg_confidence = recent_decisions
  ...
  defp update_optimization_metrics(metrics, _learning_data), do: Map.update!(metrics, :total_requests, &(&1 + 1))
