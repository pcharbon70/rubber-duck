defmodule RubberDuck.Prompts.Services.PromptSearchEngine do
  ...
  @search_cache_ttl :timer.minutes(10)  # 10-minute cache TTL for search results
  ...
  defp execute_advanced_search(search_criteria, user_id, advanced_options, state) do
    ...
    case Prompt.read(combined_filters) do
      {:ok, advanced_results} ->
        ...
        multi_criteria_ranked = apply_multi_criteria_ranking(advanced_results, search_criteria, state)
  ...
  defp generate_search_suggestions(partial_query, user_id, suggestion_options, state) do
    ...
    all_suggestions = (name_suggestions ++ content_suggestions)
    |> Enum.uniq()
  ...
  defp build_advanced_search_filters(search_criteria) do
    ...
    filters = if Map.has_key?(search_criteria, :category) do
    ...
    filters = if Map.has_key?(search_criteria, :date_range) do
    ...
    filters = if Map.has_key?(search_criteria, :tags) do
    ...
    filters = if Map.has_key?(search_criteria, :text) do
  ...
  defp calculate_exact_match_score(prompt, query_lower) do
    ...
    cond do
      String.downcase(prompt.name) == query_lower -> 1.0
      String.contains?(String.downcase(prompt.name), query_lower) -> 0.8
      prompt.description && String.contains?(String.downcase(prompt.description), query_lower) -> 0.6
      String.contains?(String.downcase(prompt.content), query_lower) -> 0.4
      true -> 0.0
    end
  end
  ...
  defp calculate_recency_score(prompt) do
    ...
    case days_old do
      days when days <= 7 -> 1.0    # Very recent
      days when days <= 30 -> 0.8   # Recent
      days when days <= 90 -> 0.6   # Moderately recent
      days when days <= 365 -> 0.4  # Within a year
      _ -> 0.2                      # Older
    end
  end
  ...
  defp calculate_user_preference_score(prompt, user_id) do
    ...
    case prompt.prompt_type do
      :user -> 1.0    # User's own prompts get preference
      :project -> 0.7 # Project prompts get moderate preference
      :system -> 0.5  # System prompts get standard score
      _ -> 0.3
    end
  end
  ...
  defp calculate_criterion_match_score(prompt, criterion, value) do
    ...
    case criterion do
      ...
      :text ->
        if String.contains?(String.downcase(prompt.content), String.downcase(value)), do: 0.8, else: 0.0
