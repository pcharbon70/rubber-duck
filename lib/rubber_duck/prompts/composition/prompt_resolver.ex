defmodule RubberDuck.Prompts.Composition.PromptResolver do
  ...
  def resolve_hierarchy(prompt_name, context, options \\ %{}) do
    ...
    with {:ok, hierarchy_key} <- build_hierarchy_cache_key(prompt_name, context),
         {:ok, resolved_prompts} <- resolve_with_cache_strategy(hierarchy_key, prompt_name, context, merged_options),
         {:ok, validated_hierarchy} <- validate_hierarchy_completeness(resolved_prompts, context, merged_options) do
  ...
  def resolve_batch(prompt_names, context, options \\ %{}) do
    ...
    case get_batch_from_cache(batch_cache_key, merged_options) do
      ...
      {:error, :cache_miss} ->
        execute_batch_resolution(prompt_names, context, merged_options, batch_cache_key, batch_start_time)
    end
  end
  ...
  defp resolve_and_cache_hierarchy(hierarchy_key, prompt_name, context, options) do
    case resolve_hierarchy_from_database(prompt_name, context, options) do
      {:ok, resolved_prompts} ->
        ...
        case CompositionCache.put_hierarchy(hierarchy_key, resolved_prompts, options.cache_ttl_seconds) do
          :ok ->
            Logger.debug("PromptResolver: Hierarchy cached for future use", prompt_name: prompt_name)
            {:ok, resolved_prompts}
          ...
          {:error, cache_error} ->
            ...
            {:ok, resolved_prompts}  # Continue even if caching fails
        end
  ...
  defp resolve_hierarchy_from_database(prompt_name, context, options) do
    ...
    resolved_prompts = case resolve_single_prompt(prompt_name, :system, Map.merge(context, %{tenant_id: tenant_id}), options) do
      {:ok, system_prompt} -> [system_prompt | resolved_prompts]
      {:error, _} -> resolved_prompts
    end
    
    # Resolve project prompt if project context available
    resolved_prompts = if project_id do
      case resolve_single_prompt(prompt_name, :project, Map.merge(context, %{project_id: project_id}), options) do
    ...
    # Resolve user prompt if user context available
    resolved_prompts = if user_id do
      case resolve_single_prompt(prompt_name, :user, Map.merge(context, %{user_id: user_id}), options) do
    ...
    case resolved_prompts do
      ...
      prompts -> {:ok, Enum.reverse(prompts)}  # Return in priority order
    end
  end
  ...
  defp resolve_single_prompt(prompt_name, prompt_type, context, _options) do
    ...
    filter_params = case prompt_type do
  ...
  defp validate_hierarchy_completeness(resolved_prompts, context, options) do
    ...
    case resolved_prompts do
      ...
      prompts ->
        validated_prompts = Enum.filter(prompts, fn prompt ->
  ...
  defp validate_prompt_context_compatibility(prompt, context) do
    ...
    case prompt.prompt_type do
      :system -> true  # System prompts are always compatible
  ...
  defp build_batch_cache_key(prompt_names, context) do
    ...
    names_hash = :crypto.hash(:sha256, Enum.join(Enum.sort(prompt_names), ",")) 
