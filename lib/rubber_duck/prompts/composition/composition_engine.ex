defmodule RubberDuck.Prompts.Composition.CompositionEngine do
  ...
  def compose_prompt(prompt_name, context, options \\ %{}) do
    ...
    with {:ok, resolved_prompts} <- resolve_hierarchical_prompts(prompt_name, context, merged_options),
         {:ok, composed_content} <- execute_composition_strategy(resolved_prompts, merged_options),
         {:ok, interpolated_content} <- interpolate_variables(composed_content, context, merged_options),
         {:ok, optimized_content} <- optimize_composition(interpolated_content, merged_options),
         {:ok, validated_content} <- validate_composition_security(optimized_content, merged_options) do
  ...
  def compose_prompts_batch(prompt_requests, context, options \\ %{}) do
    ...
    results = Enum.map(prompt_requests, fn %{name: name, variables: variables} ->
  ...
  defp execute_hierarchical_merge(resolved_prompts) do
    ...
    composed_content = resolved_prompts
  ...
  defp execute_priority_override(resolved_prompts) do
    ...
    case Enum.sort_by(resolved_prompts, fn prompt -> prompt.priority end, :desc) do
      ...
      [highest_priority | rest] ->
        ...
        overridden_content = Enum.reduce(rest, base_content, fn prompt, acc_content ->
  ...
  defp attempt_fallback_composition(prompt_name, context, options, _original_error) do
    if options.fallback_to_system do
      Logger.info("CompositionEngine: Attempting system prompt fallback", prompt_name: prompt_name)
  ...
  defp build_inheritance_chain(resolved_prompts) do
    ...
    inheritance_chain = resolved_prompts
  ...
  defp build_chain_from_root(prompt, prompts_by_id) do
    children = prompts_by_id
               |> Map.values()
               |> Enum.filter(fn p -> p.parent_id == prompt.id end)
    
    case children do
      [] -> [prompt]
      _ -> [prompt | Enum.flat_map(children, fn child -> build_chain_from_root(child, prompts_by_id) end)]
    end
  end
