defmodule RubberDuck.Workflows.Actions.CreateAgentTemplateAction do
  ...
  use Jido.Action,
    ...
    schema: [
      source_data: [
        type: :map,
        required: true,
        doc: "Source data for template generation (agent patterns, performance data, etc.)"
      ],
      template_type: [
        type: :atom,
        required: true,
        doc:
          "Type of template to generate (:performance, :error_recovery, :coordination, :lifecycle)"
      ],
      generation_strategy: [
        type: :atom,
        default: :pattern_based,
        doc:
          "Template generation strategy (:pattern_based, :performance_based, :hybrid, :adaptive)"
      ],
      ...
      customization_options: [
        type: :map,
        default: %{},
        doc: "Template customization and parameterization options"
      ]
    ]
  ...
  def run(params, context) do
    ...
    merged_validation_requirements =
      Map.merge(@default_validation_requirements, validation_requirements)

    merged_customization_options =
      Map.merge(@default_customization_options, customization_options)
    ...
    with {:ok, validated_params} <-
           validate_template_generation_params(
             source_data,
             template_type,
             strategy,
         ...
         {:ok, template_specification} <-
           generate_template_specification(
             pattern_analysis,
             template_type,
             strategy,
             merged_customization_options
  ...
  defp validate_template_type(template_type) when template_type in @supported_template_types,
    do: :ok

  defp validate_template_type(_), do: {:error, :unsupported_template_type}

  defp validate_generation_strategy(strategy) when strategy in @supported_generation_strategies,
    do: :ok
  ...
  defp generate_template_specification(
         pattern_analysis,
         template_type,
         strategy,
         customization_options
    ...
    template_spec = %{
      ...
      customization_config:
        build_customization_configuration(pattern_analysis, customization_options),
  ...
  defp validate_generated_template(generated_template, validation_requirements, context) do
    validation_results = %{
      template_quality_validation:
        validate_template_quality(generated_template, validation_requirements),
      performance_impact_validation:
        validate_template_performance_impact(generated_template, validation_requirements),
      reusability_validation:
        validate_template_reusability(generated_template, validation_requirements),
      compliance_validation:
        validate_template_compliance(generated_template, validation_requirements, context),
      effectiveness_prediction:
        validate_template_effectiveness_prediction(generated_template, validation_requirements),
  ...
  defp calculate_pattern_richness(source_data) do
    ...
    pattern_count =
      case Map.get(source_data, :patterns, []) do
  ...
  defp calculate_sample_size_adequacy(source_data) do
    ...
    sample_size =
      case Map.get(source_data, :sample_count, 0) do
  ...
  defp assess_template_performance_impact(_generated_template),
    do: %{acceptable_impact: true, impact_score: 0.9}

  defp assess_template_reusability(_generated_template),
    do: %{highly_reusable: true, reusability_score: 0.88}

  defp assess_template_compliance(_generated_template),
    do: %{compliant: true, compliance_score: 0.92}

  defp validate_effectiveness_prediction(_generated_template),
    do: %{prediction_reliable: true, prediction_confidence: 0.8}
