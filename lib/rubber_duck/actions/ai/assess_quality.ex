defmodule RubberDuck.Actions.AI.AssessQuality do
  ...
  use Jido.Action,
    ...
    schema: [
      ...
      criteria: [
        type: {:list, :atom},
        default: [:accuracy, :relevance, :completeness, :actionability]
      ]
    ]
  ...
  defp calculate_coverage(result) do
    ...
    covered_aspects =
      if result[:suggestions], do: [:suggestions | covered_aspects], else: covered_aspects
  ...
  defp calculate_single_trend(current, historical_values) do
    if length(historical_values) > 0 do
      avg_historical =
        Enum.sum(Enum.map(historical_values, & &1.score)) / length(historical_values)
  ...
  defp derive_quality_insights(assessment, comparison) do
    ...
    high_performers =
      Enum.filter(assessment, fn {_, v} ->
        v[:score] && v.score > 0.8
      end)

    insights =
      if length(high_performers) > 0 do
        [
          "Strong performance in: #{Enum.map_join(high_performers, ", ", &elem(&1, 0))}"
          | insights
        ]
      else
        insights
  ...
  defp calculate_consistency(assessment, historical) do
    ...
    if length(historical_scores) > 0 do
      ...
      variance =
        Enum.reduce(historical_scores, 0, fn score, acc ->
