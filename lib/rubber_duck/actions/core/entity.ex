defmodule RubberDuck.Actions.Core.Entity do
  ...
  defstruct [
    :id,
    :type,
    :resource,      # Original Ash resource or external data
    :version,
    :metadata,      # Runtime metadata
    :impact_cache,  # Cached impact calculations
    :dependencies,  # Loaded dependencies
    :snapshot,      # Pre-change snapshot for rollback
    :source         # Where this came from (:ash, :external, :cache)
  ]
  ...
  def load_dependencies(%__MODULE__{} = entity, depth) when depth > 0 do
    dependencies = case entity.type do
  ...
  def to_map(%__MODULE__{} = entity) do
    base_map = case entity.source do
  ...
  defp load_user_dependencies(user_resource) do
    %{
      sessions: [],  # Would load actual sessions
      preferences: Map.get(user_resource, :preferences, %{}),
      projects: []   # Would load user's projects
    }
  end
  
  defp load_project_dependencies(project_resource) do
    %{
      code_files: [],  # Would load project files
      analyses: [],    # Would load project analyses
      collaborators: [] # Would load collaborators
    }
  end
  ...
  defp load_code_file_dependencies(file_resource) do
    %{
      ...
      imports: [],  # Would analyze imports
      tests: []     # Would find related tests
    }
  end
