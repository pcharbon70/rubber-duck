defmodule RubberDuck.Actions.Core.UpdateEntity.Learner do
  ...
  defp calculate_error_rate_change(_entity, execution_result) do
    ...
    if execution_result[:errors] do
      ...
      -0.02  # Assume slight improvement if no errors
    end
  end
  ...
  defp calculate_resource_change(_entity, execution_result) do
    ...
    memory_change = (execution_result[:memory_usage] || 0) / 1_000_000  # Convert to MB
    cpu_change = (execution_result[:cpu_usage] || 0) / 100  # Convert to percentage
  ...
  def analyze_patterns(entity, impact_assessment, execution_result) do
    %{
      ...
      correlation_analysis: perform_correlation_analysis(entity, impact_assessment, execution_result)
    }
  end
  ...
  defp calculate_accuracy(predicted, actual) when predicted == 0, do: if(actual == 0, do: 1.0, else: 0.0)
  ...
  defp perform_correlation_analysis(entity, impact_assessment, execution_result) do
    %{
      ...
      complexity_success_correlation: correlate_complexity_with_success(impact_assessment, execution_result),
  ...
  defp measure_risk_accuracy(impact_assessment, execution_result) do
    ...
    true_positives = Enum.count(predicted_risks, fn {risk, _} -> 
    ...
    precision = if length(predicted_risks) > 0 do
      true_positives / length(predicted_risks)
    else
      1.0
    end
    
    recall = if length(materialized_risks) > 0 do
  ...
  defp measure_performance_accuracy(impact_assessment, execution_result) do
    predicted_latency = parse_latency(impact_assessment[:performance_impact][:expected_latency_change])
  ...
  defp measure_dependency_accuracy(impact_assessment, execution_result) do
    ...
    correct_predictions = Enum.count(predicted_affected, fn entity ->
    ...
    %{
      ...
      accuracy_percentage: if length(predicted_affected) > 0 do
  ...
  defp calculate_feedback_score(params, context) do
    ...
    base_score = case context[:user_feedback][:satisfaction] do
    ...
    base_score = if execution_result[:execution_time] && execution_result[:execution_time] < 100 do
      base_score + 0.1
    else
      base_score
    end
  ...
  defp determine_prediction_updates(entity, execution_result) do
    ...
    updates = if execution_result[:prediction_error] && execution_result[:prediction_error] > 0.2 do
    ...
    updates = if execution_result[:new_patterns] do
  ...
  defp determine_risk_updates(execution_result) do
    ...
    updates = if execution_result[:unexpected_risks] do
    ...
    updates = if execution_result[:risk_overestimated] do
  ...
  defp determine_performance_updates(execution_result) do
    ...
    updates = if execution_result[:performance_deviation] && execution_result[:performance_deviation] > 50 do
  ...
  defp determine_pattern_updates(_entity, execution_result) do
    ...
    updates = if execution_result[:pattern_frequency] && execution_result[:pattern_frequency] > 5 do
    ...
    updates = if execution_result[:pattern_misses] && execution_result[:pattern_misses] > 10 do
  ...
  def should_update_models?(learning_data) do
    ...
    has_sufficient_data and (needs_accuracy_improvement or has_new_patterns or has_recommended_updates)
  end
  ...
  defp calculate_confidence_score(learning_data) do
    ...
    confidence = if pattern_count > 5 do
  ...
  defp suggest_recovery_options(failure_data) do
    ...
    options = case failure_data.failure_category do
  ...
  defp generate_failure_recommendations(_failure_data, insights) do
    ...
    recommendations = ["Immediate recovery: #{inspect(hd(insights.recovery_options))}" | recommendations]
    
    # Add prevention recommendations
    recommendations = ["Prevention: #{hd(insights.prevention_strategies)}" | recommendations]
    
    # Add pattern-based recommendations
    recommendations = case insights.failure_pattern do
