defmodule RubberDuck.Actions.Core.UpdateEntity.Executor do
  ...
  def execute(params, context) do
    ...
    with {:ok, snapshot} <- create_snapshot(entity),
         ...
         {:ok, updated_entity} <- apply_changes(versioned_entity, validated_changes, impact_assessment),
  ...
  defp verify_constraints_satisfied(_entity, validated_changes) do
    ...
    constraints = validated_changes[:validations][:constraint_validation][:constraints_checked] || 0
