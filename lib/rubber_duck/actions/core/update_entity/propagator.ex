defmodule RubberDuck.Actions.Core.UpdateEntity.Propagator do
  ...
  defp determine_propagation_order(affected_entities, source_entity) do
    # Group by priority
    priority_groups =
      Enum.group_by(affected_entities, fn entity ->
  ...
  defp generate_validation_warnings(plan) do
    ...
    warnings =
      if plan.circular_dependencies.has_circular_dependencies do
        [
          "Circular dependencies detected: #{inspect(plan.circular_dependencies.cycles)}"
          | warnings
        ]
      else
        warnings
    ...
    warnings =
      if length(plan.affected_entities) > 50 do
  ...
  defp generate_validation_recommendations(plan) do
    ...
    recommendations =
      if length(plan.affected_entities) > 20 and
  ...
  def execute_propagation(plan, context) do
    ...
    execution_result =
      case plan.propagation_strategy.type do
        :sequential ->
  ...
  defp execute_sequential_with_validation(plan, context) do
    ...
    final_results =
      Enum.reduce_while(plan.propagation_order, results, fn entity, acc ->
      ...
      |> Enum.reduce_while({results, checkpoints}, fn {entity, index}, {res, checks} ->
        case propagate_to_entity(entity, plan.source_entity, context) do
          {:ok, _} = success ->
            ...
            new_checkpoints =
              if rem(index + 1, checkpoint_frequency) == 0 do
  ...
  def resolve_circular_dependencies(plan) do
    if plan.circular_dependencies.has_circular_dependencies do
      ...
      resolved_plan =
        Enum.reduce(cycles, plan, fn cycle, acc ->
  ...
  defp break_cycle(plan, cycle) do
    ...
    updated_order =
      Enum.reject(plan.propagation_order, fn entity ->
