defmodule RubberDuck.Preferences.Security.SecurityMonitor do
  ...
  @monitoring_window 300_000  # 5 minutes
  @alert_thresholds %{
  ...
  defp track_user_activity(state, event) do
    ...
    user_activity = Map.get(state.user_activity, user_id, %{
      access_events: [],
      change_events: [],
      last_activity: current_time
    })
    
    updated_activity = case event.action do
  ...
  defp check_access_anomalies(state, event) do
    ...
    if is_sensitive_preference?(event.preference_key) do
  ...
  defp check_change_anomalies(state, event) do
    ...
    sensitive_changes = Enum.filter(recent_changes, fn change_event ->
      is_sensitive_preference?(change_event.preference_key)
    end)
  ...
  defp check_sensitive_access_patterns(user_activity, event, thresholds) do
    sensitive_access = Enum.filter(user_activity.access_events, fn access_event ->
      is_sensitive_preference?(access_event.preference_key)
    end)
  ...
  defp is_sensitive_preference?(preference_key) do
  ...
  defp trigger_security_alert(alert_data) do
    ...
    # Log to audit system
    AuditLogger.log_security_event(Map.merge(alert_data, %{
  ...
  defp cleanup_old_activity(state) do
    cutoff_time = DateTime.add(DateTime.utc_now(), -24 * 3600, :second)  # 24 hours
    
    cleaned_activity = Enum.reduce(state.user_activity, %{}, fn {user_id, activity}, acc ->
  ...
  defp schedule_cleanup do
    Process.send_after(self(), :cleanup_activity, 3_600_000)  # 1 hour
  end
end