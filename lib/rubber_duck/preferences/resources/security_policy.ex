defmodule RubberDuck.Preferences.Resources.SecurityPolicy do
  ...
  attributes do
    ...
    attribute :policy_type, :atom do
      ...
      constraints one_of: [:access_control, :approval_required, :encryption_required, :audit_required]
      allow_nil? false
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :policy_name, :policy_type, :resource_type, :preference_pattern,
        :required_permissions, :required_roles, :approval_required, :approval_roles,
        :encryption_required, :audit_level, :conditions, :priority, :active, :description
      ]
    ...
    update :update do
      ...
      accept [
        :policy_name, :preference_pattern, :required_permissions, :required_roles,
        :approval_required, :approval_roles, :encryption_required, :audit_level,
        :conditions, :priority, :active, :description
      ]
  ...
  calculations do
    ...
    calculate :effective_permissions, :map, expr(%{
  ...
  defp validate_policy_consistency(changeset) do
    ...
    if approval_required and Enum.empty?(approval_roles) do
      Ash.Changeset.add_error(changeset, :approval_roles, "Approval roles required when approval is enabled")
    else
      changeset
    end
  end
  ...
  defp validate_approval_roles_consistency(changeset) do
    ...
    invalid_approvers = Enum.filter(approval_roles, fn approval_role ->
      Enum.any?(required_roles, fn required_role ->
        get_role_level(approval_role, role_hierarchy) < get_role_level(required_role, role_hierarchy)
      end)
    end)
    ...
    if Enum.empty?(invalid_approvers) do
      ...
      Ash.Changeset.add_error(changeset, :approval_roles, 
  ...
  defp validate_encryption_pattern_consistency(changeset) do
    ...
    if encryption_required and pattern do
      if RubberDuck.Preferences.Security.EncryptionManager.sensitive_preference?(pattern) do
        ...
        Ash.Changeset.add_error(changeset, :preference_pattern,
