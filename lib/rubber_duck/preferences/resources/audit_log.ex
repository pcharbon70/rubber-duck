defmodule RubberDuck.Preferences.Resources.AuditLog do
  ...
  attributes do
    ...
    attribute :ip_address, :string do
      ...
      constraints max_length: 45  # IPv6 length
      allow_nil? true
    end
  ...
  actions do
    ...
    create :create do
      ...
      accept [
        :event_type, :event_data, :user_id, :preference_key, :action,
        :ip_address, :user_agent, :session_id, :event_id, :severity, :metadata
      ]
  ...
  defp determine_security_severity(%{threat_level: level}) when level in ["high", "critical"], do: :critical
