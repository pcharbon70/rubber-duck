defmodule RubberDuck.Skills.UserManagementSkill do
  ...
  def handle_signal(%{type: "user.session.end"} = signal, state) do
    ...
    case get_in(state, [:users, user_id, :current_session]) do
      ...
      session ->
        ...
        updated_state = if state.opts.behavior_learning_enabled do
        ...
        updated_state = update_in(updated_state, [:users, user_id, :session_history], fn history ->
  ...
  def handle_signal(%{type: "user.behavior.track"} = signal, state) do
    ...
    if state.opts.behavior_learning_enabled do
      ...
      updated_state = update_in(state, [:users, user_id, :behavior_history], fn history ->
        [behavior | Enum.take(history || [], state.opts.prediction_window - 1)]
      end)

      # Learn patterns if we have enough data
      updated_state = if length(get_in(updated_state, [:users, user_id, :behavior_history]) || []) >= 10 do
  ...
  def handle_signal(%{type: "user.preference.update"} = signal, state) do
    ...
    updated_state = update_in(state, [:users, user_id, :preferences], fn prefs ->
      current_prefs = prefs || %{}

      # Apply adaptation rate if preference exists
      new_value = if Map.has_key?(current_prefs, preference_key) do
        adapt_preference(current_prefs[preference_key], preference_value, state.opts.adaptation_rate)
      else
        preference_value
      end
    ...
    updated_state = update_in(updated_state, [:users, user_id, :preference_history], fn history ->
      change = %{
        key: preference_key,
  ...
  def handle_signal(%{type: "user.pattern.detect"} = signal, state) do
    ...
    confident_patterns = Enum.filter(patterns, fn pattern ->
  ...
  def handle_signal(%{type: "user.interaction.complete"} = signal, state) do
    ...
    updated_state = update_in(updated_state, [:users, user_id, :stats], fn stats ->
  ...
  defp detect_behavioral_patterns(behaviors) do
    ...
    patterns = Enum.flat_map(grouped, fn {action, instances} ->
  ...
  defp predict_next_actions(state, user_id) do
    ...
    if length(recent_behaviors) >= 2 do
      recent_actions = recent_behaviors
        |> Enum.take(2)
        |> Enum.map(& &1.action)
        |> Enum.reverse()

      # Find matching sequence patterns
      matching_sequences = patterns
  ...
  defp generate_proactive_suggestions(patterns, _user_state) do
    ...
    recurring_suggestions = patterns
  ...
  defp adapt_preference(old_value, new_value, adaptation_rate) when is_number(old_value) and is_number(new_value) do
  ...
  defp merge_patterns(existing, new_patterns, adaptation_rate) do
    ...
    existing_map = Map.new(existing, fn pattern ->
    ...
    |> Enum.reduce(existing_map, fn new_pattern, acc ->
      ...
      Map.update(acc, key, new_pattern, fn existing_pattern ->
        ...
        %{existing_pattern |
          confidence: adapt_preference(existing_pattern.confidence, new_pattern.confidence, adaptation_rate),
  ...
  defp pattern_key(%{type: :interaction_pattern, interaction_type: type}), do: {:interaction, type}
