defmodule RubberDuck.Skills.ProviderSelectionSkill do
    ...
    case UniversalProviderService.get_available_providers(domain) do
      {:ok, available_providers} ->
        ...
        recommendation_result = %{
          ...
          analysis_metadata: if(include_detailed_analysis, do: generate_analysis_metadata(requirements, state), else: %{}),
  ...
  defp apply_provider_optimization(available_providers, requirements, criteria, state) do
    ...
    |> Enum.map(fn {provider_type, provider_info} ->
      score_provider_comprehensively(provider_type, provider_info, requirements, criteria, state)
    end)
    |> Enum.sort_by(& &1.overall_score, :desc)
  ...
  defp score_provider_comprehensively(provider_type, provider_info, requirements, criteria, state) do
    ...
    quality_prediction_score = calculate_quality_prediction_score(provider_type, requirements, state)
    ...
    weighted_score = apply_optimization_weights(
      %{
    ...
    %{
      ...
      optimization_reasoning: generate_optimization_reasoning(provider_type, weighted_score, criteria)
    }
  end
  ...
  defp calculate_capability_match_score(provider_info, requirements) do
    ...
    feature_matches = requirements.specialized_features
    |> Enum.map(fn feature ->
      if feature in Map.get(provider_info, :specializations, []), do: 1.0, else: 0.3
    end)
    
    feature_score = case feature_matches do
  ...
  defp calculate_cost_efficiency_score(provider_type, requirements, state) do
    ...
    base_cost_score = if estimated_cost <= max_acceptable_cost do
      1.0 - (estimated_cost / max_acceptable_cost) * 0.7  # Better cost = higher score
    else
      0.2  # Cost too high
    end
  ...
  defp calculate_quality_prediction_score(provider_type, requirements, state) do
    ...
    base_quality = case provider_type do
      :anthropic -> 0.9   # High quality with Constitutional AI
      :openai -> 0.85     # Good general quality
      :ollama -> 0.75     # Decent quality for local
      _ -> 0.8
    end
    
    # Adjust for domain specialization
    domain_bonus = case {provider_type, requirements.domain} do
      {:anthropic, :evaluation} -> 0.05  # Anthropic excels at evaluation
      {:openai, :orchestration} -> 0.05  # OpenAI good for orchestration
      _ -> 0.0
    end
  ...
  defp calculate_performance_score(provider_type, requirements, state) do
    ...
    base_performance = case provider_type do
      :ollama -> 0.9      # Fast local models
      :openai -> 0.8      # Generally fast
      :anthropic -> 0.7   # Slower but thorough
      _ -> 0.75
    end
    ...
    if performance_requirement < 3000 and provider_type == :anthropic do
      base_performance * 0.8  # Penalize Anthropic for speed requirements
    else
      base_performance
    end
  end
  ...
  defp calculate_reliability_score(provider_type, state) do
    ...
    base_reliability = case provider_type do
      :openai -> 0.95
      :anthropic -> 0.92
      :ollama -> 0.85
      _ -> 0.9
  ...
  defp apply_learning_adjustment(provider_type, state) do
    ...
    case learning_data do
      ...
      data ->
        ...
        improvement_rate * confidence * 0.1  # Small but meaningful adjustment
    end
  end
  
  defp apply_optimization_weights(scores, criteria) do
    # Apply optimization criteria weights to scores
    weights = Map.get(criteria, :weights, %{
      capability: 0.25,
  ...
  defp estimate_token_requirements(request) do
    ...
    base_tokens + 300  # Add overhead for prompts and response
  end
  ...
  defp analyze_coordination_complexity(request) do
    ...
    complexity_score = coordination_indicators
  ...
  defp get_historical_quality_adjustment(provider_type, state) do
    improvement_rate = get_in(state, [:provider_learning, provider_type, :improvement_rate]) || 0.0
    improvement_rate * 0.1  # Convert to quality adjustment
  end
  ...
  defp generate_optimization_recommendations(enhanced_analysis) do
    ...
    recommendations = if enhanced_analysis.cost_constraints.cost_priority == :high do
    ...
    recommendations = if enhanced_analysis.quality_requirements.constitutional_ai_required do
    ...
    recommendations = if enhanced_analysis.performance_requirements.max_response_time < 3000 do
  ...
  defp assess_anthropic_suitability(analysis) do
    score = 0.8  # Base suitability
    
    # Boost for Constitutional AI requirements
    score = if analysis.quality_requirements.constitutional_ai_required do
      score + 0.15
    else
      score
    end
    ...
    score = if analysis.cost_constraints.cost_priority == :high do
      score - 0.1
    else
      score
    end
  ...
  defp assess_openai_suitability(analysis) do
    score = 0.85  # Base suitability
    
    # Boost for performance requirements
    score = if analysis.performance_requirements.performance_priority == :high do
      score + 0.1
    else
      score
    end
  ...
  defp assess_ollama_suitability(analysis) do
    ...
    score = if analysis.performance_requirements.max_response_time < 3000 do
      score + 0.15
    else
      score
    end
  ...
  defp generate_provider_recommendation(provider_type, provider_info, requirements, state) do
    suitability_score = case provider_type do
    ...
    %{
      ...
      cost_estimate: estimate_provider_cost(provider_type, requirements),
      quality_prediction: predict_provider_quality(provider_type, requirements, state),
      recommendation_reasoning: generate_provider_recommendation_reasoning(provider_type, suitability_score)
    }
  end
  ...
  defp generate_provider_recommendation_reasoning(provider_type, score) do
    quality_description = case score do
