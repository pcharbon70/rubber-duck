defmodule RubberDuck.Skills.ProjectManagementSkill do
  ...
  use Jido.Skill,
    ...
    opts_schema: [
      ...
      optimization_level: [type: :atom, default: :moderate, values: [:minimal, :moderate, :aggressive]]
    ]
  ...
  def handle_signal(%{type: "project.updated"} = signal, state) do
    ...
    case get_in(state, [:projects, project_id]) do
      ...
      project_state ->
        ...
        suggestions = if state.opts.auto_refactor_suggestions do
  ...
  def handle_signal(%{type: "project.file.added"} = signal, state) do
    ...
    updated_state = update_in(state, [:projects, project_id], fn project ->
      if project do
        project
  ...
  def handle_signal(%{type: "project.file.modified"} = signal, state) do
    ...
    optimizations = if project && state.opts.optimization_level != :minimal do
  ...
  def handle_signal(%{type: "project.dependency.check"} = signal, state) do
    ...
    case get_in(state, [:projects, project_id]) do
      ...
      project ->
        ...
        alerts = []
        alerts = if length(deps_result.vulnerable) > 0 do
          [%{type: :vulnerable_dependencies, count: length(deps_result.vulnerable)} | alerts]
        else
          alerts
        end

        alerts = if length(deps_result.outdated) > 0 do
    ...
    case get_in(state, [:projects, project_id]) do
      ...
      project ->
        ...
        updated_project = project
          ...
          |> put_in([:monitoring, :trend], determine_quality_trend(project, quality_results.metrics))
  ...
  defp check_quality_thresholds(project, opts) do
    ...
    alerts = if metrics.score < opts.quality_threshold do
      [%{type: :low_quality, value: metrics.score, threshold: opts.quality_threshold} | alerts]
    else
      alerts
    end

    alerts = if metrics.complexity > opts.max_complexity do
      [%{type: :high_complexity, value: metrics.complexity, threshold: opts.max_complexity} | alerts]
    else
      alerts
    end

    alerts = if metrics.test_coverage < opts.min_test_coverage do
      [%{type: :low_test_coverage, value: metrics.test_coverage, threshold: opts.min_test_coverage} | alerts]
    else
      alerts
    end
  ...
  defp generate_refactoring_suggestions(project, opts) do
    ...
    suggestions = if project.quality_metrics.complexity > opts.max_complexity do
      [%{
    ...
    suggestions = if project.quality_metrics.test_coverage < opts.min_test_coverage do
      [%{
    ...
    suggestions = if project.quality_metrics.technical_debt > 100 do
      [%{
  ...
  defp analyze_change_impact(old_content, new_content) do
    ...
    %{
      ...
      complexity_change: estimate_file_complexity(new_content) - estimate_file_complexity(old_content)
    }
  end
  ...
  defp suggest_optimizations(_project, file_path, content, level) do
    ...
    # Basic optimizations
    suggestions = if String.contains?(content, "Enum.map") && String.contains?(content, "Enum.filter") do
      [%{
        type: :combine_enum_operations,
        file: file_path,
        description: "Combine Enum.map and Enum.filter into Enum.flat_map or for comprehension"
      } | suggestions]
    else
      suggestions
    end
    ...
    suggestions = if level in [:moderate, :aggressive] do
    ...
    suggestions = if level == :aggressive do
  ...
  defp check_quality_violations(metrics, opts) do
    ...
    violations = if metrics.score < opts.quality_threshold do
      [%{metric: :score, value: metrics.score, threshold: opts.quality_threshold} | violations]
    else
      violations
    end

    violations = if metrics.complexity > opts.max_complexity do
      [%{metric: :complexity, value: metrics.complexity, threshold: opts.max_complexity} | violations]
    else
      violations
    end
  ...
  defp rank_refactoring_suggestions(suggestions) do
    ...
    Enum.sort_by(suggestions, fn s ->
      priority_score = case s.priority do
        :high -> 3
        :medium -> 2
        :low -> 1
      end
  ...
  defp estimate_file_complexity(content) do
    ...
    conditionals = Enum.count(lines, fn line ->
