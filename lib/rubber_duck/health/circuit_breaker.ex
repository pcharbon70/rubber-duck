defmodule RubberDuck.Health.CircuitBreaker do
  ...
  @default_config %{
    error_threshold: 3,        # Fewer failures for health checks
    timeout: 60_000,           # 1 minute recovery
  ...
  @check_types [:database, :memory, :processes, :atoms, :ash_authentication, :repo_pool, :external_service]
  ...
  defp get_check_config(check_type) do
    ...
    check_configs = %{
      ...
      memory: %{error_threshold: 10, timeout: 30_000},        # System resource, lenient
      processes: %{error_threshold: 10, timeout: 30_000},     # System resource, lenient
      atoms: %{error_threshold: 10, timeout: 30_000},         # System resource, lenient
