defmodule RubberDuck.Health.ProtectedHealthCheck do
  ...
  defp perform_protected_health_check(state) do
    ...
    checks = if state.config.enable_circuit_breaker do
    ...
    new_consecutive_failures = if status == :unhealthy do
    ...
    new_degraded_mode = if new_consecutive_failures >= 3 and not state.degraded_mode do
      Logger.warning("Auto-enabling degraded mode after #{new_consecutive_failures} consecutive failures")
      true
    else
      state.degraded_mode
    end
  ...
  defp perform_checks_with_circuit_breaker(state) do
    ...
    other_results = if state.degraded_mode do
      skip_non_priority_checks(check_funs, state.config.priority_checks, state.config.skip_on_degraded)
    else
  ...
  defp adjust_check_interval(state) do
    cond do
      ...
      many_circuits_open?() ->
        %{state | check_interval: min(state.check_interval * 2, 300_000)} # Max 5 minutes
  ...
  defp enhance_with_circuit_info(state) do
    ...
    enhanced_checks = 
      ...
      |> Enum.map(fn {check_type, check_result} ->
        ...
        enhanced_result = Map.merge(check_result, %{
