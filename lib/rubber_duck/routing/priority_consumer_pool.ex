defmodule RubberDuck.Routing.PriorityConsumerPool do
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, state) do
    ...
    if priority do
      ...
      new_stats = Map.update!(state.stats, priority, fn pstats ->
  ...
  defp process_message_chunk(priority, messages, _metadata, state) do
    ...
    tasks = Enum.map(messages, fn {message, context} ->
    ...
    # Track active tasks
    new_active_tasks = Enum.reduce(tasks, state.active_tasks, fn task, acc ->
    ...
    final_active_tasks = Enum.reduce(tasks, new_active_tasks, fn task, acc ->
  ...
  defp partition_results(results) do
    Enum.reduce(results, {0, 0}, fn result, {success, failure} ->
      case result do
        ...
        {_task, nil} -> {success, failure + 1}  # Timeout
      end
    end)
  end
