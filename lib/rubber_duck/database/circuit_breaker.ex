defmodule RubberDuck.Database.CircuitBreaker do
  ...
  @default_config %{
    error_threshold: 5,        # More failures allowed for DB
    ...
    slow_query_threshold: 5000 # 5 seconds = slow query
  }
  ...
  def with_circuit_breaker(operation_type, fun, opts \\ []) when operation_type in @operation_types do
  ...
  def execute_ash_query(resource, query_or_changeset, opts \\ []) do
    ...
    with_circuit_breaker(operation_type, fn ->
      case operation_type do
        :read -> Ash.read(resource, query_or_changeset, opts)
  ...
  defp get_operation_config(operation_type) do
    ...
    operation_configs = %{
      read: %{error_threshold: 10, timeout: 20_000},      # More lenient for reads
      write: %{error_threshold: 5, timeout: 30_000},      # Standard for writes
      transaction: %{error_threshold: 3, timeout: 60_000}, # Strict for transactions
      bulk: %{error_threshold: 5, timeout: 45_000}        # Balanced for bulk ops
    }
  ...
  defp handle_database_error(operation_type, %Postgrex.Error{} = error) do
    ...
    case error do
      ...
      %{postgres: %{code: :admin_shutdown}} ->
        ...
        force_open_circuit(operation_type, :maintenance, 300_000) # 5 minutes
