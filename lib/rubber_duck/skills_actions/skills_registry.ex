defmodule RubberDuck.SkillsActions.SkillsRegistry do
  ...
  def handle_call({:register_skill, skill_module, metadata}, _from, state) do
    ...
    case extract_skill_metadata(skill_module, metadata) do
      {:ok, skill_metadata} ->
        ...
        updated_dependency_graph = update_dependency_graph(state.dependency_graph, skill_module, skill_metadata)
        ...
        Phoenix.PubSub.broadcast(RubberDuck.PubSub, "skills_registry_events", 
  ...
  def handle_call({:discover_skills, capability_requirements}, _from, state) do
    ...
    case Map.get(state.capabilities_cache, cache_key) do
      nil ->
        ...
        matching_skills = perform_capability_matching(state.skills, capability_requirements, state.config)
        
        # Cache the result
        updated_cache = Map.put(state.capabilities_cache, cache_key, {matching_skills, DateTime.utc_now()})
  ...
  def handle_call(:get_all_skills, _from, state) do
    skills_with_capabilities = state.skills
    |> Enum.map(fn {skill_module, metadata} ->
      {skill_module, Map.put(metadata, :performance_metrics, 
  ...
  def handle_call({:get_capabilities, skill_module}, _from, state) do
    case Map.get(state.skills, skill_module) do
      ...
      skill_metadata ->
        ...
        enhanced_capabilities = Map.merge(capabilities, %{
  ...
  def handle_call({:resolve_dependencies, skill_modules}, _from, state) do
    dependency_resolution = resolve_skill_dependency_chain(skill_modules, state.dependency_graph, state.skills)
  ...
  def handle_cast(:auto_register_skills, state) do
    ...
    updated_state = Enum.reduce(@skill_modules, state, fn skill_module, acc_state ->
      case extract_skill_metadata(skill_module, %{auto_registered: true}) do
        {:ok, metadata} ->
          ...
          updated_dependency_graph = update_dependency_graph(acc_state.dependency_graph, skill_module, metadata)
    ...
    Logger.info("Auto-registration completed: #{map_size(updated_state.skills)} skills registered")
  ...
  def handle_info({:skill_performance_update, skill_module, performance_data}, state) do
    ...
    {:noreply, %{state | performance_metrics: updated_performance, capabilities_cache: updated_cache}}
  end
  ...
  defp analyze_skill_dependencies(skill_module) do
    ...
    %{
      required_skills: [],  # Would analyze actual dependencies
      optional_skills: [],
  ...
  defp get_skill_integration_points(skill_module) do
    ...
    %{
      jido_framework: true,  # All skills use Jido
  ...
  defp perform_weighted_overlap_matching(skills, requirements) do
    ...
    |> Enum.filter(fn {_module, scores} -> scores.match_score > 0.3 end)  # Threshold for relevance
  ...
  defp perform_simple_matching(skills, requirements) do
    ...
    |> Enum.filter(fn {_skill_module, metadata} ->
      ...
      signal_overlap = MapSet.intersection(
  ...
  defp parse_llm_skill_recommendation(llm_content, available_skills) do
    case Jason.decode(llm_content) do
      {:ok, %{"recommended_skills" => skill_names, "reasoning" => reasoning}} ->
        ...
        recommended_modules = skill_names
  ...
  defp resolve_skill_dependency_chain(skill_modules, dependency_graph, skills) do
    ...
    all_dependencies = skill_modules
  ...
  defp detect_circular_dependencies(skill_modules, dependency_graph) do
    ...
    |> Enum.filter(fn skill ->
      ...
      skill in dependencies  # Self-dependency is circular
    end)
  end
  ...
  defp calculate_capability_overlap(required_capabilities, skill_capabilities) do
    if Enum.empty?(required_capabilities) do
      0.5  # Neutral score if no specific requirements
    else
      ...
      overlap_count = required_capabilities
  ...
  defp calculate_performance_match(performance_requirements, skill_capabilities) do
    ...
    if skill_speed <= required_speed do
      1.0 - (skill_speed / required_speed) * 0.5  # Better performance = higher score
    else
      0.3  # Performance requirement not met
    end
  end
  ...
  defp calculate_dependency_complexity(dependency_graph) do
    if map_size(dependency_graph) == 0 do
      ...
      total_dependencies = dependency_graph
      |> Map.values()
      |> Enum.map(&length/1)
      |> Enum.sum()
