defmodule RubberDuck.SkillsActions.Adapters.LlmProviderAdapter do
  ...
  def recommend_skills_for_context(context, agent_id, user_id \\ nil) do
    ...
    case UniversalProviderService.complete(recommendation_request, :orchestration, %{
      ...
      max_tokens: 800,
      temperature: 0.3  # Balanced creativity for recommendations
    }) do
      {:ok, llm_response} ->
  ...
  """
  def optimize_workflow_with_llm(workflow_definition, performance_history, optimization_goals \\ %{}) do
    Logger.debug("Optimizing workflow with LLM assistance")
    
    optimization_request = build_workflow_optimization_request(workflow_definition, performance_history, optimization_goals)
  ...
  """
  def coordinate_actions_with_llm(actions, coordination_strategy, execution_context, user_id \\ nil) do
    Logger.debug("Coordinating #{length(actions)} actions with LLM assistance")
    
    coordination_request = build_action_coordination_request(actions, coordination_strategy, execution_context)
  ...
  def estimate_execution_cost(skills_actions, execution_context, user_id \\ nil) do
    ...
    {:ok, %{
      ...
      estimation_accuracy: :high  # Would track actual vs estimated in production
    }}
  end
  ...
  defp parse_skill_recommendations_from_text(text) do
    ...
    skill_names = [
      "learning_skill", "threat_detection_skill", "project_management_skill",
      "authentication_skill", "code_analysis_skill", "policy_enforcement_skill",
      "query_optimization_skill", "token_management_skill", "user_management_skill"
    ]
    
    mentioned_skills = skill_names
    |> Enum.filter(&String.contains?(String.downcase(text), &1))
    
    {:ok, %{
      recommended_skills: mentioned_skills,
      reasoning: text,
      priority_order: mentioned_skills,  # Same order for text parsing
      alternative_options: []
    }}
  end
  ...
  defp validate_skill_recommendations(recommendations) do
    ...
    validated_recommendations = recommendations.recommended_skills
  ...
  defp fallback_to_registry_recommendations(context, agent_id) do
    ...
    case SkillsRegistry.discover_skills(capability_requirements) do
      {:ok, matching_skills} ->
        recommendations = matching_skills
  ...
  defp build_workflow_optimization_request(workflow_definition, performance_history, optimization_goals) do
    """
    ...
    case Jason.decode(llm_content) do
      {:ok, %{
        ...
        {:ok, %{
          ...
          overall_expected_improvement: improvement,
          reasoning: reasoning,
          constitutional_ai_assessment: Map.get(parsed, "constitutional_ai_assessment", "Assessment not provided")
        }}
  ...
  defp parse_optimizations_from_text(text) do
    ...
    optimizations = optimization_keywords
    |> Enum.filter(&String.contains?(String.downcase(text), &1))
    |> Enum.map(fn keyword ->
      %{
        category: "general",
  ...
  defp calculate_expected_improvement(optimizations) do
    ...
    |> Enum.sum()
    |> min(0.8)  # Cap at 80% improvement to be realistic
  end
  ...
  defp perform_llm_assisted_skill_execution(skill_module, params, execution_context, user_id) do
    ...
    skill_guidance_prompt = build_skill_execution_guidance_prompt(skill_module, params, execution_context)
  ...
  defp execute_skill_directly(skill_module, params, execution_context) do
    ...
    try do
      ...
      result = apply(skill_module, :execute, [params, skill_state])
  ...
  defp create_skills_streaming_callback(original_callback, skill_module, execution_context) do
    fn stream_event ->
      ...
      enhanced_event = case Map.get(stream_event, :type) do
        :chunk ->
          %{
  ...
  defp estimate_single_execution_cost(skill_action, execution_context, user_id) do
    case skill_action.type do
      ...
      _ ->
        {:ok, 0.0}  # Unknown type, assume no cost
    end
  end
    ...
    case SkillsRegistry.get_skill_capabilities(skill_module) do
      {:ok, capabilities} ->
        ...
        if Map.get(capabilities.integration_capabilities, :llm_integration, false) do
          ...
          {:ok, 0.0}  # No LLM cost for non-LLM skills
        end
      
      _ ->
        {:ok, 0.05}  # Default low cost estimate
    end
  end
  ...
  defp estimate_llm_task_cost(skill_action, execution_context, user_id) do
    ...
    estimated_tokens = div(String.length(task_description), 4) + 300  # Basic estimation
  ...
  defp extract_expected_outcomes(llm_guidance) do
    ...
    %{
      success_probability: 0.85,  # Would parse from guidance
      potential_issues: [],
