defmodule RubberDuck.Telemetry.MessageReporter do
  ...
  defp process_telemetry_event([:rubber_duck, :routing, :message, :stop], measurements, metadata, state) do
    duration_ms = measurements.duration / 1_000
    
    # Update totals
    state = %{state |
      total_routed: state.total_routed + 1,
      total_succeeded: if(metadata.success, do: state.total_succeeded + 1, else: state.total_succeeded),
      total_failed: if(not metadata.success, do: state.total_failed + 1, else: state.total_failed),
    ...
    updated_priority_stats = %{priority_stats |
      count: priority_stats.count + 1,
      total_duration_us: priority_stats.total_duration_us + measurements.duration
    }
    
    state = %{state | by_priority: Map.put(state.by_priority, metadata.priority, updated_priority_stats)}
  ...
  defp process_telemetry_event([:rubber_duck, :circuit_breaker, event], _measurements, metadata, state) do
    cb_event = %{
      event: event,
      message_type: metadata.message_type,
      timestamp: System.system_time(:second),
  ...
  defp compile_stats(state) do
    ...
    %{
      ...
      success_rate: if(state.total_routed > 0, do: state.total_succeeded / state.total_routed, else: 0),
      avg_duration_ms: if(state.total_routed > 0, do: state.total_duration_us / state.total_routed / 1_000, else: 0),
  ...
  defp get_top_types_by_volume(by_type, limit) do
    ...
    |> Enum.map(fn {type, stats} ->
      %{
        ...
        avg_duration_ms: if(stats.count > 0, do: stats.total_duration_us / stats.count / 1_000, else: 0)
      }
    end)
  end
  ...
  defp format_priority_stats(by_priority) do
    ...
    |> Enum.map(fn {priority, stats} ->
      avg_ms = if(stats.count > 0, do: Float.round(stats.total_duration_us / stats.count / 1_000, 2), else: 0)
