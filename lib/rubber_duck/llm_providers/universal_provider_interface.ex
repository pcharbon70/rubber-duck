defmodule RubberDuck.LlmProviders.UniversalProviderInterface do
  ...
  defp validate_domain(context) do
    case Map.get(context, :domain) do
      domain when domain in [:evaluation, :orchestration, :planning, :communication, :tooling] -> :ok
      _ -> {:error, "Invalid or missing domain in context"}
    end
  end
  ...
  """
  def standardize_universal_response(provider_response, provider_type, model, domain_context, metadata \\ %{}) do
    %{
  ...
  """
  def calculate_universal_cost_estimate(token_count, cost_per_1k_tokens, domain_context) when is_integer(token_count) and is_number(cost_per_1k_tokens) do
    base_cost = (token_count / 1000) * cost_per_1k_tokens
    
    # Apply domain-specific cost adjustments
    domain_multiplier = case domain_context.domain do
      :evaluation -> 1.0    # Base cost for evaluation
      :orchestration -> 0.9 # Slight discount for orchestration bulk usage
      :planning -> 1.1      # Slight premium for complex planning
      :communication -> 0.8 # Lower cost for simple communication
      _ -> 1.0
    end
  ...
  """
  def parse_universal_health_status(response_time_ms, success_rate, domain_health_data \\ %{}, error_count \\ 0) do
    status = cond do
  ...
  defp build_specialized_instructions(options) do
    ...
    instructions = if :constitutional_ai in specialized_features do
      ["Apply Constitutional AI safety principles throughout your analysis"] ++ instructions
    else
      instructions
    end
    
    instructions = if :cost_optimization in specialized_features do
      ["Consider cost-efficiency while maintaining quality in your response"] ++ instructions
    else
      instructions
    end
    
    instructions = if :safety_critical in specialized_features do
      ["This is a safety-critical evaluation - prioritize thorough analysis over speed"] ++ instructions
    else
      instructions
    end
  ...
  def supports_domain_requirements?(provider_capabilities, domain_requirements) do
    domain_supported = domain_requirements.domain in Map.get(provider_capabilities, :supported_domains, [])
    use_case_supported = domain_requirements.use_case in Map.get(provider_capabilities, :supported_use_cases, [])
    ...
    performance_met = case domain_requirements.performance_requirements do
    ...
    case target_domain do
      :evaluation ->
        ...
        %{
          ...
          reasoning: Map.get(universal_response.domain_specific_data, :reasoning, universal_response.content),
      ...
      :orchestration ->
        ...
        %{
          ...
          success: universal_response.success,
          cost_usd: universal_response.cost_usd,
          completion_quality: Map.get(universal_response.domain_specific_data, :completion_quality, 0.8),
      ...
      :planning ->
        ...
        %{
          content: universal_response.content,
          reasoning_quality: Map.get(universal_response.domain_specific_data, :reasoning_depth, 0.7),
  ...
  def aggregate_domain_health(domain_health_map) do
    if Enum.empty?(domain_health_map) do
      ...
      domain_scores = domain_health_map
      |> Map.values()
  ...
  defp calculate_domain_health_score(domain_health) when is_map(domain_health) do
    ...
    response_time_score = calculate_response_time_score(Map.get(domain_health, :avg_response_time_ms, 5000))
  ...
  defp calculate_response_time_score(response_time_ms) do
    cond do
      response_time_ms <= 1000 -> 1.0   # Excellent
      response_time_ms <= 3000 -> 0.8   # Good  
      response_time_ms <= 5000 -> 0.6   # Acceptable
      response_time_ms <= 10000 -> 0.4  # Poor
      true -> 0.2                       # Very poor
    end
  end
  ...
  defp validate_domain_configurations(config) do
    ...
    invalid_domains = domain_configs
    |> Map.keys()
    |> Enum.filter(&(&1 not in [:evaluation, :orchestration, :planning, :communication, :tooling]))
  ...
  defp validate_anthropic_universal_config(config) do
    ...
    models = Map.get(config, :models, %{})
    valid_models = ["claude-3-5-sonnet-20241022", "claude-3-haiku-20240307", "claude-3-opus-20240229"]
