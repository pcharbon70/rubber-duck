defmodule RubberDuck.LlmProviders.ProviderRouter do
  ...
  def select_provider(domain, requirements \\ %{}) do
    ...
    with {:ok, available_providers} <- ProviderRegistry.get_providers_for_domain(domain),
         {:ok, routing_decision} <-
           apply_routing_strategy(available_providers, domain, requirements) do
  ...
  defp apply_routing_strategy(available_providers, domain, requirements)
       when map_size(available_providers) > 0 do
      ...
      anthropic_provider ->
        {:ok,
         build_routing_decision(
           ...
           anthropic_provider,
           :constitutional_ai_first,
           "Constitutional AI provider selected"
         )}
    end
  end
  ...
  defp select_quality_first_provider(providers, domain, requirements) do
    ...
    preferred_provider =
      case domain do
        # Claude excels at evaluation
        :evaluation -> :anthropic
        # GPT good at orchestration
  ...
  defp score_provider_balanced({provider_type, provider_info}, domain, requirements) do
    ...
    provider_bonus =
      case {provider_type, domain} do
        # Claude good for evaluation
        {:anthropic, :evaluation} -> 0.3
        # GPT good for orchestration
        {:openai, :orchestration} -> 0.3
        _ -> 0.0
      end
  ...
  defp calculate_simple_score(provider_type, domain) do
    case {provider_type, domain} do
      ...
      # Good for planning
      {:anthropic, :planning} -> 0.90
      # Excellent for orchestration
      {:openai, :orchestration} -> 0.95
      # Good for evaluation
      {:openai, :evaluation} -> 0.85
      # Good for simple communication
      {:ollama, :communication} -> 0.80
      _ -> 0.75
    end
  end
