defmodule RubberDuck.LlmProviders.Adapters.OrchestrationAdapter do
  ...
  """
  def orchestrate(prompt, orchestration_type, user_id, project_id \\ nil, options \\ %{})
      when orchestration_type in @orchestration_types do
    Logger.debug(
      "Orchestrating agents using universal provider adapter for type: #{orchestration_type}"
    )

    # Build orchestration context with cost optimization
    orchestration_context =
      build_orchestration_context(prompt, orchestration_type, user_id, project_id, options)

    # Determine cost optimization requirements
    cost_optimization_required =
      orchestration_type in @cost_sensitive_types or
        Map.get(options, :cost_optimization_required, true)

    # Select provider with cost optimization preference
    provider_options =
      build_orchestration_provider_options(
        orchestration_type,
        cost_optimization_required,
        options
      )
    ...
    case UniversalProviderService.complete(prompt, :orchestration, provider_options) do
      {:ok, universal_response} ->
        ...
        orchestration_response =
          adapt_universal_response_to_orchestration(universal_response, orchestration_context)
  ...
  def agent_conversation(conversation_history, user_id, project_id \\ nil, options \\ %{}) do
    ...
    conversation_context =
      build_conversation_context(conversation_history, user_id, project_id, options)
    ...
    case UniversalProviderService.complete(conversation_history, :orchestration, provider_options) do
      {:ok, universal_response} ->
        conversation_response =
          adapt_universal_response_to_conversation(universal_response, conversation_context)
  ...
  def orchestrate_streaming(
    ...
    orchestration_callback =
      create_orchestration_streaming_callback(callback, orchestration_context)
    ...
    case UniversalProviderService.stream(
           prompt,
           :orchestration,
           orchestration_callback,
           provider_options
  ...
  def estimate_orchestration_cost(
    ...
    case UniversalProviderService.estimate_cost(prompt, :orchestration, provider_options) do
      {:ok, base_cost} ->
        ...
        optimized_cost =
          apply_orchestration_cost_optimization(base_cost, orchestration_type, options)
  ...
  defp build_orchestration_context(prompt, orchestration_type, user_id, project_id, options) do
    %{
      ...
      cost_priority: Map.get(options, :cost_priority, 0.6),
      # Lower quality threshold for efficiency
      quality_threshold: Map.get(options, :quality_threshold, 0.7),
      # Shorter responses for orchestration
  ...
  defp build_orchestration_provider_options(
         orchestration_type,
         cost_optimization_required,
         options
       ) do
    base_options = %{
      ...
      max_tokens: Map.get(options, :max_tokens, 1000),
      # Medium temperature for orchestration
    ...
    routing_options =
      if cost_optimization_required do
  ...
  defp build_conversation_provider_options(conversation_history, options) do
    ...
    max_tokens =
      case conversation_length do
        # Shorter responses for long conversations
        length when length > 10 -> 800
        # Medium responses
        length when length > 5 -> 1000
        # Longer responses for short conversations
        _ -> 1200
      end
    ...
    %{
      ...
      # Balanced creativity for conversation
      temperature: 0.4,
      # Optimize for conversation efficiency
  ...
  defp adapt_universal_response_to_orchestration(universal_response, orchestration_context) do
    ...
    %{
      ...
      # Preserve metadata
      metadata:
        Map.merge(universal_response.metadata, %{
        ...
        case Map.get(stream_event, :type) do
          ...
          _ ->
            Map.put(
              stream_event,
              :orchestration_context,
              orchestration_context.orchestration_type
            )
        end
  ...
  defp apply_orchestration_cost_optimization(base_cost, orchestration_type, options) do
    ...
    optimization_factor =
      case orchestration_type do
        # 20% discount for bulk operations
        :bulk_operations -> 0.8
        # 10% discount for frequent use
        :frequent_coordination -> 0.9
        # 15% discount for background tasks
    ...
    volume_factor =
      case Map.get(options, :volume_tier, :standard) do
