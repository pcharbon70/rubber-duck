defmodule RubberDuck.LlmProviders.Adapters.EvaluationAdapter do
  ...
  """
  def evaluate_code(code, evaluation_type, user_id, project_id \\ nil, options \\ %{})
      when evaluation_type in @evaluation_types do
    ...
    evaluation_context =
      build_evaluation_context(code, evaluation_type, user_id, project_id, options)

    # Determine if Constitutional AI is required
    constitutional_ai_required =
      evaluation_type in @constitutional_ai_types or
        Map.get(options, :constitutional_ai_required, false)

    # Select provider with Constitutional AI preference if needed
    provider_options =
      build_provider_selection_options(evaluation_type, constitutional_ai_required, options)
    ...
    case UniversalProviderService.complete(code, :evaluation, provider_options) do
      {:ok, universal_response} ->
        ...
        evaluation_response =
          adapt_universal_response_to_evaluation(universal_response, evaluation_context)
      ...
      {:ok, providers} ->
        evaluation_providers =
          ...
          |> Enum.map(fn {type, provider_config} ->
            {type,
             %{
               supports_constitutional_ai:
                 :constitutional_ai in Map.get(provider_config, :specializations, []),
               # Assume universal providers support streaming
  ...
  defp determine_evaluation_features(evaluation_type, options) do
    ...
    constitutional_features =
      if evaluation_type in @constitutional_ai_types do
  ...
  defp build_provider_selection_options(evaluation_type, constitutional_ai_required, options) do
    ...
    routing_options =
      if constitutional_ai_required do
  ...
  defp get_provider_evaluation_strengths(provider_type) do
    case provider_type do
      :anthropic ->
        [:constitutional_ai, :safety_analysis, :thoughtful_evaluation, :bias_mitigation]

      :openai ->
        [:general_evaluation, :performance_analysis, :versatile_assessment]

      :ollama ->
        [:privacy_focused, :local_evaluation, :cost_free]
