defmodule RubberDuck.LlmProviders.UniversalProviderRegistry do
  ...
  @health_check_interval 15_000  # Improved: 15 seconds vs previous 30s
  ...
  """
  def register_provider(provider_type, module, config, supported_domains \\ @supported_domains) when provider_type in @provider_types do
    GenServer.call(__MODULE__, {:register_provider, provider_type, module, config, supported_domains})
  end
  ...
  """
  def get_available_providers_for_domain(domain, requirements \\ %{}) when domain in @supported_domains do
  ...
  """
  def mark_provider_unhealthy(provider_type, reason \\ "Manual intervention", affected_domains \\ @supported_domains) do
  ...
  @impl true
  def handle_call({:register_provider, provider_type, module, config, supported_domains}, _from, state) do
    Logger.info("Registering universal provider: #{provider_type} for domains: #{inspect(supported_domains)}")
    
    case initialize_universal_provider(module, config, supported_domains) do
      {:ok, provider_state} ->
        # Get universal provider capabilities
        capabilities = get_universal_provider_capabilities(module, provider_state, supported_domains)
        ...
        updated_domain_health = initialize_domain_health(state.domain_health, provider_type, supported_domains)
        
        # Initialize usage stats
        updated_usage_stats = initialize_usage_stats(state.domain_usage_stats, provider_type, supported_domains)
        ...
        Phoenix.PubSub.broadcast(RubberDuck.PubSub, "provider_registry_events", 
  ...
  def handle_call(:get_providers, _from, state) do
    providers_with_health = state.providers
    |> Enum.map(fn {type, provider_info} ->
      ...
      enhanced_info = Map.merge(provider_info, %{
  ...
  def handle_cast({:record_usage, provider_type, domain, usage_data}, _from, state) do
    ...
    updated_usage_stats = state.domain_usage_stats
    ...
    # Update analytics
    updated_analytics = state.analytics
  ...
  def handle_cast({:mark_unhealthy, provider_type, reason, affected_domains}, state) do
    Logger.warning("Marking provider #{provider_type} as unhealthy for domains #{inspect(affected_domains)}: #{reason}")
    
    # Update domain health for affected domains
    updated_domain_health = Enum.reduce(affected_domains, state.domain_health, fn domain, acc ->
      put_in(acc, [provider_type, domain, :status], :unhealthy)
    end)
    
    # Update failure counts
    updated_failure_counts = Enum.reduce(affected_domains, state.failure_counts, fn domain, acc ->
      Map.update(acc, {provider_type, domain}, 1, &(&1 + 1))
    end)
    
    # Broadcast health changes
    Phoenix.PubSub.broadcast(RubberDuck.PubSub, "provider_health_changes",
  ...
  def handle_info(:universal_health_check, state) do
    ...
    updated_state = Enum.reduce(state.providers, state, fn {provider_type, provider_info}, acc_state ->
      health_results = perform_comprehensive_health_check(provider_info)
  ...
  defp initialize_universal_provider(module, config, supported_domains) do
    ...
    case UniversalProviderInterface.validate_universal_provider_config(get_provider_type_from_module(module), config) do
      :ok ->
        try do
  ...
  defp initialize_domain_health(domain_health, provider_type, supported_domains) do
    domain_health_map = supported_domains
    |> Enum.map(fn domain -> 
      {domain, %{
        status: :unknown,
        last_check: nil,
  ...
  defp initialize_usage_stats(usage_stats, provider_type, supported_domains) do
    domain_usage_map = supported_domains
    ...
    case domain do
      :evaluation ->
        %{
          ...
          constitutional_ai_available: :constitutional_ai in Map.get(response, :specialized_features, [])
        }
  ...
  defp meets_domain_requirements?(provider_info, domain, requirements) do
    ...
    token_requirements_met = case Map.get(requirements, :max_tokens) do
      nil -> true
      required -> required <= Map.get(capabilities, :max_context_tokens, 4096)
    end
    
    streaming_supported = case Map.get(requirements, :streaming) do
      true -> Map.get(capabilities, :supports_streaming, false)
      _ -> true
    end
    
    specialized_features_supported = case Map.get(requirements, :specialized_features) do
    ...
    domain_supported and token_requirements_met and streaming_supported and specialized_features_supported
  end
  ...
  defp build_comprehensive_analytics(state) do
    ...
    domain_analytics = @supported_domains
    |> Enum.map(fn domain ->
      ...
      {domain, %{
        ...
        avg_requests_per_provider: if(domain_providers > 0, do: domain_requests / domain_providers, else: 0)
      }}
    end)
    |> Enum.into(%{})
    ...
    Map.merge(state.analytics, %{
      ...
      health_percentage: if(total_providers > 0, do: healthy_providers / total_providers * 100, else: 0),
    ...
    Enum.each(configs, fn {provider_type, config} ->
      if Map.get(config, :enabled, false) do
        ...
        case register_provider(provider_type, provider_module, config, supported_domains) do
          :ok -> Logger.info("Auto-registered universal provider: #{provider_type}")
          {:error, reason} -> Logger.warning("Failed to auto-register #{provider_type}: #{inspect(reason)}")
        end
      end
    end)
  end
  ...
  defp get_provider_supported_domains(provider_type, config) do
    ...
    base_domains = case provider_type do
      :openai -> [:evaluation, :orchestration, :planning, :communication]
      :anthropic -> [:evaluation, :orchestration, :planning]  # Constitutional AI focus
      :ollama -> [:evaluation, :communication]  # Local, privacy-focused
      _ -> [:evaluation]
    end
  ...
  def handle_call(:shutdown_all, _from, state) do
    ...
    Enum.each(state.providers, fn {provider_type, provider_info} ->
      try do
        ...
        error ->
          Logger.error("Error shutting down universal provider #{provider_type}: #{Exception.message(error)}")
      end
    end)
