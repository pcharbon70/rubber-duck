defmodule RubberDuck.LlmProviders.UniversalProviderService do
  ...
  defp estimate_provider_cost(provider_info, content, domain, options) do
    ...
    estimated_tokens = div(content_length, 4) + 300
  ...
  defp get_domain_cost_multiplier(domain) do
    case domain do
      # Evaluation might need more detailed analysis
      :evaluation -> 1.2
      # Orchestration might get bulk pricing
      :orchestration -> 0.9
      _ -> 1.0
    end
  end
        ...
        case VerdictConfigurationResolver.resolve_configuration(
          ...
          {:ok, config} ->
            {:ok,
             %{
               preferred_providers:
                 Map.get(config, :preferred_providers, ["openai", "anthropic"]),
