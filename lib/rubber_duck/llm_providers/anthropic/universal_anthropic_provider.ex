defmodule RubberDuck.LlmProviders.Anthropic.UniversalAnthropicProvider do
  ...
  @supported_domains [:evaluation, :orchestration, :planning]  # Claude excels at thoughtful domains
  @supported_models %{
  ...
  @default_config %{
    ...
    timeout_ms: 45_000  # Claude can be slower but more thorough
  }
  ...
  def initialize(config) do
    ...
    case validate_universal_anthropic_config(merged_config) do
      :ok ->
        ...
        Logger.info("Universal Anthropic provider with Constitutional AI initialized successfully")
        {:ok, state}
  ...
  def process_request(state, request) do
    ...
    with {:ok, validated_request} <- validate_universal_request(request),
         ...
         {:ok, claude_request} <- build_claude_request(validated_request, model, constitutional_check),
         {:ok, response} <- make_anthropic_api_call(claude_request, state.config) do
      
      # Apply Constitutional AI post-processing
      processed_response = apply_constitutional_ai_processing(response, constitutional_check, validated_request.context)
      
      # Parse and format response for domain
      universal_response = parse_universal_anthropic_response(processed_response, validated_request.context, model)
      
      # Update stats including Constitutional AI usage
      updated_state = update_universal_stats(state, validated_request.context.domain, universal_response, constitutional_check)
  ...
  def process_streaming_request(state, request, callback) when is_function(callback) do
    Logger.debug("Processing universal Anthropic streaming request for domain: #{request.context.domain}")
    
    with {:ok, validated_request} <- validate_universal_request(request),
         {:ok, constitutional_check} <- perform_constitutional_ai_check(state, validated_request),
         {:ok, model} <- select_model_for_request(state, validated_request),
         {:ok, claude_request} <- build_streaming_claude_request(validated_request, model, constitutional_check) do
      ...
      case make_anthropic_streaming_call(claude_request, constitutional_callback, state.config) do
        {:ok, final_response} ->
          processed_response = apply_constitutional_ai_processing(final_response, constitutional_check, validated_request.context)
          universal_response = parse_universal_anthropic_response(processed_response, validated_request.context, model)
  ...
  def get_capabilities(_state) do
    capabilities = %{
      ...
      max_context_tokens: 200_000,       # Claude's large context advantage
      supported_domains: @supported_domains,
      supported_use_cases: [
        :security_evaluation, :quality_assessment, :safety_analysis,  # Evaluation domain
        :ethical_orchestration, :responsible_coordination,  # Orchestration domain
        :constitutional_planning, :ethical_reasoning,  # Planning domain
        :helpful_communication  # Communication domain (limited)
      ],
      cost_per_1k_tokens: @supported_models,
      rate_limits: @default_config.rate_limits,
      specialized_features: [
        :constitutional_ai, :safety_checks, :bias_mitigation, :content_filtering,
        :ethical_guidelines, :large_context, :thoughtful_analysis, :safety_first_evaluation
      ]
    }
  ...
  defp assess_content_safety(content, context) do
    ...
    detected_patterns = Enum.filter(concerning_patterns, fn pattern ->
      String.contains?(String.downcase(content_text), pattern)
    end)
    
    safety_score = case {length(detected_patterns), context.domain} do
      {0, _} -> 1.0
      {count, :evaluation} when count <= 2 -> 0.8  # Security evaluation may mention vulnerabilities
  ...
  defp assess_potential_bias(request) do
    ...
    %{
      bias_risk: :low,  # Claude is designed to minimize bias
  ...
  defp assess_ethical_alignment(context) do
    ...
    domain_ethics = case context.domain do
      :evaluation -> %{
  ...
  defp assess_domain_safety(domain, use_case) do
    ...
    case {domain, use_case} do
      {:evaluation, :security} -> 
        %{safety_level: :high, constitutional_guidance: "Security analysis with safety principles"}
      
      {:evaluation, _} ->
        %{safety_level: :high, constitutional_guidance: "Code evaluation with constructive feedback"}
      ...
      _ ->
        %{safety_level: :standard, constitutional_guidance: "General Constitutional AI principles"}
    end
  end
  ...
  defp select_model_for_request(state, request) do
    ...
    model = case {domain, use_case} do
      ...
      {:evaluation, :security} -> Map.get(models, :evaluation_comprehensive, "claude-3-opus-20240229")
      {:evaluation, :quality} -> Map.get(models, :evaluation_detailed, "claude-3-5-sonnet-20241022")
      {:evaluation, _} -> Map.get(models, :evaluation_screening, "claude-3-haiku-20240307")
      
      # Orchestration domain - balance ethics and efficiency  
      {:orchestration, :ethical} -> Map.get(models, :orchestration_ethical, "claude-3-opus-20240229")
      {:orchestration, _} -> Map.get(models, :orchestration_standard, "claude-3-5-sonnet-20241022")
      
      # Planning domain - prioritize reasoning with ethics
      {:planning, _} -> Map.get(models, :planning_complex, "claude-3-opus-20240229")
      
      # Default
      _ -> Map.get(models, :evaluation_screening, "claude-3-haiku-20240307")
    end
  ...
  defp build_constitutional_evaluation_request(request, model, constitutional_check) do
    ...
    user_prompt = case request.content do
  ...
  defp build_constitutional_orchestration_request(request, model, constitutional_check) do
    ...
    {:ok, %{
      ...
      temperature: 0.2  # Slightly creative for orchestration
    }}
  end
  ...
  defp make_anthropic_api_call(claude_request, config) do
    ...
    Process.sleep(150)  # Constitutional AI processing overhead
    ...
    case context.domain do
        ...
        case Jason.decode(content) do
          {:ok, eval_data} ->
            %{
              ...
              recommendations: enhance_recommendations_with_constitutional_ai(Map.get(eval_data, "recommendations", [])),
      ...
      :orchestration ->
        %{
          ...
          cost_efficiency: 0.8,
          ethical_score: 0.95,  # Constitutional AI ensures ethical coordination
      ...
      :planning ->
        %{
          reasoning_depth: 0.9,   # Claude excels at reasoning
          plan_quality: 0.85,
          ethical_considerations: 0.95,  # Constitutional AI for responsible planning
          planning_data: %{
            constitutional_ai_enhanced: true,
  ...
  defp enhance_recommendations_with_constitutional_ai(recommendations) when is_list(recommendations) do
    Enum.map(recommendations, fn rec ->
      # Ensure recommendations are constructive and safe
      enhanced_rec = String.replace(rec, ~r/\b(avoid|don't|never)\s+/i, "consider alternatives to ")
  ...
  defp estimate_tokens_for_request(request, model) do
    content_tokens = case request.content do
      content when is_binary(content) -> trunc(String.length(content) / 3.5)  # Claude tokenization more efficient
      content when is_list(content) ->
        content
    ...
    domain_overhead = case request.context.domain do
      :evaluation -> 1000  # Detailed evaluation with Constitutional AI
      :orchestration -> 500 # Orchestration with ethical guidelines
      :planning -> 800     # Planning with responsibility considerations
      _ -> 400
    end
    
    response_estimate = 400  # Claude provides detailed responses
    
    total_estimate = content_tokens + constitutional_overhead + domain_overhead + response_estimate
  ...
  defp estimate_claude_input_tokens(claude_request) do
    ...
    message_tokens = claude_request.messages
    |> Enum.map(fn msg -> trunc(String.length(Map.get(msg, :content, "")) / 3.5) end)
    |> Enum.sum()
  ...
  defp get_domain_avg_response_time(domain) do
    case domain do
      :evaluation -> 4000    # Evaluation with Constitutional AI can be thorough
      :orchestration -> 3000 # Ethical orchestration processing
      :planning -> 5000      # Constitutional planning is comprehensive
      _ -> 3500
    end
  end
  ...
  defp update_universal_stats(state, domain, response, constitutional_check) do
    ...
    constitutional_ai_increment = if constitutional_check.constitutional_compliance, do: 1, else: 0
    ...
    |> Map.update(domain, %{}, fn domain_stats ->
      %{
        ...
        constitutional_ai_usage: Map.get(domain_stats, :constitutional_ai_usage, 0) + constitutional_ai_increment,
    ...
    updated_stats = %{state.stats |
      ...
      constitutional_ai_checks: state.stats.constitutional_ai_checks + constitutional_ai_increment
    }
  ...
  def get_constitutional_ai_features do
    [
      :safety_checks, :bias_mitigation, :content_filtering,
      :ethical_guidelines, :helpful_harmless_honest, :responsible_ai
    ]
  end
  ...
  def recommend_model_for_domain(domain, use_case, constitutional_ai_required? \\ true) do
    case {domain, use_case, constitutional_ai_required?} do
      {:evaluation, :security, true} -> "claude-3-opus-20240229"      # Highest Constitutional AI capability
      {:evaluation, _, true} -> "claude-3-5-sonnet-20241022"         # Balanced Constitutional AI
      {:orchestration, :ethical, true} -> "claude-3-opus-20240229"   # Ethical orchestration
      {:planning, _, true} -> "claude-3-opus-20240229"               # Responsible planning
      _ -> "claude-3-haiku-20240307"                                  # Efficient option with Constitutional AI
    end
  end
end