defmodule RubberDuck.LlmProviders.UniversalHealthMonitor do
  ...
  @health_check_interval 15_000      # Improved: 15 seconds (vs 30s in original systems)
  @performance_window_minutes 60     # 1 hour performance tracking
  @cross_domain_analysis_interval 300_000  # 5 minutes for cross-domain analysis
  
  @alert_thresholds %{
    success_rate_critical: 0.90,
    success_rate_warning: 0.95,
    response_time_critical: 10_000,   # 10 seconds
    response_time_warning: 5_000,     # 5 seconds
    cost_efficiency_warning: 0.6,    # Cost efficiency threshold
    cross_domain_variance_warning: 0.3  # Performance variance across domains
  }
  ...
  def handle_call(:get_universal_health_status, _from, state) do
    ...
    universal_status = state.domain_health_data
  ...
  def handle_call({:get_domain_health, provider_type, domain}, _from, state) do
    ...
    case domain_health do
      ...
      health_data ->
        detailed_health = Map.merge(health_data, %{
          ...
          domain_specific_recommendations: generate_domain_recommendations(domain, health_data, domain_performance)
        })
  ...
  def handle_cast({:record_universal_usage, provider_type, domain, usage_result}, state) do
    ...
    updated_history = state.performance_history
    |> Map.update(provider_type, %{}, fn provider_history ->
      Map.update(provider_history, domain, [usage_entry], fn domain_history ->
        ...
        recent_entries = Enum.filter(domain_history, &(DateTime.compare(&1.timestamp, cutoff_time) != :lt))
        [usage_entry | recent_entries] |> Enum.take(1000)
      end)
    end)
    
    # Update domain health data
    updated_domain_health = update_domain_health_from_usage(state.domain_health_data, provider_type, domain, usage_entry)
    
    # Update cross-domain performance analysis
    updated_cross_domain = update_cross_domain_performance(state.cross_domain_performance, provider_type, usage_entry)
    ...
    check_and_send_universal_health_alerts(provider_type, domain, usage_entry, state.alert_thresholds)
  ...
  def handle_info(:universal_health_monitoring, state) do
    if state.monitoring_enabled do
      ...
      case UniversalProviderRegistry.get_providers() do
        ...
        {:error, reason} ->
          Logger.error("Failed to get providers for universal health monitoring: #{inspect(reason)}")
          schedule_universal_health_monitoring()
          {:noreply, state}
      end
    else
  ...
  defp perform_universal_health_analysis(providers, state) do
    Enum.reduce(providers, state, fn {provider_type, provider_info}, acc_state ->
      ...
      domain_health_results = provider_info.supported_domains
      |> Enum.map(fn domain ->
      ...
      # Update domain health data
      updated_domain_health = Map.put(acc_state.domain_health_data, provider_type, domain_health_results)
  ...
  defp analyze_domain_health(domain_performance, domain) do
    if Enum.empty?(domain_performance) do
      ...
      avg_response_time = Enum.reduce(domain_performance, 0, & &1.response_time_ms + &2) / total_requests
  ...
  defp determine_domain_health_status(success_rate, avg_response_time, domain) do
    ...
    domain_thresholds = case domain do
      :evaluation -> %{success_threshold: 0.95, response_threshold: 8000}   # Evaluation needs high accuracy
      :orchestration -> %{success_threshold: 0.90, response_threshold: 5000}  # Orchestration needs speed
      :planning -> %{success_threshold: 0.85, response_threshold: 15000}    # Planning can be slower
      _ -> %{success_threshold: 0.90, response_threshold: 10000}
    end
    
    cond do
      success_rate >= domain_thresholds.success_threshold and avg_response_time <= domain_thresholds.response_threshold ->
        :healthy
  ...
  defp get_default_domain_metrics(domain) do
    case domain do
      :evaluation -> %{constitutional_ai_usage: 0.0, evaluation_types_handled: [], avg_confidence: 0.0}
      :orchestration -> %{agent_communications: 0, cost_efficiency: 0.0, orchestration_complexity: 0.0}
      :planning -> %{planning_tasks: 0, reasoning_depth: 0.0, plan_success_rate: 0.0}
      _ -> %{}
    end
  end
  ...
  defp calculate_domain_specific_metrics(domain_performance, domain) do
    case domain do
      :evaluation ->
        constitutional_ai_usage = Enum.count(domain_performance, fn entry ->
          :constitutional_ai in Map.get(entry, :specialized_features_used, [])
        end) / length(domain_performance)
        
        evaluation_types = domain_performance
      ...
      :planning ->
        ...
        %{
          ...
          plan_success_rate: Enum.count(domain_performance, & &1.success) / length(domain_performance)
        }
  ...
  defp calculate_domain_performance_trend(domain_performance) do
    if length(domain_performance) < 5 do
      ...
      older_success = if Enum.empty?(older_performance) do
        recent_success
      else
  ...
  defp update_domain_health_from_usage(domain_health_data, provider_type, domain, usage_entry) do
    current_domain_health = get_in(domain_health_data, [provider_type, domain], %{
      status: :unknown,
      consecutive_successes: 0,
      consecutive_failures: 0
    })
    
    updated_domain_health = if usage_entry.success do
  ...
  defp update_cross_domain_performance(cross_domain_performance, provider_type, usage_entry) do
    current_cross_domain = Map.get(cross_domain_performance, provider_type, %{
  ...
  defp perform_cross_domain_analysis(state) do
    ...
    updated_cross_domain = state.providers
    |> Enum.reduce(state.cross_domain_performance, fn {provider_type, provider_info}, acc ->
      cross_domain_metrics = analyze_provider_cross_domain_performance(state.performance_history, provider_type, provider_info.supported_domains)
  ...
  defp analyze_provider_cross_domain_performance(performance_history, provider_type, supported_domains) do
    provider_performance = Map.get(performance_history, provider_type, %{})
    
    domain_metrics = supported_domains
    |> Enum.map(fn domain ->
      domain_data = Map.get(provider_performance, domain, [])
      
      metrics = if Enum.empty?(domain_data) do
        %{requests: 0, avg_response_time: 0, success_rate: 0.0, avg_cost: 0.0}
      else
        %{
          requests: length(domain_data),
          avg_response_time: Enum.reduce(domain_data, 0, & &1.response_time_ms + &2) / length(domain_data),
    ...
    %{
      ...
      optimization_recommendations: generate_cross_domain_optimization_recommendations(domain_metrics)
    }
  end
  ...
  defp calculate_variance(values) when length(values) > 1 do
    ...
    sum_squared_diffs = Enum.reduce(values, 0, fn value, acc ->
  ...
  defp calculate_overall_consistency(response_times, success_rates) do
    response_consistency = 1.0 - min(1.0, calculate_variance(response_times) / 10000)  # Normalize
  ...
  defp analyze_domain_switching_patterns(provider_performance) do
    ...
    all_usage = provider_performance
    |> Map.values()
    |> List.flatten()
  ...
  defp check_and_send_universal_health_alerts(provider_type, domain, usage_entry, thresholds) do
    ...
    alerts = if not usage_entry.success do
      ["Universal provider #{provider_type} failed in #{domain} domain"] ++ alerts
    else
      alerts
    end
    
    alerts = if usage_entry.response_time_ms > thresholds.response_time_critical do
      ["Provider #{provider_type} response time critical in #{domain}: #{usage_entry.response_time_ms}ms"] ++ alerts
    else
      alerts
    end
    
    # Check cost efficiency for orchestration domain
    alerts = if domain == :orchestration and usage_entry.cost_usd > 0 do
      cost_efficiency = calculate_usage_cost_efficiency(usage_entry)
      if cost_efficiency < thresholds.cost_efficiency_warning do
        ["Provider #{provider_type} cost efficiency low in orchestration: #{Float.round(cost_efficiency, 2)}"] ++ alerts
      else
        alerts
      end
    else
  ...
  defp calculate_usage_cost_efficiency(usage_entry) do
    ...
    if usage_entry.cost_usd > 0 and usage_entry.tokens_used > 0 do
      usage_entry.tokens_used / usage_entry.cost_usd / 1000  # Tokens per dollar per 1K tokens
    else
      1.0
    end
  end
  ...
  defp send_universal_health_alerts(provider_type, domain, alerts) do
    ...
    Phoenix.PubSub.broadcast(RubberDuck.PubSub, "universal_provider_alerts", 
      {:universal_provider_alerts, provider_type, domain, alerts, DateTime.utc_now()})
    
    Logger.warning("Universal health alerts for #{provider_type}/#{domain}: #{Enum.join(alerts, "; ")}")
  end
  ...
  defp summarize_performance_across_domains(_), do: %{total_requests: 0, overall_success_rate: 0.0}
  ...
  defp generate_health_recommendations(domain_health, cross_domain_performance) do
    ...
    recommendations = if overall_consistency < 0.7 do
      ["High performance variance across domains - consider provider configuration review"] ++ recommendations
    else
      recommendations
    end
  ...
  defp generate_domain_recommendations(domain, health_data, performance_data) do
    ...
    recommendations = case {domain, health_data.status} do
      {:evaluation, :degraded} ->
        ["Consider enabling Constitutional AI features for improved evaluation quality"] ++ recommendations
      
      {:orchestration, :degraded} ->
        ["Consider cost optimization review for orchestration use case"] ++ recommendations
      
      {:planning, :unhealthy} ->
        ["Planning domain showing issues - consider using alternative provider for complex reasoning"] ++ recommendations
    ...
    if length(performance_data) > 5 do
      ...
      recommendations = if avg_response_time > 8000 do
        ["Response times elevated for #{domain} - consider model optimization"] ++ recommendations
      else
        recommendations
      end
    end
  ...
  defp compare_domain_performance(performance_history, provider_type) do
    ...
    if map_size(provider_performance) < 2 do
      ...
      domain_stats = provider_performance
      |> Enum.map(fn {domain, usage_list} ->
  ...
  defp identify_domain_optimization_opportunities(domain_stats) do
    ...
    high_cost_domains = domain_stats
    |> Enum.filter(fn {_domain, stats} -> stats.avg_cost > 0.5 end)
    |> Enum.map(fn {domain, _stats} -> domain end)
    
    opportunities = if not Enum.empty?(high_cost_domains) do
      ["High cost domains (#{Enum.join(high_cost_domains, ", ")}) - consider cost optimization"] ++ opportunities
    else
      opportunities
    end
    
    # Find domains with slow response times
    slow_domains = domain_stats
    |> Enum.filter(fn {_domain, stats} -> stats.avg_response_time > 8000 end)
    |> Enum.map(fn {domain, _stats} -> domain end)
    
    opportunities = if not Enum.empty?(slow_domains) do
      ["Slow response domains (#{Enum.join(slow_domains, ", ")}) - consider performance optimization"] ++ opportunities
    else
      opportunities
    end
  ...
  defp analyze_cross_domain_consistency(domain_health) do
    if map_size(domain_health) < 2 do
      ...
      consistency = case length(unique_statuses) do
        1 -> :perfect    # All domains same status
        2 -> :good       # Minor variations
        _ -> :poor       # High variability
      end
  ...
  defp identify_optimization_opportunities(cross_domain_data) do
    ...
    opportunities = if consistency < 0.7 do
      ["High cross-domain performance variance - consider unified configuration optimization"] ++ opportunities
    else
      opportunities
    end
    ...
    underperforming = domain_metrics
    ...
    opportunities = if not Enum.empty?(underperforming) do
      ["Underperforming domains: #{Enum.join(underperforming, ", ")} - needs attention"] ++ opportunities
    else
      opportunities
    end
  ...
  defp generate_cross_domain_optimization_recommendations(domain_metrics) do
    ...
    recommendations = domain_metrics
    |> Enum.reduce(recommendations, fn {domain, stats}, acc ->
      if domain != best_domain do
        suggestions = []
        
        suggestions = if stats.avg_response_time > best_stats.avg_response_time * 1.5 do
          ["Consider response time optimization for #{domain}"] ++ suggestions
        else
          suggestions
        end
        
        suggestions = if stats.avg_cost > best_stats.avg_cost * 1.3 do
