defmodule RubberDuck.LlmProviders.Orchestration.LearningEngine do
  ...
  # 5 minutes
  @learning_update_interval 300_000
  ...
  def handle_cast({:record_outcome, provider, request_data, outcome_data}, state) do
    ...
    updated_outcomes =
      ...
      |> Map.update(provider, [outcome_record], fn existing_outcomes ->
        # Keep last 1000
        [outcome_record | existing_outcomes] |> Enum.take(1000)
      end)

    # Update stats
    updated_stats = Map.update!(state.stats, :total_outcomes_recorded, &(&1 + 1))

    new_state = %{state | provider_outcomes: updated_outcomes, stats: updated_stats}
  ...
  def handle_call({:get_predictions, provider, request_characteristics}, _from, state) do
    ...
    predictions =
      case Map.get(state.learning_models, provider) do
        nil ->
  ...
  def handle_call(:get_insights, _from, state) do
    ...
    insights = %{
      provider_performance_summary:
        generate_provider_performance_summary(state.provider_outcomes),
  ...
  defp generate_learned_predictions(learning_model, request_characteristics) do
    ...
    %{
      ...
      performance_prediction:
        predict_performance_with_learning(learning_model, request_characteristics),
  ...
  defp predict_performance_with_learning(learning_model, request_characteristics) do
    ...
    baseline_performance =
      estimate_baseline_performance(learning_model.provider, request_characteristics)
  ...
  defp calculate_cost_learning_factor(outcomes) do
    ...
    if avg_accuracy > 0.8 do
      # Good accuracy, no adjustment needed
      1.0
    else
      # Adjust based on accuracy gap
      1.0 + (0.8 - avg_accuracy) * 0.2
    end
  end
  ...
  defp calculate_quality_improvement_factor(outcomes) do
    ...
    # Cap improvement factor
    max(0.0, min(0.2, avg_improvement))
  end
  ...
  defp calculate_performance_learning_factor(outcomes) do
    ...
    performance_variations =
      ...
      |> Enum.map(fn outcome ->
        ...
        # Better performance = higher factor
        baseline_time / max(actual_time, 100)
      end)

    avg_performance_factor = Enum.sum(performance_variations) / length(performance_variations)

    # Reasonable bounds
    max(0.5, min(2.0, avg_performance_factor))
  end
  ...
  defp estimate_outcome_quality_from_data(outcome) do
    ...
    response_length =
      get_in(outcome, [:outcome, :content])
      |> then(fn content ->
  ...
  defp identify_optimization_opportunities(provider_outcomes) do
    ...
    opportunities =
      ...
      |> Enum.reduce(opportunities, fn {provider, outcomes}, acc ->
        ...
        if avg_cost > 0.1 do
          [
            "Consider cost optimization for #{provider} provider (avg cost: $#{Float.round(avg_cost, 3)})"
            | acc
          ]
        else
          acc
    ...
    opportunities =
      ...
      |> Enum.reduce(opportunities, fn {provider, outcomes}, acc ->
        ...
        if avg_time > 8000 do
          [
            "Consider performance optimization for #{provider} provider (avg time: #{avg_time}ms)"
            | acc
          ]
        else
          acc
