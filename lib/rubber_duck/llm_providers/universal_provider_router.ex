defmodule RubberDuck.LlmProviders.UniversalProviderRouter do
  ...
  @routing_strategies [
    :cost_optimized,           # Minimize cost (from both systems)
    :quality_first,            # Maximize quality (from Verdict system)
    :balanced,                 # Balance cost/quality/speed (from both systems)
    :fastest,                  # Minimize response time (from Verdict system)
    :most_reliable,            # Maximize uptime/success rate (from Verdict system)  
    :constitutional_ai_first,  # Prioritize Constitutional AI (from Verdict system)
    :orchestration_optimized,  # Optimize for agent communication (from Preferences system)
    :cross_domain_efficient    # Optimize for cross-domain usage (new unified feature)
  ]
  ...
  def select_universal_provider(universal_request, options \\ []) do
    ...
    with {:ok, routing_config} <- get_universal_routing_configuration(universal_request.context),
         ...
         {:ok, routing_decision} <- apply_universal_routing_strategy(available_providers, universal_request, routing_config, options) do
      
      Logger.debug("Selected universal provider: #{routing_decision.provider} for #{universal_request.context.domain}")
      {:ok, routing_decision}
    else
      error ->
  ...
  def get_universal_provider_recommendations(universal_request) do
    Logger.debug("Getting universal provider recommendations for #{universal_request.context.domain}")
    
    with {:ok, routing_config} <- get_universal_routing_configuration(universal_request.context),
         {:ok, available_providers} <- get_domain_available_providers(universal_request) do
      
      recommendations = available_providers
      |> Enum.map(&build_universal_provider_recommendation(&1, universal_request, routing_config))
  ...
  defp get_universal_routing_configuration(context) do
    ...
    case context do
      ...
      %{domain: :orchestration, user_id: user_id, project_id: project_id} when not is_nil(user_id) ->
  ...
  defp adapt_verdict_config_for_universal_routing(verdict_config) do
    %{
      ...
      cost_weight: 0.3,  # Verdict focuses more on quality
      quality_weight: 0.5,
      speed_weight: 0.2,
  ...
  defp adapt_preferences_config_for_universal_routing(preferences_result) do
    %{
      strategy: :cost_optimized,  # Preferences system focuses on cost optimization
      preferred_providers: [to_string(preferences_result.provider)],
      cost_weight: 0.6,  # Higher cost focus for orchestration
      quality_weight: 0.3,
      speed_weight: 0.1,
      specialized_features: [:cost_optimization, :agent_communication],
      max_cost_per_request: 0.5,  # Lower cost tolerance for orchestration
  ...
  defp apply_universal_routing_strategy(available_providers, universal_request, routing_config, options) do
    strategy = Keyword.get(options, :strategy, routing_config.strategy)
    
    case strategy do
      :cost_optimized ->
        select_cost_optimized_universal_provider(available_providers, universal_request, routing_config)
      
      :quality_first ->
        select_quality_first_universal_provider(available_providers, universal_request, routing_config)
      ...
      :most_reliable ->
        select_most_reliable_universal_provider(available_providers, universal_request, routing_config)
      
      :constitutional_ai_first ->
        select_constitutional_ai_universal_provider(available_providers, universal_request, routing_config)
      
      :orchestration_optimized ->
        select_orchestration_optimized_provider(available_providers, universal_request, routing_config)
      
      :cross_domain_efficient ->
        select_cross_domain_efficient_provider(available_providers, universal_request, routing_config, options)
  ...
  defp select_cost_optimized_universal_provider(providers, request, config) do
    ...
    case provider_scores do
      [best_provider | _] ->
        {:ok, build_universal_routing_decision(best_provider, :cost_optimized, "Lowest cost universal provider")}
  ...
  defp select_quality_first_universal_provider(providers, request, config) do
    ...
    case provider_scores do
      [best_provider | _] ->
        {:ok, build_universal_routing_decision(best_provider, :quality_first, "Highest quality universal provider")}
  ...
  defp select_balanced_universal_provider(providers, request, config) do
    ...
    case provider_scores do
      [best_provider | _] ->
        {:ok, build_universal_routing_decision(best_provider, :balanced, "Best universal cost/quality/speed balance")}
  ...
  defp select_constitutional_ai_universal_provider(providers, request, config) do
    ...
    constitutional_providers = providers
    ...
    case constitutional_providers do
      ...
      constitutional_providers ->
        ...
        case provider_scores do
          [best_provider | _] ->
            {:ok, build_universal_routing_decision(best_provider, :constitutional_ai_first, "Constitutional AI optimized provider")}
  ...
  defp select_orchestration_optimized_provider(providers, request, config) do
    ...
    case provider_scores do
      [best_provider | _] ->
        {:ok, build_universal_routing_decision(best_provider, :orchestration_optimized, "Orchestration optimized provider")}
  ...
  defp select_cross_domain_efficient_provider(providers, request, config, options) do
    ...
    provider_scores = providers
    |> Enum.map(&score_provider_for_cross_domain_efficiency(&1, request, config, cross_domain_context))
    |> Enum.sort_by(& &1.cross_domain_efficiency_score, :desc)
    
    case provider_scores do
      [best_provider | _] ->
        {:ok, build_universal_routing_decision(best_provider, :cross_domain_efficient, "Cross-domain efficient provider")}
  ...
  defp score_provider_for_universal_cost({provider_type, provider_info}, request, _config) do
    ...
    %{
      ...
      cost_score: 1.0 / max(cost_estimate, 0.001),  # Inverse for scoring
  ...
  defp score_provider_universal_balanced({provider_type, provider_info}, request, config) do
    ...
    overall_score = 
      ...
      (domain_specialization * 0.1)  # Slight bonus for domain specialization
  ...
  defp score_provider_for_constitutional_ai({provider_type, provider_info}, request, config) do
    ...
    constitutional_ai_score = if :constitutional_ai in Map.get(capabilities, :specialized_features, []) do
      1.0  # Full Constitutional AI support
    else
      0.0  # No Constitutional AI support
    end
    
    # Consider safety-related capabilities
    safety_features = [:safety_checks, :bias_mitigation, :content_filtering]
    safety_score = safety_features
    ...
    %{
      ...
      quality_score: quality_score,
      overall_constitutional_score: constitutional_ai_score * 0.5 + safety_score * 0.3 + quality_score * 0.2
    }
  end
  ...
  defp score_provider_for_orchestration({provider_type, provider_info}, request, _config) do
    ...
    %{
      ...
      speed_score: speed_score,
      orchestration_optimization_score: cost_efficiency * 0.4 + communication_score * 0.4 + speed_score * 0.2
    }
  end
  
  defp score_provider_for_cross_domain_efficiency({provider_type, provider_info}, request, _config, cross_domain_context) do
    ...
    multi_domain_bonus = if length(Enum.filter(user_domains, &(&1 in provider_info.supported_domains))) > 1 do
      0.15
    else
      0.0
    end
  ...
  defp estimate_universal_token_count(request, domain) do
    content_length = case request.content do
    ...
    domain_tokens = case domain do
      :evaluation -> 800   # Evaluation context and structured response
      :orchestration -> 400  # Agent communication overhead
      :planning -> 600     # Planning context and reasoning
      :communication -> 200  # Simple communication overhead
      _ -> 300
    end
  ...
  defp get_domain_cost_rate(provider_info, domain, use_case) do
    ...
    # Get base cost rate
    base_rate = case provider_type do
      :openai -> 0.02   # Average GPT cost
      :anthropic -> 0.015  # Average Claude cost
      :ollama -> 0.0    # Local models
      _ -> 0.015
    end
    
    # Apply domain-specific adjustments
    domain_multiplier = case {domain, use_case} do
      {:evaluation, :security} -> 1.2    # Security analysis needs detailed models
      {:evaluation, :quality} -> 1.0     # Standard evaluation
      {:orchestration, :planning} -> 1.1  # Planning needs reasoning
      {:orchestration, :communication} -> 0.8  # Communication is simpler
      {:planning, _} -> 1.3               # Planning is complex
      _ -> 1.0
    end
  ...
  defp get_universal_provider_quality_score(provider_info, context) do
    ...
    base_quality = case {provider_type, domain} do
      {:anthropic, :evaluation} -> 0.95   # Claude excellent for evaluation
      {:anthropic, :orchestration} -> 0.90  # Claude good for orchestration
      {:openai, :evaluation} -> 0.90      # GPT good for evaluation
      {:openai, :orchestration} -> 0.92   # GPT excellent for orchestration
      {:ollama, :evaluation} -> 0.80      # Local models decent
      {:ollama, :communication} -> 0.85   # Local models good for simple communication
      _ -> 0.85
    end
    
    # Adjust for specialized use cases
    use_case_modifier = case {provider_type, use_case} do
      {:anthropic, :security} -> 0.05     # Claude excels at security
      {:openai, :performance} -> 0.03     # GPT good at performance analysis
      {:ollama, :privacy} -> 0.10         # Local models excellent for privacy
      _ -> 0.0
    end
  ...
  defp get_universal_provider_speed_score(provider_info, domain) do
    ...
    base_speed = case provider_type do
      :ollama -> 0.9      # Local models fastest
      :openai -> 0.8      # OpenAI generally fast
      :anthropic -> 0.7   # Claude thoughtful but slower
      _ -> 0.6
    end
    
    # Domain-specific speed requirements
    domain_speed_modifier = case domain do
      :communication -> 0.1  # Communication needs speed
      :orchestration -> 0.05 # Orchestration values speed
      :evaluation -> 0.0     # Evaluation can wait for quality
      :planning -> -0.1      # Planning can be slower
      _ -> 0.0
    end
  ...
  defp get_domain_specialization_score(provider_type, domain) do
    case {provider_type, domain} do
      {:anthropic, :evaluation} -> 0.9   # Claude specialized for thoughtful evaluation
      {:anthropic, :planning} -> 0.85    # Claude good at reasoning/planning
      {:openai, :orchestration} -> 0.9   # GPT excellent for orchestration
      {:openai, :communication} -> 0.8   # GPT good for communication
      {:ollama, :communication} -> 0.7   # Local models decent for simple communication
      _ -> 0.6
    end
  end
  ...
  defp get_provider_cost_efficiency(provider_type, domain) do
    ...
    case {provider_type, domain} do
      {:ollama, _} -> 1.0           # Local models most cost efficient
      {:openai, :orchestration} -> 0.8   # OpenAI decent efficiency for orchestration
      {:anthropic, :evaluation} -> 0.7   # Claude expensive but valuable for evaluation
      {:openai, :evaluation} -> 0.75     # OpenAI good balance for evaluation
      _ -> 0.6
    end
  end
  ...
  defp get_communication_optimization_score(provider_info) do
    ...
    function_calling_bonus = if Map.get(capabilities, :supports_function_calling, false), do: 0.15, else: 0.0
  ...
  defp get_provider_cross_domain_efficiency(provider_type, provider_info) do
    ...
    base_efficiency = case provider_type do
      :openai -> 0.85      # GPT handles multiple domains well
      :anthropic -> 0.80   # Claude good across domains
      :ollama -> 0.70      # Local models more limited
      _ -> 0.65
    end
  ...
  defp build_universal_routing_decision(provider_score, strategy, reasoning) do
    %{
      ...
      model: select_optimal_model_for_domain(provider_score.provider_info, provider_score[:domain]),
      strategy: strategy,
      reasoning: reasoning,
      ...
      expected_response_time_ms: get_expected_response_time(provider_score.provider_type, provider_score[:domain]),
      specialized_features_available: get_available_specialized_features(provider_score.provider_info),
      metadata: %{
        routing_timestamp: DateTime.utc_now(),
        universal_routing: true,
        alternative_providers: []  # Would include other viable options
      }
    }
  end
  ...
  defp calculate_universal_decision_confidence(provider_score) do
    ...
    confidence = if Map.has_key?(provider_score, :cost_estimate) do
      confidence + 0.05
    else
      confidence
    end
    
    confidence = if Map.has_key?(provider_score, :quality_score) do
      confidence + 0.05
    else
      confidence
    end
    
    confidence = if Map.has_key?(provider_score, :domain_specialization) do
      confidence + 0.05
    else
      confidence
    end
    
    confidence = if Map.has_key?(provider_score, :constitutional_ai_score) do
      confidence + 0.05
    else
      confidence
    end
  ...
  defp get_expected_response_time(provider_type, domain) do
    base_time = case provider_type do
      :ollama -> 1200      # Fast local inference
      :openai -> 2000      # Generally fast API
      :anthropic -> 3000   # Thoughtful but slower
      _ -> 2500
    end
    
    # Adjust for domain complexity
    domain_modifier = case domain do
      :evaluation -> 1.5   # Evaluation can take longer
      :planning -> 2.0     # Planning is complex
      :orchestration -> 1.0  # Standard orchestration
      :communication -> 0.7  # Communication should be fast
      _ -> 1.0
    end
  ...
  defp apply_universal_fallback_selection(universal_request, options) do
    ...
    case UniversalProviderRegistry.get_available_providers_for_domain(domain) do
      {:ok, all_providers} ->
        available_fallback = fallback_order
  ...
  defp validate_provider_health(provider_info, domain) do
    ...
    case domain_health.status do
      :healthy -> :ok
      :degraded -> :ok  # Accept degraded for now
  ...
  defp build_universal_provider_recommendation({provider_type, provider_info}, request, config) do
    ...
    %{
      ...
      recommendation_reason: generate_universal_recommendation_reason(provider_type, request.context.domain, quality_score, cost_estimate, speed_score, domain_specialization)
    }
  end
  
  defp generate_universal_recommendation_reason(provider_type, domain, quality_score, cost_estimate, speed_score, domain_specialization) do
    strengths = []
    ...
    strengths = if domain_specialization > 0.8, do: ["#{domain} specialized"] ++ strengths, else: strengths
    
    provider_description = case provider_type do
