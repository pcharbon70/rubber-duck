  ...
  defimpl RubberDuck.Protocol.Message do
    ...
    def route(%ProviderSelect{} = msg, context) do
      ...
      if Code.ensure_loaded?(RubberDuck.Agents.LLMOrchestratorAgent) do
        RubberDuck.Agents.LLMOrchestratorAgent.handle_instruction({:select_provider, msg}, context)
      else
    ...
    defp validate_request_type(type) when type in [:completion, :embedding, :chat, :function_call], do: :ok
    ...
    defp validate_quality_threshold(threshold) when is_float(threshold) and threshold >= 0 and threshold <= 1, do: :ok
