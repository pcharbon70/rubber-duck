  ...
  defimpl RubberDuck.Protocol.Message do
    ...
    def route(%SecurityScan{} = msg, context) do
      if Code.ensure_loaded?(RubberDuck.Skills.CodeAnalysis) do
        RubberDuck.Skills.CodeAnalysis.handle_security_scan(msg, context)
      else
    ...
    def priority(_), do: :high  # Security is always high priority
    ...
    defp validate_file_type(type) when type in [:elixir, :elixir_script, :javascript, :unknown], do: :ok
